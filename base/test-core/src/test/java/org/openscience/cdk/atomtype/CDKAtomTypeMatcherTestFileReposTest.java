...
/**
 * This class tests the matching of atom types defined in the cdk atom type list, using the test
 * files in <code>src/test/data</code>.
 *
...
public class CDKAtomTypeMatcherTestFileReposTest extends CDKTestCase {
    ...
    public void testPDBfiles() throws Exception {
        ...
        String[] testFiles = {
            "114D.pdb", "1CRN.pdb", "1D66.pdb", "1IHA.pdb", "1PN8.pdb",
        };
        int tested = 0;
        int failed = 0;
    ...
    private TestResults testFile(String dir, String filename, ISimpleChemObjectReader reader)
        ...
        for (IAtom atom : mol.atoms()) {
            ...
            if (matched == null) {
                results.failed++;
                System.out.println(
                        "Could not match atom: " + results.tested + " in file " + filename);
            } else
            ...
            if (!Objects.equals(atom.getSymbol(), matched.getSymbol())) {
                ...
                results.failed++;
                System.out.println(
                        "Symbol does not match: " + results.tested + " in file " + filename);
                System.out.println(
                        "Found: " + atom.getSymbol() + ", expected: " + matched.getSymbol());
            } else if (atom.getHybridization() != CDKConstants.UNSET
                    && atom.getHybridization() != matched.getHybridization()) {
                results.failed++;
                System.out.println(
                        "Hybridization does not match: " + results.tested + " in file " + filename);
                System.out.println(
                        "Found: "
                ...
                results.failed++;
                System.out.println(
                        "Formal charge does not match: " + results.tested + " in file " + filename);
                System.out.println(
                        "Found: "
                ...
                if (matched.getFormalNeighbourCount() != CDKConstants.UNSET
                    ...
                    System.out.println(
                            "Found: "
                                    + connectionCount
                                    + ", expected (max): "
                                    + matched.getFormalNeighbourCount()
                                    + " ("
                                    + matched.getAtomTypeName()
                                    + ")");
                }
                ...
                //            		results.failed++;
                //            		System.out.println("Number of neighbors is too high: " +
                // results.tested + " in file " + filename);
