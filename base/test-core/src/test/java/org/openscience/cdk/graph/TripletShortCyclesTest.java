...
public class TripletShortCyclesTest {
    ...
    public void empty() throws Exception {
        TripletShortCycles esssr =
                new TripletShortCycles(new MinimumCycleBasis(new int[0][]), false);
    ...
    public void anthracenePaths() {
        TripletShortCycles esssr =
                new TripletShortCycles(new MinimumCycleBasis(anthracene()), false);
        int[][] paths = esssr.paths();
        assertThat(paths[0].length - 1, is(6));
        assertThat(paths[1].length - 1, is(6));
    ...
    public void norbornanePaths() {
        TripletShortCycles esssr =
                new TripletShortCycles(new MinimumCycleBasis(norbornane()), false);
        int[][] paths = esssr.paths();
    ...
    /**
     * Ensures non-canonic really does give you all cycles. If we didn't use multiple shortest paths
     * here we would miss one of the larger cycles
     */
    @Test
    public void cyclophanePaths() {
        TripletShortCycles esssr =
                new TripletShortCycles(new MinimumCycleBasis(cyclophane()), false);
        assertThat(
                esssr.paths(),
                is(
                        new int[][] {
    ...
    /**
     * @cdk.inchi
     *     InChI=1S/C20H36/c1-2-6-18-13-15-20(16-14-18)8-4-3-7-19-11-9-17(5-1)10-12-19/h17-20H,1-16H2
     */
