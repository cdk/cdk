...
/**
 * Given a (subgraph-)isomorphism state this class can lazily iterate over the mappings in a
 * non-recursive manner. The class currently implements and {@link Iterator} but is better suited to
 * the {@code Stream} class (which will be available in JDK 8).
 *
 * @author John May
...
final class StateStream implements Iterator<int[]> {
    ...
    /**
     * Progress the state-machine - the function return false when a mapping is found on the mapping
     * is done.
     *
    ...
    /**
     * A fixed size stack to keep track of which vertices are mapped. This stack allows us to turn
     * the recursive algorithms it to lazy iterating mappers. A reclusive call is usually
     * implemented as call-stack which stores the variable in each subroutine invocation. For the
     * mapping we actually only need store the candidates.
     */
