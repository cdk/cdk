 ...
 *
 * <p>Note: This class is internal and will be private in future.
 *
 * @author John May
 * @cdk.module smarts
 * @cdk.githash
...
final class QueryStereoFilter implements Predicate<int[]> {
    ...
    private static int indexOf(int[] xs, int x) {
        for (int i = 0; i < xs.length; i++) if (xs[i] == x) return i;
        return -1;
    }

    /**
     * Verify the tetrahedral stereo-chemistry (clockwise/anticlockwise) of atom {@code u} is
    ...
    private boolean checkTetrahedral(int u, int[] mapping) {
        ...
        if (targetTypes[v] != null && targetTypes[v] != Type.Tetrahedral) return false;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (j < 0) us[indexOf(us, focusIdx)] = vs[i];
        }

        int parity =
                permutationParity(us) * permutationParity(vs) * parity(targetElement.getStereo());
        ...
        if (groupInfo != 0) {
            ...
            if (groupConfigAdjust[v] == 0) {
                ...
                int adjust = 1;
                if ((parity == -1 && !leftOk) || (parity == 1 && !rghtOk)) adjust = -1;
        ...
        if (parity < 0)
            ...
            return ((QueryAtom) queryAtom)
                    .getExpression()
                    .matches(targetAtom, IStereoElement.RIGHT);
        else return ((QueryAtom) queryAtom).getExpression().matches(targetAtom, 0);
    }
    ...
    private boolean checkGeometric(int u1, int u2, int[] mapping) {
        ...
        if (targetTypes[v1] != null && targetTypes[v1] != Type.Geometric) return false;
        if (targetTypes[v2] != null && targetTypes[v2] != Type.Geometric) return false;
        ...
        if (targetTypes[v1] == Type.Geometric && targetTypes[v2] == Type.Geometric) {
            IDoubleBondStereochemistry targetElement =
                    (IDoubleBondStereochemistry) targetElements[v1];
            ...
            else config = targetElement.getConfigOrder();
        } else {
    ...
    private IBond getMappedBond(IBond qbond, int[] mapping) {
        return target.getBond(
                target.getAtom(mapping[query.indexOf(qbond.getBegin())]),
