...
/**
 * A predicate for verifying component level grouping in query/target structure matching. The
 * grouping is used by SMARTS and is critical to querying reactions. The grouping specifies that
 * substructures in the query should match to separate components in the target. The grouping
 * specification is indicated by an {@code int[]} array of length (|V(query)| + 1). The final index
 * indicates the maximum component group (in the query). A specification of '0' indicates there are
...
final class ComponentFilter implements Predicate<int[]> {
    ...
    /** Key indicates where the grouping should be store in the query properties. */
    ...
    public ComponentFilter(IAtomContainer query, IAtomContainer target) {
        this(
                query.getProperty(KEY) == null
                        ? determineComponents(query, false)
    ...
    private static int[] determineComponents(IAtomContainer target, boolean auto) {
        ...
        if (target.isEmpty()) components = new int[0];
        // defined by reaction grouping
        if (components == null
                && target.getAtom(0).getProperty(CDKConstants.REACTION_GROUP) != null) {
            int max = 0;
        ...
        if (components == null && auto) {
            ...
            int max = 0;
            for (int grp : components) if (grp > max) max = grp;
            components[target.getAtomCount()] = max;
        }
        return components;
    }
    ...
    /**
     * Create a predicate to match components for the provided query (grouping) and target
