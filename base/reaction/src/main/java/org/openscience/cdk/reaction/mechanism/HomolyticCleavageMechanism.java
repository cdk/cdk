...
/**
 * This mechanism breaks the chemical bond between atoms. Generating two atoms with attached
 * radicals. It returns the reaction mechanism which has been cloned the {@link IAtomContainer}.
 *
...
public class HomolyticCleavageMechanism implements IReactionMechanism {
     ...
     * @param atomList The list of atoms taking part in the mechanism. Only allowed two atoms. Both
     *     atoms acquire a ISingleElectron
    ...
    public IReaction initiate(
        ...
        if (bondList.size() != 1) {
            throw new CDKException(
                    "HomolyticCleavageMechanism only expect one bond in the ArrayList");
        }
        ...
        reactantCloned.addSingleElectron(
                bond1.getBuilder().newInstance(ISingleElectron.class, atom1C));
        reactantCloned.addSingleElectron(
                bond1.getBuilder().newInstance(ISingleElectron.class, atom2C));
