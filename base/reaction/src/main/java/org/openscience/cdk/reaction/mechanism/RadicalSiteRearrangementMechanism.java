...
/**
 * This mechanism displaces an Atom or substructure (R) from one position to an other. It returns
 * the reaction mechanism which has been cloned the {@link IAtomContainer}.
 *
 * <p>This reaction could be represented as [A*]-(X)_n-Y-Z =&gt; A(Z)-(X)_n-[Y*]
 *
...
public class RadicalSiteRearrangementMechanism implements IReactionMechanism {
     ...
     *     first atom is the atom which must be moved and the second is the atom which receives the
     *     atom1 and the third is the atom which loss the first atom
    ...
    public IReaction initiate(
        ...
        if (atomContainerSet.getAtomContainerCount() != 1) {
            throw new CDKException(
                    "RadicalSiteRearrangementMechanism only expects one IAtomContainer");
        }
        if (atomList.size() != 3) {
            throw new CDKException(
                    "RadicalSiteRearrangementMechanism expects three atoms in the ArrayList");
        }
        if (bondList.size() != 1) {
            throw new CDKException(
                    "RadicalSiteRearrangementMechanism only expect one bond in the ArrayList");
        }
        ...
        IBond newBond =
                atom1.getBuilder().newInstance(IBond.class, atom1C, atom2C, IBond.Order.SINGLE);
        ...
        reactantCloned.addSingleElectron(
                atom2C.getBuilder().newInstance(ISingleElectron.class, atom3C));
