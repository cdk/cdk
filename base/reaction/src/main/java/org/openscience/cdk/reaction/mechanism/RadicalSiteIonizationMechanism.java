...
/**
 * This mechanism extracts an atom because of the stabilization of a radical. It returns the
 * reaction mechanism which has been cloned the IAtomContainer.
 *
 * <p>This reaction could be represented as Y-B-[C*] =&gt; [Y*] + B=C
 *
...
public class RadicalSiteIonizationMechanism implements IReactionMechanism {
     ...
     *     first atom is the atom which contains the ISingleElectron and the second third is the
     *     atom which will be removed the first atom
    ...
    public IReaction initiate(
        ...
        if (atomList.size() != 3) {
            throw new CDKException(
                    "RadicalSiteIonizationMechanism expects three atoms in the ArrayList");
        }
        if (bondList.size() != 2) {
            throw new CDKException(
