...
/**
 * IReactionProcess which produces a tautomerization chemical reaction. As most commonly
 * encountered, this reaction results in the formal migration of a hydrogen atom or proton,
    ...
    public IReactionSet initiate(IAtomContainerSet reactants, IAtomContainerSet agents)
        ...
        while (atoms.hasNext()) {
            ...
            if (atomi.getFlag(CDKConstants.REACTIVE_CENTER)
                    && (atomi.getFormalCharge() == CDKConstants.UNSET ? 0 : atomi.getFormalCharge())
                            == 0
                                ...
                                if (bondj.getFlag(CDKConstants.REACTIVE_CENTER)
                                    ...
                                    if (atomk.getFlag(CDKConstants.REACTIVE_CENTER)
                                            && (atomk.getFormalCharge() == CDKConstants.UNSET
                                                    ...
                                                    == 0
                                            && reactant.getConnectedSingleElectronsCount(atomk)
                                                    == 0) {
                                        Iterator<IBond> bondks =
                                                reactant.getConnectedBondsList(atomk).iterator();
                                        while (bondks.hasNext()) {
                                            ...
                                            if (bondk.getFlag(CDKConstants.REACTIVE_CENTER)
                                                ...
                                                if (atoml.getFlag(CDKConstants.REACTIVE_CENTER)
                                                    ...
                                                    break; // because of the others atoms are
                                                           // hydrogen too.
                                                }
                                            }
                                        }
                        ...
                        if ((atomj.getFormalCharge() == CDKConstants.UNSET
                            ...
                            while (bondjs.hasNext()) {
                                ...
                                if (bondj.getOrder() == IBond.Order.SINGLE) {
                                    IAtom atomk = bondj.getOther(atomj); // Atom pos 3
                                    if ((atomk.getFormalCharge() == CDKConstants.UNSET
                                                            ? 0
                                                            : atomk.getFormalCharge())
                                                    == 0
                                            && reactant.getConnectedSingleElectronsCount(atomk)
                                        ...
                                        while (bondks.hasNext()) {
                                            ...
                                            if (bondk.getOrder() == IBond.Order.SINGLE) {
                                                ...
                                                if (atoml.getSymbol().equals("H")) {
                                                    atomi.setFlag(
                                                            CDKConstants.REACTIVE_CENTER, true);
                                                    atomj.setFlag(
                                                            CDKConstants.REACTIVE_CENTER, true);
                                                    atomk.setFlag(
                                                            CDKConstants.REACTIVE_CENTER, true);
