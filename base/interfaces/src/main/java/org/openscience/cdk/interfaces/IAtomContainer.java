...
public interface IAtomContainer extends IChemObject, IChemObjectListener {
    ...
    static IAtom getCommonAtom(IBond a, IBond b) {
    ...
    static void updateCarriers(List<?> se, IChemObject org, IChemObject rep) {
    ...
    static void updateStereochemistry(IAtomContainer mol, IBond bondBeingRemoved) {
        ...
        Iterator<IStereoElement> iter = mol.stereoElements().iterator();
        while (iter.hasNext()) {
            ...
            if (focus.equals(beg)) {
                ...
                if (common != null && common.getBondCount() > 1) {
                    IBond other = null;
                    for (IBond bond : mol.getConnectedBondsList(common)) {
