...
/**
 * Small customization of ValencyHybridChecker suggested by Todd Martin specially tuned for SMILES
 * parsing.
 *
...
public class SmilesValencyChecker implements IValencyChecker, IDeduceBondOrderTool {
    ...
    public boolean saturate(IBond[] bonds, IAtomContainer atomContainer) throws CDKException {
        ...
        if (bonds.length > 0) {
            ...
            if (isSaturated(bond, atomContainer)) {
                logger.debug(
                        "OK, bond is saturated, now try to saturate remaining bonds (if needed)");
                ...
                logger.debug(
                        "Option 1: Saturating this bond directly, then trying to saturate rest");
                ...
                if (bondsAreFullySaturated) {
                    ...
                    bondsAreFullySaturated =
                            saturate(leftBonds, atomContainer) && isSaturated(bond, atomContainer);
    ...
    /**
     * Returns whether a bond is unsaturated. A bond is unsaturated if <b>all</b> Atoms in the bond
     * are unsaturated.
     */
     ...
     */
    public boolean saturateByIncreasingBondOrder(IBond bond, IAtomContainer atomContainer)
            throws CDKException {
        IAtom[] atoms = BondManipulator.getAtomArray(bond);
        IAtom atom = atoms[0];
    ...
    /**
     * Determines if the atom can be of type AtomType. That is, it sees if this AtomType only
     * differs in bond orders, or implicit hydrogen count.
     */
    public boolean couldMatchAtomType(
            IAtom atom, double bondOrderSum, IBond.Order maxBondOrder, IAtomType type) {
    ...
    /**
     * Calculates the number of hydrogens that can be added to the given atom to fullfil the atom's
     * valency. It will return 0 for PseudoAtoms, and for atoms for which it does not have an entry
     * in the configuration file.
     */
    public int calculateNumberOfImplicitHydrogens(
            IAtom atom, double bondOrderSum, IBond.Order maxBondOrder, int neighbourCount)
            throws CDKException {
    ...
    /**
     * Checks whether an Atom is saturated by comparing it with known AtomTypes. It returns true if
     * the atom is an PseudoAtom and when the element is not in the list.
     */
    @Override
