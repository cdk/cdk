...
abstract class AbstractStereo<F extends IChemObject, C extends IChemObject>
    ...
    AbstractStereo(F focus, C[] carriers, int value) {
        ...
        if (carriers.length != numCarriers(value))
            throw new IllegalArgumentException(
                    "Unexpected number of stereo carriers! expected "
                            + ((value >>> 12) & 0xf)
                            + " was "
                            + carriers.length);
        for (C carrier : carriers) {
            if (carrier == null) throw new NullPointerException("A carrier was undefined!");
        }
        this.value = value;
        this.focus = focus;
    ...
    public void setGroupInfo(int grp) {
        ...
        value |=
                (grp
                        & IStereoElement
                                .GRP_MASK); // set the new value ensure low bits aren't overwritten
    }
    ...
    public boolean contains(IAtom atom) {
        ...
        for (C carrier : carriers) {
            if (carrier.equals(atom)
                    || (carrier instanceof IBond && ((IBond) carrier).contains(atom))) return true;
        }
        return false;
    }
    ...
    public IStereoElement<F, C> map(Map<IAtom, IAtom> atoms, Map<IBond, IBond> bonds) {
        if (atoms == null) throw new IllegalArgumentException("Atom map should be non-null");
        if (bonds == null) throw new IllegalArgumentException("Bond map should be non-null");
    ...
    public IStereoElement<F, C> map(Map<IChemObject, IChemObject> chemobjs) {
        if (chemobjs == null) throw new NullPointerException("chemobj map was not provided!");
        ...
        for (int i = 0; i < newcarriers.size(); i++) {
            ...
            if (newcarrier != null) {
                ...
                if (newcarriers == carriers) newcarriers = new ArrayList<>(carriers);
                newcarriers.set(i, newcarrier);
            }
        }
        // no change, return self
        if (newfocus == focus && newcarriers == carriers) return this;
    ...
    public IChemObjectBuilder getBuilder() {
        if (builder == null) throw new UnsupportedOperationException("Non-domain object!");
        return this.builder;
    }
    ...
    protected static <T> T[] invapply(T[] src, int[] perm) {
        T[] res = src.clone();
        for (int i = 0; i < src.length; i++) res[i] = src[perm[i]];
        return res;
    }
}
