...
public class PathTools {
    ...
    /**
     * All-Pairs-Shortest-Path computation based on Floyd's algorithm {@cdk.cite FLO62}. It takes an
     * nxn matrix C of edge costs and produces an nxn matrix A of lengths of shortest paths.
     *
    ...
    /**
     * Recursively performs a depth first search in a molecular graphs contained in the
     * AtomContainer molecule, starting at the root atom and returning when it hits the target atom.
     *
     * <p>CAUTION: This recursive method sets the VISITED flag of each atom does not reset it after
     * finishing the search. If you want to do the operation on the same collection of atoms more
     * than once, you have to set all the VISITED flags to false before each operation by looping of
     * the atoms and doing a "atom.setFlag((CDKConstants.VISITED, false));"
     *
     * <p>Note that the path generated by the search will not contain the root atom, but will
     * contain the target atom
     *
     ...
     */
    public static boolean depthFirstTargetSearch(
            IAtomContainer molecule, IAtom root, IAtom target, IAtomContainer path) {
        List<IBond> bonds = molecule.getConnectedBondsList(root);
        IAtom nextAtom;
        root.setFlag(CDKConstants.VISITED, true);
        boolean first = path.isEmpty();
        if (first) path.addAtom(root);
        for (IBond bond : bonds) {
            ...
            if (!nextAtom.getFlag(CDKConstants.VISITED)) {
                ...
                if (nextAtom.equals(target)) {
                    if (first) path.removeAtomOnly(root);
                    return true;
                } else {
     ...
     * usually consists of one start atom. While searching the graph, the method marks each visited
     * atom. It then puts all the atoms connected to the atoms in the given sphere into a new vector
     * which forms the sphere to search for the next recursive method call. All atoms that have been
     * visited are put into a molecule container. This breadthFirstSearch does thus find the
     * connected graph for a given start atom.
     *
     * @param atomContainer The AtomContainer to be searched
     * @param sphere A sphere of atoms to start the search with
     * @param molecule A molecule into which all the atoms and bonds are stored that are found
     *     during search
     */
    public static void breadthFirstSearch(
            IAtomContainer atomContainer, List<IAtom> sphere, IAtomContainer molecule) {
        // logger.debug("Staring partitioning with this ac: " + ac);
        breadthFirstSearch(atomContainer, sphere, molecule, -1);
    }

    /**
     * Returns the atoms which are closest to an atom in an AtomContainer by bonds. If number of
     * atoms in or below sphere x&lt;max and number of atoms in or below sphere x+1&gt;max then
     ...
     * connected graph for a given start atom.
     *
     * <p>IMPORTANT: this method does not reset the VISITED flags, which must be done if calling
     * this method twice!
     *
     * @param atomContainer The AtomContainer to be searched
     * @param sphere A sphere of atoms to start the search with
     ...
     */
    public static void breadthFirstSearch(
            IAtomContainer atomContainer, List<IAtom> sphere, IAtomContainer molecule, int max) {
        IAtom nextAtom;
        ...
        for (IAtom atom : sphere) {
            ...
            List<ISingleElectron> singleElectrons =
                    atomContainer.getConnectedSingleElectronsList(atom);
    ...
    /**
     * Performs a breadthFirstTargetSearch in an AtomContainer starting with a particular sphere,
     * which usually consists of one start atom. While searching the graph, the method marks each
     * visited atom. It then puts all the atoms connected to the atoms in the given sphere into a
     * new vector which forms the sphere to search for the next recursive method call. The method
     * keeps track of the sphere count and returns it as soon as the target atom is encountered.
     *
     ...
     * @param pathLength The current path length, incremented and passed in recursive calls. Call
     *     this method with "zero".
     ...
     */
    public static int breadthFirstTargetSearch(
     ...
     *
     * <p>In this method, d is the topological distance (ie edge count).
     *
     * @param atomContainer The molecule to consider
     ...
     *
     * <p>This method uses the Djikstra algorithm to find all the atoms in the shortest path between
     * the two specified atoms. The start and end atoms are also included in the path returned
     *
     * @param atomContainer The molecule to search in
     * @param start The starting atom
     * @param end The ending atom
     * @return A <code>List</code> containing the atoms in the shortest path between <code>start
     *     </code> and <code>end</code> inclusive
     * @see ShortestPaths
     * @see ShortestPaths#atomsTo(IAtom)
     * @see AllPairsShortestPaths
     * @deprecated This implementation recalculates all shortest paths from the start atom for each
     *     method call and does not indicate if there are equally short paths from the start to the
     *     end. Replaced by {@link ShortestPaths#atomsTo(IAtom)}
     */
    @Deprecated
    public static List<IAtom> getShortestPath(
            IAtomContainer atomContainer, IAtom start, IAtom end) {
        int natom = atomContainer.getAtomCount();
        ...
        while (true) {
            ...
            List<IAtom> connected =
                    atomContainer.getConnectedAtomsList(atomContainer.getAtom(index));
     ...
     *
     * <p>If the two atoms are the same an empty list is returned. Note that this problem is NP-hard
     * and so can take a long time for large graphs.
     *
     * @param atomContainer The molecule to consider
     ...
     */
    public static List<List<IAtom>> getAllPaths(
            IAtomContainer atomContainer, IAtom start, IAtom end) {
    ...
    private static void findPathBetween(
     ...
     *
     * <p>This method returns a set of paths. Each path is a <code>List</code> of atoms that make up
     * the path (ie they are sequentially connected).
     *
     * @param atomContainer The molecule to consider
     * @param start The starting atom
     ...
     */
    public static List<List<IAtom>> getPathsOfLength(
            IAtomContainer atomContainer, IAtom start, int length) {
     ...
     * @return A <code>List</code> containing the paths found
     */
    public static List<List<IAtom>> getPathsOfLengthUpto(
            IAtomContainer atomContainer, IAtom start, int length) {
        List<IAtom> curPath = new ArrayList<IAtom>();
        List<List<IAtom>> paths = new ArrayList<List<IAtom>>();
        List<List<IAtom>> allpaths = new ArrayList<List<IAtom>>();
    ...
    /**
     * Get all the paths starting from an atom of length 0 up to the specified length. If the number
     * of paths exceeds the the set {@code limit} then an exception is thrown. This method returns a
     * set of paths. Each path is a <code>List</code> of atoms that make up the path (ie they are
     * sequentially connected).
     *
     * @param atomContainer The molecule to consider
     * @param start The starting atom
     * @param length The maximum length of paths to look for
     * @param limit Limit the number of paths - thrown an exception if exceeded
     * @return A <code>List</code> containing the paths found
     * @throws CDKException throw if the number of paths generated was larger than the limit.
     */
    public static List<List<IAtom>> getLimitedPathsOfLengthUpto(
            IAtomContainer atomContainer, IAtom start, int length, int limit) throws CDKException {
        List<IAtom> curPath = new ArrayList<IAtom>();
        List<List<IAtom>> paths = new ArrayList<List<IAtom>>();
        List<List<IAtom>> allpaths = new ArrayList<List<IAtom>>();
        curPath.add(start);
