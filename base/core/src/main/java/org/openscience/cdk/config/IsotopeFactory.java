...
/**
 * Used to store and return data of a particular isotope. The classes {@link Isotopes} extends this
 * class and is to be used to get isotope information.
 *
 * @cdk.module core
 * @cdk.githash
 * @author steinbeck
...
public abstract class IsotopeFactory {
    ...
    public int getSize() {
        int count = 0;
        for (List<IIsotope> isotope : isotopes) if (isotope != null) count += isotope.size();
        return count;
    }
    ...
    public IIsotope[] getIsotopes(int elem) {
        if (isotopes[elem] == null) return EMPTY_ISOTOPE_ARRAY;
    ...
    /**
     * Returns the most abundant (major) isotope with a given atomic number. Note that some high
     * mass elements do not have a major isotopes (0% abundance) and this method will return null
     * for those.
     *
    ...
    public IIsotope getMajorIsotope(int elem) {
        ...
        if (isotopes != null) {
            for (IIsotope isotope : isotopes) {
                if (isotope.getNaturalAbundance() <= 0) continue;
                if (major == null || isotope.getNaturalAbundance() > major.getNaturalAbundance()) {
                    major = isotope;
                }
            }
            if (major != null) this.majorIsotope[elem] = major;
            else {
    ...
    /**
     * Get the mass of the most abundant (major) isotope, if there is no major isotopes 2*elem is
     * returned.
     *
    ...
    public double getMajorIsotopeMass(int elem) {
        if (this.majorIsotope[elem] != null) return this.majorIsotope[elem].getExactMass();
    ...
    /**
     * Get the exact mass of a specified isotope for an atom. If there is no isotope with that mass
     * number then it is returned.
     *
    ...
    public double getExactMass(Integer atomicNumber, Integer massNumber) {
        if (atomicNumber == null || massNumber == null) return 0;
        for (IIsotope isotope : this.isotopes[atomicNumber]) {
            if (isotope.getMassNumber().equals(massNumber)) return isotope.getExactMass();
        }
        return massNumber;
    }
    ...
    /**
     * Configures an atom. Finds the correct element type by looking at the atoms element symbol. If
     * the element symbol is not recognised, it will throw an {@link IllegalArgumentException}.
     *
    ...
    public IAtom configure(IAtom atom) {
        IIsotope isotope;
        if (atom.getMassNumber() == null) return atom;
        else isotope = getIsotope(atom.getSymbol(), atom.getMassNumber());
        if (isotope == null)
            throw new IllegalArgumentException(
                    "Cannot configure an unrecognized element/mass: "
                            + atom.getMassNumber()
                            + " "
                            + atom);
    ...
    /**
     * Configures atoms in an AtomContainer to carry all the correct data according to their element
     * type.
     *
    ...
    /**
     * Gets the natural mass of this element, defined as average of masses of isotopes, weighted by
     * abundance.
     *
