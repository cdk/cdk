...
public class BondManipulator {
    ...
    /**
     * Returns true if the first bond has a lower bond order than the second bond. It returns false
     * if the bond order is equal, and if the order of the first bond is larger than that of the
     * second. Also returns false if either bond order is unset.
     *
     ...
     * @see #isHigherOrder(org.openscience.cdk.interfaces.IBond.Order,
     *     org.openscience.cdk.interfaces.IBond.Order)
     */
    public static boolean isLowerOrder(IBond.Order first, IBond.Order second) {
        if (first == null
                || second == null
                || first == IBond.Order.UNSET
                || second == IBond.Order.UNSET) return false;
        return first.compareTo(second) < 0;
    }

    /**
     * Returns true if the first bond has a higher bond order than the second bond. It returns false
     * if the bond order is equal, and if the order of the first bond is lower than that of the
     * second. Also returns false if either bond order is unset.
     *
     * @param first The first bond order object
     * @param second The second bond order object
     * @return true if the first bond order is higher than the second one, false otherwise
     * @see #isLowerOrder(org.openscience.cdk.interfaces.IBond.Order,
     *     org.openscience.cdk.interfaces.IBond.Order)
     */
    ...
    /**
     * Returns the IBond.Order one higher. Does not increase the bond order beyond the QUADRUPLE
     * bond order.
     *
    ...
    /**
     * Returns the IBond.Order one lower. Does not decrease the bond order lower the QUADRUPLE bond
     * order.
     *
     * @param oldOrder the old order
    ...
    /**
     * Convenience method to convert a double into an IBond.Order. Returns NULL if the bond order is
     * not 1.0, 2.0, 3.0 and 4.0.
     *
     ...
     *
     * <p>Single, double, triple and quadruple bonds are converted to 1.0, 2.0, 3.0 and 4.0
     * respectively.
     *
    ...
    public static IBond.Order getMaximumBondOrder(IBond.Order firstOrder, IBond.Order secondOrder) {
        ...
        if (isHigherOrder(firstOrder, secondOrder)) return firstOrder;
     ...
     *
     * <p>This sums the bond orders of all the bonds.
     *
