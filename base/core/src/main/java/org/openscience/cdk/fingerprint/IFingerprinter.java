...
public interface IFingerprinter {
    ...
    /**
     * Generate a fingerprint type version description in chemfp's FPS format. We report the library
     * version rather than an individual version per fingerprint, although this is awkward as many
     * fingerprint's don't/won't change between releases and we can not keep compatibility we
     * guarantee we document how the fingerprint was encoded. <br>
     * Examples:
     *
     * <pre>
    ...
    /**
     * Generate a binary fingerprint as a bit. This method will usually delegate to {@link
     * #getBitFingerprint(IAtomContainer)} and invoke {@link IBitFingerprint#asBitSet()}, it is
     ...
     * @throws CDKException may be thrown if there is an error during aromaticity detection or (for
     *     key based fingerprints) if there is a SMARTS parsing error
     ...
     * @throws CDKException if there is an error during aromaticity detection or (for key based
     *     fingerprints) if there is a SMARTS parsing error.
    ...
    /**
     * Returns the raw representation of the fingerprint for the given IAtomContainer. The raw
     * representation contains counts as well as the key strings.
     *
