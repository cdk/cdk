 ...
 * Classes using these constants should <b>not</b> implement this interface, but use it like:
 *
 * <pre>
...
public class CDKConstants {
    ...
    /**
     * A geometry of neighboring atoms when an s orbital is hybridized with three p orbitals with
     * one d orbital.
     */
    ...
    /**
     * Phosphorus NMR shift constant for use as a key in the IChemObject.physicalProperties
     * hashtable.
     *
     * @see org.openscience.cdk.ChemObject
     */
    ...
    /**
     * Deuterium NMR shift constant for use as a key in the IChemObject.physicalProperties
     * hashtable.
     *
     * @see org.openscience.cdk.ChemObject
     */
     ...
     * @see org.openscience.cdk.ChemObject
     */
    public static final String NMRSHIFT_SULFUR = "sulfur nmr shift";

    /**
     * NMR signal multiplicity constant for use as a key in the IChemObject.physicalProperties
     * hashtable.
     *
     * @see org.openscience.cdk.ChemObject
     */
    public static final String NMRSIGNAL_MULTIPLICITY = "nmr signal multiplicity";

    /**
     * NMR signal intensity constant for use as a key in the IChemObject.physicalProperties
     * hashtable.
     *
     * @see org.openscience.cdk.ChemObject
     */
    public static final String NMRSIGNAL_INTENSITY = "nmr signal intensity";

    /** NMR spectrum type constant for use as key for an arbitrary 1-dimensional NMR experiment. */
    public static final String NMRSPECTYPE_1D = "1D NMR spectrum";

    /** NMR spectrum type constant for use as key for a 1-dimensional DEPT90 NMR experiment. */
    public static final String NMRSPECTYPE_1D_DEPT90 = "1D NMR spectrum: DEPT90";

    /** NMR spectrum type constant for use as key for a 1-dimensional DEPT135 NMR experiment. */
    public static final String NMRSPECTYPE_1D_DEPT135 = "1D NMR spectrum: DEPT135";

    /** NMR spectrum type constant for use as key for a 2-dimensional HSQC NMR experiment. */
    public static final String NMRSPECTYPE_2D_HSQC = "2D NMR spectrum: HSQC";

    /** NMR spectrum type constant for use as key for a 2-dimensional H,H-COSY NMR experiment. */
    public static final String NMRSPECTYPE_2D_HHCOSY = "2D NMR spectrum: HHCOSY";

    /** NMR spectrum type constant for use as key for a 2-dimensional INADEQUATE NMR experiment. */
    public static final String NMRSPECTYPE_2D_INADEQUATE = "2D NMR spectrum: INADEQUATE";

    /** NMR spectrum type constant for use as key for a 2-dimensional HMBC NMR experiment. */
    public static final String NMRSPECTYPE_2D_HMBC = "2D NMR spectrum: HMBC";
    ...
    public static final int ISPLACED = 0x0001; // 1 << 0
    /** Flag that is set when the chemobject is part of a ring. */
    public static final int ISINRING = 0x0002; // 1 << 1
    /** Flag that is set when the chemobject is part of a ring. */
    public static final int ISNOTINRING = 0x0004; // 1 << 2
    /** Flag that is set if a chemobject is part of an aliphatic chain. */
    public static final int ISALIPHATIC = 0x0008; // 1 << 3 etc.
    /** Flag is set if chemobject has been visited. */
    public static final int VISITED = 0x0010; // Use in tree searches // 1 << 4
    /** Flag is set if chemobject is part of an aromatic system. */
    public static final int ISAROMATIC = 0x0020; // 1 << 5
    /** Flag is set if chemobject is part of a conjugated system. */
    public static final int ISCONJUGATED = 0x0040; // 1 << 6
    /**
     * Flag is set if a chemobject is mapped to another chemobject. It is used for example in
     * subgraph isomorphism search.
     */
    public static final int MAPPED = 0x0080; // 1 << 7

    /** Sets to true if the atom is an hydrogen bond donor. */
    public static final int IS_HYDROGENBOND_DONOR = 0x0100; // 1 << 8
    /** Sets to true if the atom is an hydrogen bond acceptor. */
    public static final int IS_HYDROGENBOND_ACCEPTOR = 0x0200; // 1 << 9
     ...
     *
     * <ul>
     *   <li>{@link org.openscience.cdk.interfaces.IAtomContainer} it means that one or several of
     *       the bonds have this flag raised (which may indicate aromaticity).
     *   <li>{@link org.openscience.cdk.interfaces.IBond} it means that it's unclear whether the
     *       bond is a single or double bond.
     *   <li>{@link org.openscience.cdk.interfaces.IAtom} it is a way for the Smiles parser to
     *       indicate that this atom was written with a lower case letter, e.g. 'c' rather than 'C'
     * </ul>
     */
    public static final int SINGLE_OR_DOUBLE = 1 << 12;
    /**
     * Maximum flags array index. Please update this if the value exceeds 16 - the flags are
     * currently stored as a single short value (16-bit) in the ChemObject implementations (default,
     * silent and query).
     */
    public static final int MAX_FLAG_INDEX = 13;

    // array of flags is initialised using the static constructor
    public static final int[] FLAG_MASKS = new int[MAX_FLAG_INDEX + 1];

    static {
        for (int i = 0; i < FLAG_MASKS.length; i++) FLAG_MASKS[i] = 1 << i;
    }
    ...
    /**
     * The essential rings computed for this molecule. The concept of Essential Rings is defined in
     * SSSRFinder
     */
    public static final String ESSENTIAL_RINGS = "cdk:EssentialRings";

    /**
     * The relevant rings computed for this molecule. The concept of relevant Rings is defined in
     * SSSRFinder
     */
     ...
     * has an a reaction.
     *
     * <p>Used in. ReactionManipulator.toMolecule and ReactionManipulator.toReaction.
     */
    public static final String REACTION_ROLE = "cdk:ReactionRole";

    /**
     * Property used for reactions when converted to/from molecules. It defines fragment grouping,
     * for example when handling ionic components.
     *
     * <p>Used in. ReactionManipulator.toMolecule and ReactionManipulator.toReaction.
     */
    ...
    /**
     * This property will contain an ArrayList of Integers. Each element of the list indicates the
     * size of the ring the given atom belongs to (if it is a ring atom at all).
     */
     ...
     * This is a property used for calculating NMR chemical shifts by subtracting the value from the
     * isotropic shielding value of a standard (e.g. TMS).
     */
    public static final String ISOTROPIC_SHIELDING = "cdk:IsotropicShielding";

    /**
     * A property to indicate RestH being true or false. RestH is a term used in RGroup queries: "if
     * this property is applied ('on'), sites labelled with Rgroup rrr may only be substituted with
     * a member of the Rgroup or with H"
     */
    ...
    /**
     * Key to store/fetch CTab Sgroups from Molfiles. Important! - Use at your own risk, property is
     * transitive and may be removed in future with a more specific accessor.
     */
