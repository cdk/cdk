...
/**
 * Represents an octahedral configuration of an atom six neighbors. The configuration order is
 * defined as between 1 and 30 using the same permutation tables as SMILES (e.g. @OH1 .. @OH30).
 * This allows the 720 permutations of atom ordering to be described. <br>
 * Normalizing the representation (with {@link #normalize()} returns a configuration reordered such
 * that the configuration order is 1. For example
 *
 * <pre>C[Co@OH8](F)(Br)(Cl)(I)S</pre>
 *
 * is the same as
 *
 * <pre>C[Co@OH1](F)(Cl)(Br)(I)S</pre>
 *
 * . The normalised form is easy to work with as the first and last carriers form an axis, the
 * middle four equatorial carriers are arranged anti-clockwise looking from the first carrier.
 *
 * <pre>
 *      c
 *      | a
 ...
 * </pre>
 *
 * @see <a href="http://opensmiles.org/opensmiles.html#_octahedral_centers">Octahedral Centers,
 *     OpenSMILES</a>
    ...
    public Octahedral(IAtom focus, IAtom[] carriers, int order) {
        ...
        if (getConfigOrder() < 0 || getConfigOrder() > 30)
            throw new IllegalArgumentException(
                    "Invalid configuration order!" + "Should be in range 1-30");
    }
        ...
        if (cfg < 1 || cfg > 30)
            throw new IllegalArgumentException(
                    "Invalid config order: "
                            + cfg
                            + ", octahedral should be"
                            + "1 <= order <= 30!");
        IAtom[] carriers = invapply(getCarriers().toArray(new IAtom[6]), PERMUTATIONS[cfg - 1]);
