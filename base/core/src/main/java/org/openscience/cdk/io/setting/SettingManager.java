...
/**
 * Provides dynamic management of settings. This was created with the intention of managing {@link
 * IOSetting}'s for {@link org.openscience.cdk.io.IChemObjectIO} however it could be recycled for
 * other purposes where dynamic settings are required. Settings are stored in a {@link Map} using
 * the name of the setting as the key. The name is normalised (lowercase and whitespace removal) to
 * allow 'fuzzy' setting access. This means that character case differences do not affect the
...
public class SettingManager<T extends ISetting> {
    ...
    /**
     * Generate a simple key for the given name. This method normalises the name by converting to
     * lower case and replacing spaces with '.' (e.g. "Buffer Size" is converted to "buffer.size").
     *
    ...
    /**
     * Generate a simple key for the given setting. This method is a convenience method for {@link
     * #key(String)}
     *
    ...
    /**
     * Add a setting to the manager and return the instance to use. If a 'new' setting is added to
     * the manager which matches the name and class of an previously added 'original' setting, the
     * original setting will be returned. Otherwise the new setting is returned. This allows the add
     * to be used in assignments as follows:
     *
     * <pre>{@code
     ...
     * }</pre>
     *
     * If the names are not equal or the names are equal but the classes are not the new setting is
     * added and returned.
     *
    ...
    /**
     * Access the setting stored for given name. If not setting is found the provided name an {@link
     * InvalidParameterException} will be thrown. The method is generic to allow simplified access
     * to settings. This however means that if the incorrect type is provided a {@link
    ...
    /**
     * Convenience method that allows specification of return ISetting type so that you can nest the
     * call to access the setting value.
     *
     * <pre>{@code
     * SettingManager manager = ...;
    ...
    /**
     * Compatibility method generates an array of ISetting objects. This method wraps a call to
     * {@link Collection#toArray(Object[])}} and so is used the same way. Note: it is preferable to
     * use the collection's accessor {@link #getSettings()}
     *
     * <p>Usage:
     *
     * <pre>{@code
