...
/**
 * Describes square planar configuration. The configuration around a square planar is described by 3
 * possible values (1:U, 2:4, or 3:Z) based on the ordering of the planar carries around the focus:
 *
 * <pre>{@code
 * Configurations:
 *
 ...
 *
 * <pre>IStereoElement</pre>
 *
 * where the carriers have been reorder such that the configuration is in a <code>U</code> shape
 * (order=1).
 *
 * @see <a href="http://opensmiles.org/opensmiles.html#_square_planar_centers">Square Planar
 *     Centers, OpenSMILES</a>
...
public final class SquarePlanar extends AbstractStereo<IAtom, IAtom> {
    ...
    /**
     * Create a square-planar configuration around a provided focus atom. The carriers are flat in
     * the plane and their arrangement is either, U-shape, 4-shape, or Z-shape.
     *
    ...
    public SquarePlanar(IAtom focus, IAtom[] carriers, int order) {
        ...
        if (getConfigOrder() < 0 || getConfigOrder() > 3)
            throw new IllegalArgumentException(
                    "Invalid configuration order," + "should be between 1-3");
    }
    ...
    public SquarePlanar normalize() {
        ...
        IAtom[] carriers = invapply(getCarriers().toArray(new IAtom[4]), PERMUTATIONS[cfg - 1]);
        return new SquarePlanar(getFocus(), carriers, SPU);
    }
