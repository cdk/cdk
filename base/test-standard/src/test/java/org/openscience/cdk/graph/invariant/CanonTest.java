...
public class CanonTest {
    ...
    public void atomComparatorLabelling() throws Exception {
        ...
        for (int i = 0; i < labels.length; i++)
            m.getAtom(i).setProperty(CDKConstants.ATOM_ATOM_MAPPING, (int) labels[i]);
        ...
        for (int i = 0; i < labels.length; i++)
            m.getAtom(i).setProperty(CDKConstants.ATOM_ATOM_MAPPING, (int) labels2[i]);
        assertThat(smigen(m), is("[CH:5]1=[CH:3][CH:2]=[CH:4][CH:6]=[C:7]1[OH:1]"));
    }

    /**
     * Ensure we consider the previous rank when we shatter ranks. This molecule has a
     * carbons/sulphurs which experience the same environment. We must consider that they are
     * different (due to their initial label) but not their environment.
     *
    ...
    public void canonicallyLabelCaffeineWithExplicitHydrogenRepresentation() throws Exception {
        IAtomContainer m =
                smi("[H]C1=NC2=C(N1C([H])([H])[H])C(=O)N(C(=O)N2C([H])([H])[H])C([H])([H])[H]");
        long[] labels = Canon.label(m, GraphUtil.toAdjList(m));
        org.hamcrest.MatcherAssert.assertThat(
                labels,
                is(
                        new long[] {
                            1, 14, 13, 16, 18, 19, 22, 2, 3, 4, 15, 11, 20, 17, 12, 21, 24, 8, 9,
                            10, 23, 5, 6, 7
                        }));
    }

    static final SmilesParser sp = new SmilesParser(SilentChemObjectBuilder.getInstance());
    static final SmilesGenerator sg =
            new SmilesGenerator(SmiFlavor.Isomeric | SmiFlavor.AtomAtomMap);
