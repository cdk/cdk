...
public class InvariantRankerTest {
    ...
    public void mergeSort() {
        ...
        while (values.size() < n) values.add(rnd.nextLong());
        ...
        for (int i = 1; i < n; i++) assertTrue(prev[vs[i]] > prev[vs[i - 1]]);
    }
    ...
    public void mergeSort_range() {
        ...
        // check they are sorted
        for (int i = 11; i < (n - 20); i++) assertTrue(prev[vs[i]] > prev[vs[i - 1]]);

        // other values weren't touched
        for (int i = 0; i < 10; i++) assertThat(vs[i], is(i));
        for (int i = n - 10; i < n; i++) assertThat(vs[i], is(i));
    }
