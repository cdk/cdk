...
/**
 * An <code>AtomTypeMapper</code> allows the mapping of atom types between atom type schemes. For
 * example, it allows to convert atom types from the CDK scheme to the Sybyl scheme; using this
 * approach it is possible to use the CDK atom type perception algorithm and write the resulting
 * atom types using the Sybyl atom type scheme.
 *
 * @cdk.module atomtype
 * @cdk.githash
 */
public class AtomTypeMapper {
    ...
    private AtomTypeMapper(String mappingFile) {
        ...
        OWLAtomTypeMappingReader reader =
                new OWLAtomTypeMappingReader(new InputStreamReader(stream));
    ...
    /**
     * Instantiates an atom type to atom type mapping, based on the given mapping file. For example,
     * the mapping file <code>org.openscience.cdk.config.data.cdk-sybyl-mappings.owl</code> which
     * defines how CDK atom types are mapped to Sybyl atom types.
     *
     ...
     *
     * @param mappingFile Name of the {@link InputStream} defining the atom type to atom type
     *     mappings.
    ...
    /**
     * Returns the name of this mapping. In case of file inputs, it returns the filename, but when
     * the input was an {@link InputStream} then the name is less well defined.
     *
