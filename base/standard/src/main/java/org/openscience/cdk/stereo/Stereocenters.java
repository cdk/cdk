...
/**
 * Find atoms which can support stereo chemistry based on the connectivity. Stereocenters are
 * classified as <i>True</i> when they have constitutionally different ligands and <i>Para</i>
 * ("resemble") stereo centers with constitutionally identical ligands. Some examples of
 * para-centers are listed below. Non and potential stereogenic atoms are also indicated. The method
 * partially implements the rules described by {@cdk.cite Razinger93}. Para centers are identified
 * in isolated rings (more common) but are not currently found in fused systems (e.g. decalin),
 * spiro linked 'assemblages' or acyclic interdependent centers.
 *
 * <p><b>Accepted Stereo Atoms</b>
 *
 * <p>This atoms accepted as being potentially stereogenic are those defined in the InChI Technical
 * Manual {@cdk.cite InChITechManual}. These are:
 *
 * <p><b>Tetrahedral Stereochemistry:</b>
 *
 * <ul>
 *   <li>Carbon - 4 valent, 4 sigma bonds
 *   <li>Silicon - 4 valent, 4 sigma bonds
 *   <li>Germanium - 4 valent, 4 sigma bonds
 *   <li>Tin - 4 valent, 4 sigma bonds
 *   <li>Nitrogen cation - 4 valent,4 sigma bonds
 *   <li>Phosphorus cation - 4 valent, 4 sigma bonds
 *   <li>Arsenic cation - 4 valent, 4 sigma bonds
 *   <li>Boron anion - 4 valent, 4 sigma bonds
 *   <li>Nitrogen - 5 valent, 3 sigma and 1 pi bond
 *   <li>Phosphorus - 5 valent, 3 sigma and 1 pi bond
 *   <li>Sulphur - 4 valent, 2 sigma and 1 pi bond
 *   <li>Sulphur - 6 valent, 2 sigma and 2 pi bonds
 *   <li>Sulphur Cation - 3 valent, 3 sigma bonds
 *   <li>Sulphur cation - 5 valent, 3 sigma and 1 pi bond
 *   <li>Selenium - 4 valent, 2 sigma and 1 pi bond
 *   <li>Selenium - 6 valent, 2 sigma and 2 pi bonds
 *   <li>Selenium Cation - 3 valent, 3 sigma bonds
 *   <li>Selenium cation - 5 valent, 3 sigma and 1 pi bond
 *   <li>Nitrogen - 3 valent, 3 sigma bonds and in a 3 member ring
 * </ul>
 *
 * <i>N, P, As, S or Se are not stereogenic if they have a terminal H neighbor or if they have 2
 * neighbors of the same element (O, S, Se, Te, N) which have at least one hydrogen. Consider:
 * {@code P(O)(=O)(OC)OCCC}. Phosphines and arsines are always stereogenic regardless of H
 * neighbors</i>
 *
 * <p><b>Double Bond Stereochemistry:</b> The following atoms can appear at either end of a double
 * bond.
 *
 * <ul>
 *   <li>Carbon - 4 valent, 2 sigma and 1 pi bond
 *   <li>Silicon - 4 valent, 2 sigma and 1 pi bond
 *   <li>Germanium - 4 valent, 2 sigma and 1 pi bond
 *   <li>Nitrogen - 3 valent, 1 sigma and 1 pi bond
 *   <li>Nitrogen cation - 4 valent, 2 sigma and 1 pi bond
 * </ul>
 *
 * <b>Examples of Para Stereocenters</b>
 *
 * <ul>
 *   <li>inositol - has 9 stereo isomers, {@code O[C@H]1[C@H](O)[C@@H](O)[C@H](O)[C@H](O)[C@@H]1O
 *       myo-inositol}
 *   <li>decalin - has 2 stereo isomers, {@code C1CC[C@H]2CCCC[C@H]2C1} (not currently identified)
 *   <li>spiro/double-bond linked ring - {@code
 *       InChI=1/C14H24/c1-11-3-7-13(8-4-11)14-9-5-12(2)6-10-14/h11-12H,3-10H2,1-2H3/b14-13-/t11-,12-}
 *       (not currently identified)
 *   <li>An example of a para-center not in a cycle {@code C[C@@H](O)[C@H](C)[C@H](C)O} (not
 *       currently identified)
 * </ul>
 *
 * It should be noted that para-centers may not actually have a configuration. A simple example of
 * this is seen that by changing the configuration of one center in {@code
 * C[C@@H](O)[C@H:1](C)[C@H](C)O} removes the central configuration as the ligands are now
 * equivalent {@code C[C@@H](O)[CH:1]](C)[C@@H](C)O}
 *
 * @author John May
 * @cdk.module standard
 * @cdk.githash
...
public final class Stereocenters {
    ...
    /**
     * Create a perception method for the provided container, graph representation and bond map.
     *
    ...
    /**
     * Obtain the type of stereo element support for atom at index {@code v}. Supported elements
     * types are:
     *
     * <ul>
     *   <li>{@link Type#Bicoordinate} - an central atom involved in a cumulated system (not yet
     *       supported)
     *   <li>{@link Type#Tricoordinate} - an atom at one end of a geometric (double-bond) stereo
     *       bond or cumulated system.
     *   <li>{@link Type#Tetracoordinate} - a tetrahedral atom (could also be square planar in
     *       future)
     *   <li>{@link Type#None} - the atom is not a (supported) stereo element type.
     * </ul>
     *
     ...
     *
     * <ul>
     *   <li>{@link Stereocenter#True} - the atom has constitutionally different neighbors
     *   <li>{@link Stereocenter#Para} - the atom resembles a stereo centre but has constitutionally
     *       equivalent neighbors (e.g. inositol, decalin). The stereocenter depends on the
     *       configuration of one or more stereocenters.
     *   <li>{@link Stereocenter#Potential} - the atom can supported stereo chemistry but has not be
     *       shown to be a true or para center.
     *   <li>{@link Stereocenter#Non} - the atom is not a stereocenter (e.g. methane).
     * </ul>
     *
     * @param v atom index.
    ...
    /**
     * Create {@link org.openscience.cdk.stereo.Stereocenters.StereoElement} instance for atoms
     * which support stereochemistry. Each new element is considered a potential stereo center - any
     * atoms which have not been assigned an element are non stereo centers.
     *
        ...
        for (int i = 0; i < g.length; i++) {
            ...
            for (int w : g[i]) {
                if (atomicNumber(container.getAtom(w)) == 1
                        && container.getAtom(w).getMassNumber() == null) h++;
    ...
    /**
     * Labels true stereocenters where all neighbors are constitutionally different. Potential
     * stereocenters where all constitutionally equivalent neighbors are terminal (consider
     * [C@H](C)(C)N) are also eliminated.
     *
    ...
    private void labelTrueCenters(final int[] symmetry) {
        ...
        for (int v = 0; v < g.length; v++) {
            if (stereocenters[v] == Stereocenter.Potential) {
                ...
                for (int w : ws) visited[symmetry[w]] = false;

                // neighbors are constitutionally different
                if (nUnique == ws.length) stereocenters[v] = Stereocenter.True;
    ...
    /**
     * Labels para stereocenter in isolated rings. Any elements which are now known to not be stereo
     * centers are also eliminated.
     *
    ...
    private void labelIsolatedPara(int[] symmetry) {
        ...
        for (int[] isolated : ringSearch.isolated()) {
            ...
            for (int v : isolated) {
                cyclic.set(v);
                if (stereocenters[v] == Stereocenter.Potential) potential.add(elements[v]);
    ...
    /**
     * Determine the type of stereo chemistry (if any) which could be supported by the atom at index
     * 'i'. The rules used to define the types of stereochemistry are encoded from the InChI
     * Technical Manual.
     *
     * @param i atom index
     * @param v valence
    ...
    private Type supportedType(int i, int v, int d, int h, int x) {
        ...
        if (checkSymmetry && h > 1) return Type.None;
        ...
        switch (atomicNumber(atom)) {
            ...
            case 6: // carbon
                ...
                return Type.None;
            case 7: // nitrogen
                if (x == 2 && v == 3 && d == 2 && q == 0) return Type.Tricoordinate;
                ...
                return x == 3
                                ...
                                && v == 3
                                && q == 0
                                && (isBridgeHead(i) || inThreeMemberRing(i))
                        ? Type.Tetracoordinate
                        : Type.None;
    ...
    /**
     * Verify that there are is not 2 terminal heavy atoms (of the same element) which have a
     * hydrogen count > 0. This follows the InChI specification that - An atom or positive ion N, P,
     * As, S, or Se is not treated as stereogenic if it has (a) A terminal H atom neighbor or (b) At
     * least two terminal neighbors, XHm and XHn, (n+m>0) connected by any kind of bond, where X is
     * O, S, Se, Te, or N. This avoids the issue that under Cahn-Ingold-Prelog (or canonicalisation)
     * the oxygens in 'P(=O)(O)(*)*' would not be found to be equivalent and a parity/winding would
     * be assigned.
     *
     * @param v the vertex (atom index) to check
     * @return the atom does not have > 2 terminal neighbors with a combined hydrogen count of > 0
     */
    private boolean verifyTerminalHCount(int v) {

        if (!checkSymmetry) return true;
        ...
        for (int i = 1; i < counts.length; i++) {
            ...
            for (int j = 0; j < counts[i]; j++) {
                ...
                if (degree == 1) {
                    ...
                    if (implHCount != null) terminalHCount += implHCount;
                    // O, S, Se, Te, or N with -1 charge is equiv to having a H
                    if (atom.getFormalCharge() == -1) terminalHCount++;
                }
            }
    ...
    /**
     * Index the atom by element to a number between 0-5. This allows us to quickly count up
     * neighbors we need to and the ignore those we don't (defaults to 0).
     *
    ...
    /**
     * Check if the {@code atom} at index {@code v} is a member of a small ring (n=3). This is the
     * only time a 3 valent nitrogen is allowed by InChI to be potentially stereogenic.
     *
     * @param v atom index
    ...
    private boolean inThreeMemberRing(int v) {
        ...
        for (int w : g[v]) adj.set(w);
        // is a neighbors neighbor adjacent?
        for (int w : g[v]) for (int u : g[w]) if (adj.get(u)) return true;
        return false;
    }
    ...
    private void visitPart(boolean[] visit, IAtom atom) {
        ...
        for (IBond bond : container.getConnectedBondsList(atom)) {
            ...
            if (!visit[container.indexOf(nbr)]) visitPart(visit, nbr);
        }
    }

    /**
     * Detects if a bond is a fused-ring bond (e.g. napthalene). A bond is a ring fusion if deleting
     * it creates a new component. Or to put it another way if using a flood-fill we can't visit
     * every atom without going through this bond, not we only check ring bonds.
     *
     * @param bond the bond
    ...
    private boolean isFusedBond(IBond bond) {
        ...
        if (getRingDegree(container.indexOf(beg)) < 3 && getRingDegree(container.indexOf(end)) < 3)
            return false;
        ...
        for (IBond nbond : container.getConnectedBondsList(beg)) {
            ...
            if (nbr.equals(end) || !ringSearch.cyclic(nbond)) continue;
            if (count == 0) {
                count++;
                visitPart(avisit, nbr);
            } else {
                if (!avisit[container.indexOf(nbr)]) return true;
            }
        }
        return false;
    }
    ...
    private int getRingDegree(int v) {
        int x = 0;
        for (int w : g[v]) if (ringSearch.cyclic(v, w)) x++;
        return x;
    }

    private boolean isBridgeHead(int v) {
        if (getRingDegree(v) < 3) return false;
        IAtom atom = container.getAtom(v);
        for (IBond bond : container.getConnectedBondsList(atom))
            if (isFusedBond(bond)) return false;
        return true;
    }

    /**
     * Safely obtain the atomic number of an atom. If the atom has undefined atomic number and is
     * not a pseudo-atom it is considered an error. Pseudo atoms with undefined atomic number
     * default to 0.
     *
     * @param a an atom
    ...
    /**
     * Safely obtain the formal charge on an atom. If the atom had undefined formal charge it is
     * considered as neutral (0).
     *
     * @param a an atom
    ...
    /**
     * Convert an array of long (64-bit) values to an array of (32-bit) integrals.
     *
    ...
    private static int[] toIntArray(long[] org) {
        ...
        for (int i = 0; i < cpy.length; i++) cpy[i] = (int) org[i];
        return cpy;
    }
    ...
    public enum Type {
        ...
        /**
         * A potentially stereogenic atom with 3 neighbors - one atom in a geometric centres or
         * cumulated system (allene, cumulene).
         */
        Tricoordinate,
    ...
    /**
     * Internal stereo element representation. We need to define the sides of a double bond
     * separately and want to avoid reflection (instanceof) by using a type parameter. We also store
     * the neighbors we need to check for equivalence directly.
     */
    private abstract static class StereoElement {
    ...
    /**
     * Represents one end of a double bond. The element only stores non-double bonded neighbors and
     * also indexes it's {@code other} end.
     */
    private static final class Tricoordinate extends StereoElement {
        ...
        /**
         * Create a tri-coordinate atom for one end of a double bond. Two elements need to be
         * created which reference each other.
         *
