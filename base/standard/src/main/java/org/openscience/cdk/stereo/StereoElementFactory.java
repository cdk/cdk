...
/**
 * Create stereo elements for a structure with 2D and 3D coordinates. The factory does not verify
 * whether atoms can or cannot support stereochemistry - for this functionality use {@link
 * Stereocenters}. The factory will not create stereo elements if there is missing information
 * (wedge/hatch bonds, undefined coordinates) or the layout indicates unspecified configuration.
 *
 * <p>Stereocenters specified with inverse down (hatch) bond style are created if the configuration
 * is unambiguous and the bond does not connect to another stereocenter.
 *
 * <blockquote>
 *
 * <pre>
...
public abstract class StereoElementFactory {
    ...
    private boolean visitSmallRing(int[] mark, int aidx, int prev, int depth, int max) {
        ...
        for (int nbr : graph[aidx]) {
            if (nbr != prev && visitSmallRing(mark, nbr, aidx, depth + 1, max)) return true;
        }
        mark[aidx] = 0;
        return false;
    }
    ...
    private static IAtom getShared(IBond a, IBond b) {
        if (b.contains(a.getBegin())) return a.getBegin();
        if (b.contains(a.getEnd())) return a.getEnd();
        return null;
    }
    ...
    private List<IBond> getCumulatedDbs(IBond endBond) {
        ...
        if (other == null) other = getOtherDb(endBond.getEnd(), endBond);
    ...
    /**
     * Creates all stereo elements found by {@link Stereocenters} using the or 2D/3D coordinates to
     * specify the configuration (clockwise/anticlockwise). Currently only {@link
     * ITetrahedralChirality} and {@link IDoubleBondStereochemistry} elements are created..
     *
    ...
    public List<IStereoElement> createAll() {
        ...
        FischerRecognition fischerRecon =
                new FischerRecognition(container, graph, bondMap, centers);
        CyclicCarbohydrateRecognition cycleRecon =
                new CyclicCarbohydrateRecognition(container, graph, bondMap, centers);
                    ...
                    for (int w : graph[v]) {
                        ...
                        if (centers.elementType(w) == Stereocenters.Type.Tricoordinate) {
                            List<IBond> dbs =
                                    getCumulatedDbs(
                                            container.getBond(
                                                    container.getAtom(w), container.getAtom(v)));
                            if (dbs == null) continue;
                            if (container.indexOf(dbs.get(0))
                                    > container.indexOf(dbs.get(dbs.size() - 1))) continue;
                            if ((dbs.size() & 0x1) == 0) {
                                IAtom focus =
                                        getShared(
                                                dbs.get(dbs.size() / 2),
                                ...
                                IStereoElement element =
                                        createExtendedTetrahedral(
                                                container.indexOf(focus), centers);
                                if (element != null) elements.add(element);
                            } else {
                                // extended cis-trans
                                IStereoElement element = createExtendedCisTrans(dbs, centers);
                                if (element != null) elements.add(element);
                            }
                            break;
                        }
    ...
    /**
     * Create a tetrahedral element for the atom at index {@code v}. If a tetrahedral element could
     * not be created then null is returned. An element can not be created if, one or more atoms was
     * missing coordinates, the atom has an unspecified (wavy) bond, the atom is no non-planar bonds
     * (i.e. up/down, wedge/hatch). The method does not check if tetrahedral chirality is supported
     * - for this functionality use {@link Stereocenters}.
     *
     * <blockquote>
     *
     * <pre>
    ...
    /**
     * Create a tetrahedral element for the atom. If a tetrahedral element could not be created then
     * null is returned. An element can not be created if, one or more atoms was missing
     * coordinates, the atom has an unspecified (wavy) bond, the atom is no non-planar bonds (i.e.
     * up/down, wedge/hatch). The method does not check if tetrahedral chirality is supported - for
    ...
    /**
     * Create a geometric element (double-bond stereochemistry) for the provided atom indices. If
     * the configuration could not be created a null element is returned. There is no configuration
     * is the coordinates do not indicate a configuration, there were undefined coordinates or an
     * unspecified bond label. The method does not check if double bond stereo is supported - for
     * this functionality use {@link Stereocenters}.
     *
    ...
    /**
     * Create a geometric element (double-bond stereochemistry) for the provided double bond. If the
     * configuration could not be created a null element is returned. There is no configuration is
     * the coordinates do not indicate a configuration, there were undefined coordinates or an
     * unspecified bond label. The method does not check if double bond stereo is supported - for
     * this functionality use {@link Stereocenters}.
     *
     * <blockquote>
    ...
    /**
     * Create an extended tetrahedral element for the atom at index {@code v}. If an extended
     * tetrahedral element could not be created then null is returned. An element can not be created
     * if, one or more atoms was missing coordinates, the atom has an unspecified (wavy) bond, the
     * atom is no non-planar bonds (i.e. up/down, wedge/hatch). The method does not check if
     * tetrahedral chirality is supported - for this functionality use {@link * Stereocenters}.
     *
     * <blockquote>
     *
     * <pre>
    ...
    /**
     * Create an extended cis/trans bond (cumulated) given one end (see diagram below). The stereo
     * element geometry will only be created if there is an odd number of cumulated double bonds.
     * The double bond list ('bonds') should be ordered consecutively from one end to the other.
     *
     * <pre>
     *  C               C
     *   \             /
     ...
     */
    abstract ExtendedCisTrans createExtendedCisTrans(List<IBond> bonds, Stereocenters centers);
    ...
    /**
     * Create a stereo element factory for creating stereo elements using 2D coordinates and
     * depiction labels (up/down, wedge/hatch).
     *
    ...
    /**
     * Create a stereo element factory for creating stereo elements using 3D coordinates and
     * depiction labels (up/down, wedge/hatch).
     *
     * @param container the structure to create the factory for
     * @return the factory instance
    ...
    static final class StereoElementFactory2D extends StereoElementFactory {
        ...
        /**
         * Threshold at which the determinant is considered too small (unspecified by coordinates).
         */
        ...
        IDoubleBondStereochemistry createGeometric(IBond bond, Stereocenters stereocenters) {
            return createGeometric(
                    container.indexOf(bond.getBegin()),
                    container.indexOf(bond.getEnd()),
                    stereocenters);
        }
        ...
        IStereoElement createAtropisomer(int u, int v, Stereocenters stereocenters) {
            ...
            if (hasUnspecifiedParity(end1) || hasUnspecifiedParity(end2)) return null;

            if (graph[u].length != 3 || graph[v].length != 3) return null;
            ...
            int sum1 =
                    graph[graph[u][0]].length
                            + graph[graph[u][1]].length
                            + graph[graph[u][2]].length;
            int sum2 =
                    graph[graph[v][0]].length
            ...
            for (int w : graph[u]) {
                ...
                for (int w2 : graph[w]) {
                    if (isHydrogen(container.getAtom(w2))) sum1--;
                    else if (elevation[n] == 0 && isWedged(bondMap.get(w, w2))) {
            ...
            for (int w : graph[v]) {
                ...
                for (int w2 : graph[w]) {
                    if (isHydrogen(container.getAtom(w2))) sum2--;
                    else if (elevation[n] == 0 && isWedged(bondMap.get(w, w2))) {
                        elevation[n] = elevationOf(container.getAtom(w), bondMap.get(w, w2));
                    }
                }
            ...
            tmp.setPoint2d(
                    new Point2d(
                            (end1.getPoint2d().x + end2.getPoint2d().x) / 2,
        ...
        IDoubleBondStereochemistry createGeometric(int u, int v, Stereocenters stereocenters) {
            ...
            if (isUnspecified(bondMap.get(u, us[0]))
                    ...
                    || isUnspecified(bondMap.get(v, vs[1]))) return null;
            ...
            return new DoubleBondStereochemistry(
                    bond, new IBond[] {bondMap.get(u, us[0]), bondMap.get(v, vs[0])}, conformation);
        }
        ...
        ExtendedTetrahedral createExtendedTetrahedral(int v, Stereocenters stereocenters) {
            ...
            if (stereocenters.isSymmetryChecked()
                    && (!stereocenters.isStereocenter(t0) || !stereocenters.isStereocenter(t1)))
                return null;
        ...
        ExtendedCisTrans createExtendedCisTrans(List<IBond> dbs, Stereocenters stereocenters) {
            ...
            IAtom endAtom =
                    dbs.get(dbs.size() - 1)
                            .getOther(getShared(dbs.get(dbs.size() - 1), dbs.get(dbs.size() - 2)));
            ...
            Vector2d begVec =
                    new Vector2d(
                            begNbr.getPoint2d().x - begAtom.getPoint2d().x,
                            begNbr.getPoint2d().y - begAtom.getPoint2d().y);
            Vector2d endVec =
                    new Vector2d(
                            endNbr.getPoint2d().x - endAtom.getPoint2d().x,
            ...
            if (dot < 0) config = IStereoElement.OPPOSITE;
            else config = IStereoElement.TOGETHER;
         ...
         *
         * @param atoms atoms around the double bonded atom, 0: substituent, 1: other substituent
         *     (or focus), 2: double bonded atom
        ...
        private Point2d toUnitVector(Point2d from, Point2d to) {
            if (from.equals(to)) return new Point2d(0, 0);
        ...
        /**
         * Compute the signed volume of the tetrahedron from the planar points and elevations.
         *
        ...
        /**
         * Utility find the specified value, {@code v}, in the array of values, {@code vs} and moves
         * it to the back.
         *
        ...
        /**
         * Obtain the elevation of an atom connected to the {@code focus} by the specified {@code
         * bond}.
         *
         * @param focus a focus of stereochemistry
         * @param bond a bond connecting the focus to a substituent
         * @return the elevation of the connected atom, +1 above, -1 below, 0 planar
         */
    ...
    private static final class StereoElementFactory3D extends StereoElementFactory {
        ...
        ITetrahedralChirality createTetrahedral(int v, Stereocenters stereocenters) {
            ...
            for (int w : graph[v]) neighbors[n++] = container.getAtom(w);
        ...
        IDoubleBondStereochemistry createGeometric(int u, int v, Stereocenters stereocenters) {
            ...
            return new DoubleBondStereochemistry(
                    bond,
                    new IBond[] {
                        bondMap.get(u, x), bondMap.get(v, w),
                    },
                    conformation);
        }
        ...
        ExtendedTetrahedral createExtendedTetrahedral(int v, Stereocenters stereocenters) {
            ...
            if (container.getConnectedBondsCount(focus) != 2) return null;
            ...
            if (!isColinear(focus, terminals)) return null;
        ...
        ExtendedCisTrans createExtendedCisTrans(List<IBond> dbs, Stereocenters centers) {
            ...
            Vector3d begVec =
                    new Vector3d(
                            begNbr.getPoint3d().x - begAtom.getPoint3d().x,
                            begNbr.getPoint3d().y - begAtom.getPoint3d().y,
                            begNbr.getPoint3d().z - begAtom.getPoint3d().z);
            Vector3d endVec =
                    new Vector3d(
                            endNbr.getPoint3d().x - endAtom.getPoint3d().x,
        ...
        private boolean isColinear(IAtom focus, IAtom[] terminals) {
            Vector3d vec0 =
                    new Vector3d(
                            terminals[0].getPoint3d().x - focus.getPoint3d().x,
                            terminals[0].getPoint3d().y - focus.getPoint3d().y,
                            terminals[0].getPoint3d().z - focus.getPoint3d().z);
            Vector3d vec1 =
                    new Vector3d(
                            terminals[1].getPoint3d().x - focus.getPoint3d().x,
        ...
        /**
         * Parity computation for one side of a double bond in a geometric center. The method needs
         * the 3D coordinates of the double bond atoms (first 2 arguments) and the coordinates of
         * two substituents (one at each end).
         *
