...
/**
 * A utility for ranking indices by invariants. The ranking is built around a merge/insertion sort
 * with the primary interaction through {@link #rank}.
 *
 * @author John May
 * @cdk.module standard
...
final class InvariantRanker {
    ...
    /**
     * Length at which the sub-array should be sorted using insertion sort. As insertion sort is
     * adaptive and in-place it's advantageous to use a high threshold for this use-case. Once we do
     * the first sort, the invariants will always be 'almost' sorted which is the best case for the
     * insertion sort.
     */
    ...
    /**
     * Given an array of equivalent indices (currEq) and their values (curr) assign a rank to the
     * values. The values are sorted using 'prev' and 'curr' invariants and once ranked the new
     * ranks placed in 'prev'. The values which are still equivalent are placed in 'nextEq' and
     * terminated by a '-1'.
     *
     ...
     * @param prev the prev invariants (initially = curr) used to sort and then store ranks (set by
     *     this method)
    ...
    int rank(int[] currEq, int[] nextEq, int n, long[] curr, long[] prev) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (prev[v] != tmp[u]) labelTick = (int) prev[v];
            else labelTick++;
    ...
    /**
     * Sort the values (using merge sort) in {@code vs} from {@code lo} (until {@code len}) by the
     * {@code prev[]} and then {@code curr[]} invariants to determine rank. The values in {@code vs}
     * are indices into the invariant arrays.
     *
    ...
    /**
     * Merge the values which are sorted between {@code lo} - {@code split} and {@code split} -
     * {@code len}.
     *
     * @param vs vertices
    ...
    private void merge(int[] vs, int lo, int split, int len, long[] curr, long[] prev) {
        ...
        for (int k = lo, end = lo + len; k < end; k++) {
            if (i == iMax) vs[k] = aux[j++];
            else if (j == jMax) vs[k] = aux[i++];
            else if (less(aux[i], aux[j], curr, prev)) vs[k] = aux[i++];
            else vs[k] = aux[j++];
        }
    }

    /**
     * Sort the values (using insertion sort) in {@code vs} from {@code lo} (until {@code len}) by
     * the {@code prev[]} and then {@code curr[]} invariants to determine rank. The values in {@code
     * vs} are indices into the invariant arrays.
     *
     * @param vs values (indices)
     * @param lo the first value to start sorting from
     * @param len the len of values to consider
    ...
    static void insertionSortBy(int[] vs, int lo, int len, long[] curr, long[] prev) {
        for (int j = lo + 1, hi = lo + len; j < hi; j++) {
            int v = vs[j];
            int i = j - 1;
            while ((i >= lo) && less(v, vs[i], curr, prev)) vs[i + 1] = vs[i--];
            vs[i + 1] = v;
        }
    }

    /**
     * Using the {@code prev} and {@code curr} invariants is value in index i less than j. Value i
     * is less than j if it was previously less than j (prev[]) or it was equal and it is now
     * (curr[]) less than j.
     *
     * @param i an index
     * @param j an index
