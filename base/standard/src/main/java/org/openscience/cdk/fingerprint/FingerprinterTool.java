...
public class FingerprinterTool {
    ...
    /**
     * Checks whether all the positive bits in BitSet bs2 occur in BitSet bs1. If so, the molecular
     * structure from which bs2 was generated is a possible substructure of bs1.
     *
     * <p>Example:
     *
     * <pre>
    ...
    /**
     * This lists all bits set in bs2 and not in bs2 (other way round not considered) in a list and
     * to logger. See. {@link #differences(java.util.BitSet, java.util.BitSet)} for a method to list
     * all differences, including those missing present in bs2 but not bs1.
     *
     ...
     * #listDifferences(java.util.BitSet, java.util.BitSet)} which only list those which are set in
     * <i>s</i> but not in <i>t</i>.
     *
    ...
    /**
     * Convert a mapping of features and their counts to a 1024-bit binary fingerprint. A single bit
     * is set for each pattern.
     *
    ...
    /**
     * Convert a mapping of features and their counts to a binary fingerprint. A single bit is set
     * for each pattern.
     *
     * @param features features to include
    ...
    /**
     * Convert a mapping of features and their counts to a binary fingerprint. Each feature can set
     * 1-n hashes, the amount is modified by the {@code bits} operand.
     *
     * @param features features to include
     * @param len fingerprint length
     ...
     * @return the continuous fingerprint
     */
    public static IBitFingerprint makeBitFingerprint(
            final Map<String, Integer> features, int len, int bits) {
