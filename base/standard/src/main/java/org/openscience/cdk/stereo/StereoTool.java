 ...
 *
 * <p>Some of these methods were adapted from Jmol's smiles search package.
 *
 * @author maclean
 * @cdk.module standard
 * @cdk.githash
...
public class StereoTool {
    ...
    /**
     * Currently unused, but intended for the StereoTool to indicate what it 'means' by an
     * assignment of some atoms to a class.
     */
    ...
    /**
     * The handedness of a tetrahedron, in terms of the point-plane distance of three of the
     * corners, compared to the fourth.
     *
     * <p>PLUS indices a positive point-plane distance, MINUS is a negative point-plane distance.
     */
    ...
    private static boolean isSquarePlanar(
            Point3d pointA, Point3d pointB, Point3d pointC, Point3d pointD, Vector3d normal) {
    ...
    /**
     * Given four atoms (assumed to be in the same plane), returns the arrangement of those atoms in
     * that plane.
     *
     * <p>The 'shapes' returned represent arrangements that look a little like the characters 'U',
     * '4', and 'Z'.
     *
     * @param atomA an atom in the plane
     * @param atomB an atom in the plane
    ...
    /**
     * Check that all the points in the list are coplanar (in the same plane) as the plane defined
     * by the planeNormal and the pointInPlane.
     *
     ...
     */
    public static boolean isOctahedral(
            IAtom atomA,
            IAtom atomB,
            IAtom atomC,
            IAtom atomD,
     ...
     */
    public static boolean isTrigonalBipyramidal(
            IAtom atomA, IAtom atomB, IAtom atomC, IAtom atomD, IAtom atomE, IAtom atomF) {
        Point3d pointA = atomA.getPoint3d();
        Point3d pointB = atomB.getPoint3d();
        Point3d pointC = atomC.getPoint3d();
        Point3d pointD = atomD.getPoint3d();
    ...
    /**
     * Take four atoms, and return Stereo.CLOCKWISE or Stereo.ANTI_CLOCKWISE. The first atom is the
     * one pointing towards the observer.
     *
    ...
    /**
     * Gets the tetrahedral handedness of four atoms - three of which form the 'base' of the
     * tetrahedron, and the other the apex. Note that it assumes a right-handed coordinate system,
     * and that the points {A,B,C} are in a counter-clockwise order in the plane they share.
     *
    ...
    /**
     * Checks the three supplied points to see if they fall on the same line. It does this by
     * finding the normal to an arbitrary pair of lines between the points (in fact, A-B and A-C)
     * and checking that its length is 0.
     *
     * @param ptA
     * @param ptB
     * @param ptC
    ...
    /**
     * Given a normalized normal for a plane, any point in that plane, and a point, will return the
     * distance between the plane and that point.
     *
    ...
    /**
     * Given three points (A, B, C), makes the vectors A-B and A-C, and makes the cross product of
     * these two vectors; this has the effect of making a third vector at right angles to AB and AC.
     *
     * <p>NOTE : the returned normal is normalized; that is, it has been divided by its length.
     *
    ...
    private static void getRawNormal(
            Point3d ptA, Point3d ptB, Point3d ptC, Vector3d normal, Vector3d vcAB, Vector3d vcAC) {
        // make A->B and A->C
        vcAB.sub(ptB, ptA);
        vcAC.sub(ptC, ptA);
