...
/**
 * Electron donation model closely mirroring the Daylight model for use in generating SMILES. The
 * model was interpreted from various resources and as such may not match exactly. If you find an
 * inconsistency please add a request for enhancement to the patch tracker. One known limitation is
 ...
 * </ul>
 *
 * The aromaticity model in SMILES was designed to simplify canonicalisation and express symmetry in
 * a molecule. The contributed electrons can be summarised as follows (refer to code for exact
 * specification):
 *
 * <ul>
...
final class DaylightModel extends ElectronDonation {
    ...
    int[] contribution(IAtomContainer container, RingSearch ringSearch) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // considered
            else if (!aromaticElement(element)
                    || !ringSearch.cyclic(i)
                    || degree[i] > 3
                    || nCyclicPiBonds[i] > 1) {
                electrons[i] = -1;
            ...
            // check if the bonded valence is okay (i.e. not a radical)
                ...
                if (element == WILDCARD) electrons[i] = 2;
                else if (valence(element, charge) - valence[i] >= 2) electrons[i] = 2;
                else electrons[i] = -1;
            } else {
                // cation with no double bonds - single exception?
                if (element == CARBON && charge > 0) electrons[i] = 0;
                else electrons[i] = -1;
            }
        }
    ...
    /**
     * Defines the number of electrons contributed when a pi bond is exocyclic (spouting). When an
     * atom is connected to an more electronegative atom then the electrons are 'pulled' from the
     * ring. The preset conditions are as follows:
     *
     * <ul>
     *   <li>A cyclic carbon with an exocyclic pi bond to anything but carbon contributes 0
     *       electrons. If the exocyclic atom is also a carbon then 1 electron is contributed.
     *   <li>A cyclic 4 valent nitrogen or phosphorus cation with an exocyclic pi bond will always
     *       contribute 1 electron. A 5 valent neutral nitrogen or phosphorus with an exocyclic bond
     *       to an oxygen contributes 1 electron.
     *   <li>A neutral sulphur connected to an oxygen contributes 2 electrons
     *   <li>If none of the previous conditions are met the atom is not considered as being able to
     *       participate in an aromatic system and -1 is returned.
     * </ul>
     *
     * @param element the element of the cyclic atom
     * @param otherElement the element of the exocyclic atom which is connected to the cyclic atom
     *     by a pi bond
     * @param charge the charge of the cyclic atom
     * @param nCyclic the number of cyclic pi bonds adjacent to cyclic atom
    ...
    /**
     * Is the element specified by the atomic number, allowed to be aromatic by the daylight
     * specification. Allowed elements are C, N, O, P, S, As, Se and *. This model allows all except
     ...
     * @return the valence
     * @throws UnsupportedOperationException encountered an element which the valence was not
     *     encoded for
     */
    ...
    /**
     * Lookup of the number of valence electrons for elements near those which this model considers
     * aromatic. As only the {@link #aromaticElement(int)} are checked we need only consider
     * elements within a charge range.
     *
     * @param element the atomic number of an element
     * @return the valence
     * @throws UnsupportedOperationException encountered an element which the valence was not
    ...
    private int valence(int element) {
        ...
        throw new UnsupportedOperationException(
                "Valence not yet handled for element with atomic number " + element);
    }

    /**
     * Get the atomic number as an non-null integer value. Although pseudo atoms are not considered
     * by this model the pseudo atoms are intercepted to have non-null atomic number (defaults to
     * 0).
     *
