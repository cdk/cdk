...
/**
 * Class that provides methods to give unique IDs to ChemObjects. Methods are implemented for Atom,
 * Bond, AtomContainer, AtomContainerSet and Reaction. It will only create missing IDs. If you want
 * to create new IDs for all ChemObjects, you need to delete them first.
 *
 * @cdk.module standard
 * @cdk.githash
...
public abstract class IDCreator {
    ...
    /**
     * Internal flag identifying the IDs generation policy. The old policy is to generate IDs so
     * that in a sequence of several molecules all the atoms and bonds will receive the unique IDs
     * even across molecules, i.e. in a set of 2 molecules the first atom of the first molecule will
     * be "a1" while the first atom of the second molecule will be "aX" where X equals to the number
     * of atoms in the first molecule plus 1. <br>
     * The new policy is to keep the singularity of IDs only within a single molecule, i.e. in a set
     * of two molecules first atoms of each will be "a1".
     */
    private static int policy = SET_UNIQUE_POLICY;

    /**
     * Alters the policy of ID generation. The IDCreator should in any case preserve the already
     * existing IDs therefore if one of objects already has an ID set, this ID will be skipped in
     * all the cases when attempting to generate a new ID value
     *
    ...
    /**
     * Labels the Atom's and Bond's in the AtomContainer using the a1, a2, b1, b2 scheme often used
     * in CML. Supports IAtomContainer, IAtomContainerSet, IChemFile, IChemModel, IChemSequence,
     * IReaction, IReactionSet, and derived interfaces.
     *
    ...
    /**
     * Reset the counters so that we keep generating simple IDs within single chem object or a set
     * of them
     */
    ...
    private static int setID(
            String prefix, int identifier, IChemObject object, List<String> tabuList) {
        identifier += 1;
    ...
    private static void createIDsForAtomContainer(IAtomContainer container, List<String> tabuList) {
        ...
        if (null == container.getID()) {
            ...
            atomContainerCount =
                    setID(ATOMCONTAINER_PREFIX, atomContainerCount, container, tabuList);
        }
    ...
    /**
     * Labels the Atom's and Bond's in each AtomContainer using the a1, a2, b1, b2 scheme often used
     * in CML. It will also set id's for all AtomContainers, naming them m1, m2, etc. It will not
     * the AtomContainerSet itself.
     */
    private static void createIDsForAtomContainerSet(
            IAtomContainerSet containerSet, List<String> tabuList) {
        ...
        if (null == containerSet.getID()) {
            atomContainerSetCount =
                    setID(ATOMCONTAINERSET_PREFIX, atomContainerSetCount, containerSet, tabuList);
        }
    ...
    /**
     * Labels the reactants and products in the Reaction m1, m2, etc, and the atoms accordingly,
     * when no ID is given.
     */
    ...
    private static void createIDsForReactionSet(IReactionSet reactionSet, List<String> tabuList) {
        ...
        for (Iterator<IReaction> reaction = reactionSet.reactions().iterator();
                reaction.hasNext(); ) {
