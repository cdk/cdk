...
/**
 * Recognize the configuration of tetrahedral stereocenters depicted as Fischer projection. Fischer
 * projection is a convenient means of depicting 3D geometry commonly used in depicting
 * carbohydrates.
 *
 * <p>Fischer projection depicts tetrahedral stereocenters as though they were coplanar with the
 * four substituents at cardinal directions (up,right,down, and left). The horizontal bonds (right
 * and left) are interpreted as pointing out of the plane towards the viewer; They are not depicted
 * with non-planar wedge bonds.
 *
 * <p>This class provides the recognition of Fischer projections. Each asymmetric carbon is checked
 * as to whether it's 2D depiction is coplanar with cardinal directions. All of these bonds must be
 * planar (i.e. not wedge or hatch) and sigma bonds. In a hydrogen suppressed representation, one of
 * the left or right bonds (to the implied hydrogen) may be omitted but can be correctly
 * interpreted.
 *
 * @author John May
 * @cdk.githash
 * @see <a href="http://en.wikipedia.org/wiki/Fischer_projection">Fischer projection (Wikipedia)</a>
 */
final class FischerRecognition {

    /**
     * The threshold at which to snap bonds to the cardinal direction. The threshold allows bonds
     * slightly of absolute directions to be interpreted. The tested vector is of unit length and so
    ...
    List<IStereoElement> recognise(Set<Projection> projections) {
        ...
        if (!projections.contains(Projection.Fischer)) return Collections.emptyList();

        // build atom index and only recognize 2D depictions
        Map<IAtom, Integer> atomToIndex = new HashMap<IAtom, Integer>();
        for (IAtom atom : container.atoms()) {
            if (atom.getPoint2d() == null) return Collections.emptyList();
        ...
        for (int v = 0; v < container.getAtomCount(); v++) {
            ...
            if (elem != Carbon) continue;
            if (ringSearch.cyclic(v)) continue;
            if (stereocenters.elementType(v) != Tetracoordinate) continue;
            if (!stereocenters.isStereocenter(v)) continue;
            ...
            if (!east.equals(focus) && !isTerminal(east, atomToIndex)) continue;
            if (!west.equals(focus) && !isTerminal(west, atomToIndex)) continue;
    ...
    /**
     * Create a new tetrahedral stereocenter of the given focus and neighboring bonds. This is an
     * internal method and is presumed the atom can support tetrahedral stereochemistry and it has
     * three or four explicit neighbors.
     *
     * <p>The stereo element is only created if the local arrangement looks like a Fischer
     * projection.
     *
    ...
    /**
     * Arrange the bonds adjacent to an atom (focus) in cardinal direction. The cardinal directions
     * are that of a compass. Bonds are checked as to whether they are horizontal or vertical within
     * a predefined threshold.
     *
     * @param focus an atom
     * @param bonds bonds adjacent to the atom
     * @return array of bonds organised (N,E,S,W), or null if a bond was found that exceeded the
     *     threshold
     */
    static IBond[] cardinalBonds(IAtom focus, IBond[] bonds) {
        ...
        for (final IBond bond : bonds) {
            ...
            } else if (absDeltaX > CARDINALITY_THRESHOLD && absDeltaY < CARDINALITY_THRESHOLD) {
    ...
    /**
     * Helper method to obtain the neighbouring bonds from an adjacency list graph and edge->bond
     * map.
     *
     * @param v vertex
