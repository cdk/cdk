...
/**
 * Recognise stereochemistry of Haworth, Chair, and Boat (not yet implemented) projections. These
 * projections are a common way of depicting closed-chain (furanose and pyranose) carbohydrates and
 * require special treatment to interpret stereo conformation.
 *
 * <p>The methods used are described by {@cdk.cite batchelor13}.
 *
 * @author John May
 * @cdk.githash
...
final class CyclicCarbohydrateRecognition {
     ...
     * projection. For Haworth projections, the substituents must be directly up or down (within
     * some threshold).
     *
    ...
    private List<ITetrahedralChirality> newTetrahedralCenters(
            int[] cycle, int[] above, int[] below, WoundProjection type) {
        ...
        for (int i = 1; i <= cycle.length; i++) {
            ...
            if (!stereocenters.isStereocenter(curr)) continue;
            ...
            centers.add(
                    new TetrahedralChirality(
                            container.getAtom(curr),
                            new IAtom[] {
                                container.getAtom(up),
    ...
    /**
     * Obtain the direction of a substituent relative to the center location. In a Haworth
     * projection the substituent must be directly above or below (with threshold) the center.
     *
     * @param centerXy location of center
     * @param substituentXy location fo substituent
     * @param horizontalXy horizontal offset, x > 0
     * @param haworth is Haworth project (substituent must be directly up or down)
     * @return the direction (up, down, other)
     */
    private static Direction direction(
            Point2d centerXy, Point2d substituentXy, Point2d horizontalXy, boolean haworth) {
        ...
        if (haworth && Math.abs(deltaX) > CARDINALITY_THRESHOLD) return Direction.Other;
    ...
    /**
     * Determine the horizontal offset of the projection. This allows projections that are drawn at
     * angle to be correctly interpreted. Currently only projections of chair conformations are
     * considered.
     *
    ...
    private Point2d horizontalOffset(Point2d[] points, Turn[] turns, Projection projection) {
        ...
        // is relative
        if (projection != Projection.Chair) return new Point2d(0, 0);
    ...
    /**
     * Determines the center index offset for the chair projection. The center index is that of the
     * two atoms with opposite turns (fewest). For, LLRLLR the two centers are R and the index is 2
     * (first is in position 2).
     *
    ...
    /**
     * Pairing of Projection + Winding. The wound projection is determined from an array of turns.
     */
    private enum WoundProjection {
        ...
        private static final Map<Key, WoundProjection> map = new HashMap<Key, WoundProjection>();
