...
/**
 * Generates a fingerprint for a given AtomContainer. Fingerprints are one-dimensional bit arrays,
 * where bits are set according to a the occurrence of a particular structural feature (See for
 * example the Daylight inc. theory manual for more information). Fingerprints allow for a fast
 ...
 * element currently supported in {@link org.openscience.cdk.tools.periodictable.PeriodicTable}.
 *
 * <p><font color="#FF0000">Warning: The aromaticity detection for this FingerPrinter relies on
 * AllRingsFinder, which is known to take very long for some molecules with many cycles or special
 * cyclic topologies. Thus, the AllRingsFinder has a built-in timeout of 5 seconds after which it
 * aborts and throws an Exception. If you want your SMILES generated at any expense, you need to
 * create your own AllRingsFinder, set the timeout to a higher value, and assign it to this
 * FingerPrinter. In the vast majority of cases, however, the defaults will be fine. </font>
 *
 * <p><font color="#FF0000">Another Warning : The daylight manual says: "Fingerprints are not so
 * definite: if a fingerprint indicates a pattern is missing then it certainly is, but it can only
 * indicate a pattern's presence with some probability." In the case of very small molecules, the
 * probability that you get the same fingerprint for different molecules is high. </font>
 *
 * @author steinbeck
...
public class Fingerprinter extends AbstractFingerprinter implements IFingerprinter {
    ...
    private IBond findBond(List<IBond> bonds, IAtom beg, IAtom end) {
        for (IBond bond : bonds) if (bond.contains(beg) && bond.contains(end)) return bond;
        return null;
    }

    private String encodePath(
            IAtomContainer mol,
            Map<IAtom, List<IBond>> cache,
            List<IAtom> path,
    ...
    private int appendHash(int hash, String str) {
        int len = str.length();
        for (int i = 0; i < len; i++) hash = 31 * hash + str.charAt(0);
        return hash;
    }
    ...
    private static final class State {
        ...
        void push(IAtom atom, IBond bond) {
            apath.add(atom);
            if (bond != null) bpath.add(bond);
        }

        void pop() {
            if (!apath.isEmpty()) apath.remove(apath.size() - 1);
            if (!bpath.isEmpty()) bpath.remove(bpath.size() - 1);
        }
     ...
     *
     * <p>This method will find all paths up to length N starting from each atom in the molecule and
    ...
    protected int[] findPathes(IAtomContainer container, int searchDepth) throws CDKException {
        ...
        for (IAtom startAtom : container.atoms()) {
            List<List<IAtom>> p =
                    PathTools.getLimitedPathsOfLengthUpto(
                            container, startAtom, searchDepth, pathLimit);
        ...
        for (Integer hash : hashes) result[pos++] = hash;

        return result;
    }

    protected void encodePaths(IAtomContainer mol, int depth, BitSet fp, int size)
            throws CDKException {
    ...
    private static boolean hasPseudoAtom(Iterable<IAtom> path) {
        for (IAtom atom : path) if (isPseudo(atom)) return true;
        return false;
    }
    ...
    private int encodeUniquePath(
            ...
            List<IAtom> path,
            StringBuilder buffer) {
        if (path.size() == 1) return getAtomSymbol(path.get(0)).hashCode();
        ...
        final int x;
        if (reverse.compareTo(forward) < 0) x = forward.hashCode();
        else x = reverse.hashCode();
        return x;
    }
    ...
    /**
     * Compares a path of atoms with it's self to give the lexicographically lowest traversal
    ...
    private int encodeUniquePath(List<IAtom> apath, List<IBond> bpath, StringBuilder buffer) {
        if (bpath.size() == 0) return getAtomSymbol(apath.get(0)).hashCode();
        final int x;
