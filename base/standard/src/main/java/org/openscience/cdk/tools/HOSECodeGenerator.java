...
/**
 * Generates HOSE codes {@cdk.cite BRE78}. IMPORTANT: Your molecule must contain implicit or
 * explicit hydrogens for this method to work properly.
 *
 * @author steinbeck
 * @cdk.githash
...
public class HOSECodeGenerator implements java.io.Serializable {
    ...
    /**
     * Container for the node in the next sphere Assembled in a recursive method and then passed to
     * the next recursion to become "sphereNodes".
     */
    ...
    /**
     * Delimiters used to separate spheres in the output string. Bremser uses the sequence"(//)" for
     * the first four spheres.
     */
    ...
    static final int[] symbolRankings = {
        9000, 8900, 8800, 8700, 8600, 8500, 8400, 8300, 8200, 8100, 8000, 7900, 1200, 1100, 1000
    };
        ...
        if (isotopeFac == null) {
            try {
                ...
                throw new CDKException(
                        "Could not instantiate the IsotopeFactory: " + e.getMessage(), e);
            }
        }
    }

    /**
     * This method is intended to be used to get the atoms around an atom in spheres. It is not used
     * in this class, but is provided for other classes to use. It also creates the HOSE code in
     * HOSECode as a side-effect.
     *
     * @param ac The {@link IAtomContainer} with the molecular skeleton in which the root atom
     *     resides.
     * @param root The root atom for which to produce the spheres.
     * @param noOfSpheres The number of spheres to look at.
     * @param ringsize Shall the center code have the ring size in it? Only use if you want to have
     *     the hose code later, else say false.
     * @return An array of {@link List}. The list at i-1 contains the atoms at sphere i as
     *     TreeNodes.
     */
    public List<IAtom>[] getSpheres(
            IAtomContainer ac, IAtom root, int noOfSpheres, boolean ringsize) throws CDKException {
        ...
        rootNode =
                new TreeNode(
                        ...
                        root,
                        (double) 0,
                        atomContainer.getConnectedBondsCount(root),
                        0);
        /*
    ...
    /**
     * Produces a HOSE code for Atom <code>root</code> in the {@link IAtomContainer} <code>ac</code>
     * . The HOSE code is produced for the number of spheres given by <code>noOfSpheres</code>.
     * IMPORTANT: if you want aromaticity to be included in the code, you need to run the
     * IAtomContainer <code>ac</code> to the {@link
     * org.openscience.cdk.aromaticity.CDKHueckelAromaticityDetector} prior to using <code>
     * getHOSECode()</code>. This method only gives proper results if the molecule is fully
     * saturated (if not, the order of the HOSE code might depend on atoms in higher spheres). This
     * method is known to fail for protons sometimes. IMPORTANT: Your molecule must contain implicit
     * or explicit hydrogens for this method to work properly.
     *
     * @param ac The {@link IAtomContainer} with the molecular skeleton in which the root atom
     *     resides
     ...
     * @exception org.openscience.cdk.exception.CDKException Thrown if something is wrong
     */
    public String getHOSECode(IAtomContainer ac, IAtom root, int noOfSpheres, boolean ringsize)
            throws CDKException {
        ensureIsotopeFactory();
    ...
    private void createCenterCode(IAtom root, IAtomContainer ac, boolean ringsize) {
        ...
        partnerCount =
                atomContainer.getConnectedBondsCount(root)
                        + (root.getImplicitHydrogenCount() == CDKConstants.UNSET
                                ? 0
                                : root.getImplicitHydrogenCount());
        centerCode =
                root.getSymbol()
                        + "-"
                        + partnerCount
                        + createChargeCode(root)
    ...
    /**
     * Prepares for a breadth first search within the {@link IAtomContainer}. The actual recursion
     * is done in <code>nextSphere()</code>.
     *
                ...
                if (bond.getFlag(CDKConstants.ISAROMATIC)) {
                    tempNode =
                            new TreeNode(
                                    atom.getSymbol(),
                                    new TreeNode(
                                            root.getSymbol(), null, root, (double) 0, 0, (long) 0),
                                    atom,
                                    4,
                                    atomContainer.getConnectedBondsCount(atom),
                                    0);
                } else {
                    tempNode =
                            new TreeNode(
    ...
    /**
     * The actual recursion method for our breadth first search. Each node in sphereNodes is
     * inspected for its descendants which are then stored in <code>nextSphereNodes</code>, which
     * again is passed to the next recursion level of <code>nextSphere()</code>.
     *
                            ...
                            if (bond.getFlag(CDKConstants.ISAROMATIC)) {
                                nextSphereNodes.add(
                                        new TreeNode(
                                                ...
                                                toNode,
                                                4,
                                                atomContainer.getConnectedBondsCount(toNode),
                                                treeNode.score));
                            } else {
                                nextSphereNodes.add(
                                        new TreeNode(
    ...
    /**
     * After recursively having established the spheres and assigning each node an appropriate
     * score, we now generate the complete HOSE code.
     *
    ...
    /**
     * Returns the Bremser-compatible symbols for a given element. Silicon, for example, is actually
     * "Q". :-)
     *
    ...
    /**
     * Determines the ranking score for each node, allowing for a sorting of nodes within one
     * sphere.
     *
    ...
    /**
     * Sorts the nodes (atoms) in the sphereNode vector according to their score. This is used for
     * the essential ranking of nodes in HOSE code sphere.
     *
    ...
    private void sortNodesByScore(List<TreeNode> sphereNodes) {
        ...
        do {
            ...
            for (int i = 0; i < sphereNodes.size() - 1; i++) {
                if (((TreeNode) sphereNodes.get(i + 1))
                                .stringscore.compareTo(((TreeNode) sphereNodes.get(i)).stringscore)
                        > 0) {
    ...
    /**
     * If we use less than four sphere, this fills up the code with the missing delimiters such that
     * we are compatible with Bremser's HOSE code table.
     */
    ...
    class TreeNodeComparator implements Comparator<TreeNode> {
        ...
        /**
         * Access the canonical label for the given tree node's atom. If any component is null then
         * {@link Long#MIN_VALUE} is return thus sorting that object in lower order.
         *
