...
/**
 * Class implements methods to assign mmff94 atom types for a specific atom in an molecule. The full
 * list of mmff94 atom types is defined in the file <b>cdk/config/data/mmff94_atomtypes.xml</b>.
 *
 * @author cho
...
public class MMFF94AtomTypeMatcher implements IAtomTypeMatcher {
    ...
    String[] atomTypeIds = {
        ...
        "C5A", "C5B", "C5", "HC", "HO", "HN", "HOCO", "HN=C", "HN2", "HOCC", "HOH", "HOS", "HN+",
        "HO+", "HO=+", "HP", "O", "O=", "OX", "OM", "O+", "O=+", "OH2", "Oar", "N", "N=C", "NC=C",
        "NSP", "=N=", "NAZT", "N+", "N2OX", "N3OX", "NC#N", "NO3", "N=O", "NC=O", "NSO", "N+=",
        "NCN+", "NGD+", "NR%", "NM", "N5M", "NPYD", "NPYL", "NPD+", "N5A", "N5B", "NPOX", "N5OX",
        "N5+", "N5", "S", "S=C", ">SN", "SO2", "SX", "SO2M", "=SO", "Sthi", "PTET", "P", "-P=C",
        "F", "CL", "BR", "I", "SI", "CL04", "FE+2", "FE+3", "F-", "CL-", "BR-", "LI+", "NA+", "K+",
        "ZN+2", "CA+2", "CU+1", "CU+2", "MG+2", "DU"
    };
    ...
    /**
     * Assign the mmff94 atom type to a given atom. Before this method can be called the following
     * has to be done: atomContainer=(AtomContainer)atomTypeTools.assignAtomTypePropertiesToAtom(new
    ...
    public IAtomType findMatchingAtomType(IAtomContainer atomContainer, IAtom atomInterface)
        ...
        logger.debug(
                " Symbol:"
                        + atom.getSymbol()
                        + " HoseCode>"
                        + atom.getProperty(CDKConstants.SPHERICAL_MATCHER)
                        + " ");
        // System.out.print("IN MMFF94AtomTypeMatcher Symbol:" + atom.getSymbol() +" HoseCode>" +
        // atom.getProperty(CDKConstants.SPHERICAL_MATCHER) + " ");
        ...
        for (int j = 0; j < atomTypeIds.length; j++) {
            ...
            String atomSphericalMatcher =
                    (String)
                            factory.getAtomType(atomTypeIds[j])
                                    .getProperty(CDKConstants.SPHERICAL_MATCHER);
            ...
            if (mat1.matches()) {
                ...
                if (atomTypeIds[j].equals("C")) {
                        ...
                        if (ringSize != null && maxBondOrder == IBond.Order.SINGLE) {
                            ...
                            if (mat1.matches()
                                ...
                                ID = atomTypeIds[13];
                                    ...
                                    && atomRingSize == 5) {
                                ID = atomTypeIds[12];
                            } else if (atomChemGroupConstant % 2 == 0
                                    && atom.getFlag(CDKConstants.ISAROMATIC)
                                    && atomRingSize == 5) {
                    ...
                    if (atomChemGroupConstant == 12) {
                        ID = atomTypeIds[61]; // NPOX
                    } else if (atomChemGroupConstant != -1
                            && atom.getFlag(CDKConstants.ISAROMATIC)
                            && atomRingSize == 5) {
                        ID = atomTypeIds[62]; // N5OX
                    }
