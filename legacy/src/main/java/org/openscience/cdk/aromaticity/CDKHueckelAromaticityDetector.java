...
/**
 * This aromaticity detector detects the aromaticity based on the H&uuml;ckel 4n+2 pi-electrons rule
 * applied to isolated ring systems. It assumes CDK atom types to be perceived with the {@link
 * CDKAtomTypeMatcher} or with any compatible class. For example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Important to note is that this aromaticity detector does not allow aromatic ring atoms to have
 * double bonds pointing outwards from the ring, and thus not having pi electrons shared with other
 * ring atoms. If such double bonds are not important, have a look at the {@link
...
public class CDKHueckelAromaticityDetector {
    ...
    public static boolean detectAromaticity(IAtomContainer atomContainer) throws CDKException {
        ...
        for (IAtom atom : disregard) ringSystems.removeAtom(atom);
    ...
    /**
     * Determines if the isolatedRingSystem has attached double bonds, which are not part of the
     * ring system itself, and not part of any other ring system. Exceptions: a N.sp2.3 nitrogen
     * with a double ring to an oxygen outwards.
     */
            ...
            while (neighborBonds.hasNext()) {
                ...
                if (!neighborBond.getFlag(CDKConstants.ISINRING)
                    ...
                    if (!("N.sp2.3".equals(atom.getAtomTypeName())
                            && "O.sp2".equals(neighborBond.getOther(atom).getAtomTypeName())))
                        return true;
                }
            }
        }
