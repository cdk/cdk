 ...
 * It does this in a non-general way by looking at the ring size and take everything as a special
 * case.
 *
 * @author seb
 ...
 * @deprecated the newer {@link org.openscience.cdk.aromaticity.Kekulization} provides a faster,
 *     more generic and comprehensive algorithm.
 */
@Deprecated
public class DeAromatizationTool {

    /**
     * Methods that takes a ring of which all bonds are aromatic, and assigns single and double
     * bonds. It does this in a non-general way by looking at the ring size and take everything as a
     * special case.
     *
        ...
        if (ring.getRingSize() == 6) {
            if (MolecularFormulaManipulator.getElementCount(formula, new Element("C")) == 6) {
                result = DeAromatizationTool.deAromatizeBenzene(ring);
                    ...
                    && MolecularFormulaManipulator.getElementCount(formula, new Element("N"))
                            == 1) {
        ...
        for (int i = 0; i < ring.getAtomCount(); i++) {
            ...
            if (atom.getSymbol().equals("N")) {
                ...
                while (done != 2) {
                    ...
                    if (bond.getBegin().equals(atom)) atom = bond.getEnd();
                    else atom = bond.getBegin();
                    count++;
                    if (count % 2 == 0) {
    ...
    public static boolean Dearomatize666Rings(IRingSet rings) {
        ...
        if (maxAtomsRingA != 6 || maxAtomsRingB != 6 || maxAtomsRingC != 6) {
            // Stop if not a 6-membered ring fused to 6-membered ring fused to another 6-membered
            // ring.
            return false;
        }

        if (!isAllAtomsAromatic(ringA)
                || !isAllAtomsAromatic(ringB)
                || !isAllAtomsAromatic(ringC)) {
    ...
    public static boolean Dearomatize566Rings(IRingSet rings) {
        ...
        if (maxAtomsRingA != 5 || maxAtomsRingB != 6 || maxAtomsRingC != 6) {
            // Stop if not a 5-membered ring fused to 6-membered ring fused to another 6-membered
            // ring.
            return false;
        }
        ...
        for (int a = 0; a < maxAtomsRingA; ++a) {
            ...
            if ((atom.getAtomicNumber() == 7 && atom.getImplicitHydrogenCount() >= 1)
                    || // nH
    ...
    private static boolean isMiddleRing(
            IAtomContainer ring, IAtomContainer ringA, IAtomContainer ringB) {
