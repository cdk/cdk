...
/**
 * This class contains the necessary information to predict ionization potential energy. It contains
 * the models and the families classification. It is used as IPAtomicLearningDescriptor as the
...
public class IonizationPotentialTool {
    ...
    public static double predictIP(IAtomContainer container, IAtom atom) throws CDKException {
        ...
        if (familyHalogen(atom)) value = getDTHalogenF(getQSARs(container, atom));
        else if (familyOxygen(atom)) value = getDTOxygenF(getQSARs(container, atom));
    ...
    public static double predictIP(IAtomContainer container, IBond bond) throws CDKException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (familyHalogen(atom)) return value;
            else if (familyOxygen(atom)) return value;
    ...
    /**
     * Looking if the Atom belongs to the oxygen family. The IAtoms are O, S, Se, Te.
     *
     * @param atom The IAtom
     * @return True, if it belongs
     */
    ...
    /**
     * Looking if the Atom belongs to the nitrogen family. The IAtoms are N, P, As, Sb.
     *
     * @param atom The IAtom
     * @return True, if it belongs
     */
    ...
    private static int getDoubleBondNumber(IAtomContainer container) {
        ...
        for (IBond bond : container.bonds()) {
            if (bond.getOrder().equals(IBond.Order.DOUBLE)
                    || bond.getOrder().equals(IBond.Order.TRIPLE)) doubleNumber++;
        }
        return doubleNumber;
    }

    /**
     * Get the results of 7 qsar descriptors been applied. They are: Electronegativity,
     * GasteigerMarsiliPartialCharges, GasteigerPEPEPartialCharges, Polarizability,
     * StabilizationCharge, Number of Atom in resonance if the container in resonance is aromatic.
     *
    ...
    /**
     * Get the prediction result for the Halogen family given a series of values. It is based on 167
     * instances and 9 attributes(descriptors) using the Linear Regression Model with result of Root
     * mean squared error 0.5817 with a cross validation of 10 folds.
     *
    ...
    private static double getDTHalogenF(double[] resultsH) {
        ...
        //		System.out.println("SE : "+SE+", PE : "+PE+", PSC : "+PSC+", PIC : "+PIC+", ETP :
        // "+ETP+", SPC : "+SPC+", COUNTR : "+COUNTR+", COUNTAr : "+COUNTAr);
        // model leastMedSq
        result =
                0.272 * SE
                        + 13.5814 * PSC
    ...
    /**
     * Get the prediction result for the Oxygen family given a series of values. It is based on 368
     * instances and 9 attributes(descriptors) using the Linear Regression Model with result of Root
     * mean squared error 0.64 with a cross validation of 10 folds.
     *
     * @param resultsH Array which contains the results of each descriptor
     * @return The result
     */
    ...
    /**
     * Get the prediction result for the Nitrogen family given a series of values. It is based on
     * 244 instances and 9 attributes(descriptors) using the Linear Regression Model with result of
     * Root mean squared error 0.54 with a cross validation of 10 folds.
     *
     * @param resultsH Array which contains the results of each descriptor
     * @return The result
     */
    ...
    /**
     * Get the desicion-tree result for the Halogen family given a series of values. It is based in
     * 6 qsar descriptors.
     *
     * @param resultsH Array which contains the results of each descriptor
     * @return The result
     ...
     * @throws CDKException
     */
    private static IAtomContainer initiateIonization(IAtomContainer container, IAtom atom)
            throws CDKException {
        ...
        if (setOfReactions != null
                && setOfReactions.getReactionCount() == 1
