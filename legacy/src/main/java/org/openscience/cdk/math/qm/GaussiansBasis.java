...
/**
 * This class contains the information to use gauss function as a base for calculation of quantum
 * mechanics. The function is defined as:
 *
 * <pre>
...
public class GaussiansBasis implements IBasis {
    ...
    /**
     * Set up basis with gauss funktions f(x,y,z) = (x-rx)^nx * (y-ry)^ny * (z-rz)^nz *
     * exp(-alpha*(r-ri)^2).
     *
     ...
     * @param atoms The atom will need to calculate the core potential
     */
    protected void setBasis(
            int[] nx, int[] ny, int[] nz, double[] alpha, Vector[] r, IAtom[] atoms) {
        int i;
        ...
        for (i = 0; i < count_atoms; i++) {
            ...
            //      logger.debug((i+1)+".Atom Z="+this.oz[i]+" r="+(new
            // Vector(atoms[i].getPoint3d()))+"[angstrom]");
        }
        //    logger.debug();
        ...
        for (i = 0; i < count; i++) {
            ...
            if (norm[i] == 0d) norm[i] = 1d;
            else norm[i] = 1 / norm[i];
    ...
    public double getValue(int index, double x, double y, double z) {
        ...
        double result = 1d;
        int i;
        for (i = 0; i < nx[index]; i++) result *= dx;
        for (i = 0; i < ny[index]; i++) result *= dy;
        for (i = 0; i < nz[index]; i++) result *= dz;
    ...
    public Vector getValues(int index, Matrix m) {
        ...
        mx = 1d;
        for (i = 0; i < nx[index]; i++) mx *= dx;

        my = 1d;
        for (i = 0; i < ny[index]; i++) my *= dy;

        mz = 1d;
        for (i = 0; i < nz[index]; i++) mz *= dz;
    ...
    public double calcD(
            double normi, double normj, double alphai, double alphaj, Vector ri, Vector rj) {
        double dx = ri.vector[0] - rj.vector[0];
        double dy = ri.vector[1] - rj.vector[1];
        double dz = ri.vector[2] - rj.vector[2];
        return Math.exp(-((alphai * alphaj) / (alphai + alphaj)) * (dx * dx + dy * dy + dz * dz))
                * normi
                * normj;
    }
    ...
    public double calcG(
            int n, double t, double alphai, double alphaj, double xi, double xj, double xN) {
        if (n > 1)
            return ((n - 1) / (2 * (alphai + alphaj))) * calcG(n - 2, t, alphai, alphaj, xi, xj, xN)
                    ...
                    + (((alphai * xi + alphaj * xj) / (alphai + alphaj)) - xN)
                            * t
                            * t
                            * calcG(n - 1, t, alphai, alphaj, xi, xj, xN);
        else if (n == 1)
            return (((alphai * xi + alphaj * xj) / (alphai + alphaj)) - xi)
    ...
    public double calcV(int i, int j, Vector rN, double Z) {
        ...
        double X =
                alphaij
                        * ((rxij - rN.vector[0]) * (rxij - rN.vector[0])
    ...
    public double calcG(
        ...
        double C00 =
                (xij - xi)
                        - ((u * u * alphakl * (xij - xkl))
                                / (u * u * (alphaij + alphakl) + alphaij * alphakl));
        double Cs00 =
                (xkl - xk)
                        + ((u * u * alphaij * (xij - xkl))
                                / (u * u * (alphaij + alphakl) + alphaij * alphakl));
        ...
        // Nach 1)
        for (i = 2; i <= n; i++) G[i][0] = (i - 1) * B10 * G[i - 2][0] + C00 * G[i - 1][0];

        // Nach 2)
        if (m > 0) G[0][1] = Cs00;

        // Nach 2)
        for (i = 2; i <= m; i++) G[0][i] = (i - 1) * Bs01 * G[0][i - 2] + Cs00 * G[0][i - 1];

        // Nach 1)
        if (n > 0) for (i = 1; i <= m; i++) G[1][i] = i * B00 * G[0][i - 1] + C00 * G[0][i];

        // Nach 1)
        for (i = 2; i <= n; i++)
            for (j = 1; j <= m; j++)
                G[i][j] =
                        (i - 1) * B10 * G[i - 2][j] + j * B00 * G[i - 1][j - 1] + C00 * G[i - 1][j];
    ...
    public double calcI(
        ...
        if (nj > 0)
            return calcI(ni + 1, nj - 1, nk, nl, u, alphai, alphaj, alphak, alphal, xi, xj, xk, xl)
                    + (xj - xi)
                            * calcI(
                                    ni, nj - 1, nk, nl, u, alphai, alphaj, alphak, alphal, xi, xj,
                                    xk, xl);
        ...
        if (nl > 0)
            return calcI(ni, nj, nk + 1, nl - 1, u, alphai, alphaj, alphak, alphal, xi, xj, xk, xl)
                    + (xl - xk)
                            * calcI(
                                    ni, nj, nk, nl - 1, u, alphai, alphaj, alphak, alphal, xi, xj,
                                    xk, xl);
    ...
    public double calcI(int i, int j, int k, int l) {
        ...
        double C =
                (Math.PI
                                * Math.PI
                                * Math.PI
                                / Math.pow((alpha[i] + alpha[j]) * (alpha[k] + alpha[l]), 1.5))
                        * Math.sqrt(alpha0)
                        * calcD(norm[i], norm[j], alpha[i], alpha[j], r[i], r[j])
