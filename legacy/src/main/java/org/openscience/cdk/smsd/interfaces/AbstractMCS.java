...
public abstract class AbstractMCS {
     ...
     * initialize query and target molecules.
     *
     * <p>Note: Here its assumed that hydrogens are implicit and user has called these two methods
     * percieveAtomTypesAndConfigureAtoms and CDKAromicityDetector before initializing calling this
     * method.
     *
     * @param source query mol
     * @param target target mol
     ...
     * @param energyFilter set true to return matches with minimum bond changes based on the bond
     *     breaking energy
     */
    public abstract void setChemFilters(
            boolean stereoFilter, boolean fragmentFilter, boolean energyFilter);

    /**
     * Returns summation energy score of the disorder if the MCS is removed from the target and
     * query graph. Amongst the solutions, a solution with lowest energy score is preferred.
     *
    ...
    /**
     * Returns number of fragment generated in the solution space, if the MCS is removed from the
     * target and query graph. Amongst the solutions, a solution with lowest fragment size is
     * preferred.
     *
     * @param key Index of the mapping solution
    ...
    /**
     * Returns a number which denotes the quality of the mcs. A solution with highest stereo score
     * is preferred over other scores.
     *
     * @param key Index of the mapping solution
     * @return true if no stereo mismatch occurs else false if stereo mismatch occurs
     */
     ...
     *
     * @return true if mols have different stereo chemistry else false if no stereo mismatch. true
     *     if stereo mismatch occurs else true if stereo mismatch occurs.
     */
    public abstract boolean isStereoMisMatch();

    /**
     * Checks if query is a subgraph of the target. Returns true if query is a subgraph of target
     * else false
     *
    ...
    /**
     * Returns Tanimoto similarity between query and target molecules (Score is between 0-min and
     * 1-max).
     *
