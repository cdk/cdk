...
public class McGregorChecks {
            ...
            for (int column = 0; column < neighborBondNumB; column++) {
                ...
                if (isAtomMatch(g1A, g2a, g1B, g2B)) {
                    try {
                        ...
                        if (isMatchFeasible(
                                source, reactantBond, target, productBond, shouldMatchBonds)) {
                            return true;
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
     ...
     * @return
     */
    protected static int searchCorrespondingAtom(
            int mappedAtomsSize,
        ...
        for (int a = 0; a < mappedAtomsSize; a++) {
            if ((molecule == 1)
                    && (mappedAtoms.get(a * 2 + 0).intValue() == atomFromOtherMolecule)) {
                correspondingAtom = mappedAtoms.get(a * 2 + 1);
            }
            if ((molecule == 2)
                    && (mappedAtoms.get(a * 2 + 1).intValue() == atomFromOtherMolecule)) {
    ...
    // Function compaires a structure array with itself. Sometimes a mapping occurs several times
    // within the array.
    // The function eliminates these recurring mappings. Function is called in function
    // best_solution.
    ...
    /**
     * The function is called in function partsearch. The function is given a temporary matrix and a
     * position (row/column) within this matrix. First the function sets all entries to zero, which
     * can be exlcuded in respect to the current atom by atom matching. After this the function
     * replaces all entries in the same row and column of the current position by zeros. Only the
     * entry of the current position is set to one. Return value "count_arcsleft" counts the number
     * of arcs, which are still in the matrix.
     *
     * @param row
     * @param column
     * @param marcs
    ...
    protected static void removeRedundantArcs(
            int row, int column, List<Integer> marcs, McgregorHelper mcGregorHelper) {
     ...
     * @throws IOException
     */
    protected static List<String> generateCTabCopy(IAtomContainer atomContainer)
            throws IOException {
        List<String> cTabCopy = new ArrayList<String>();
     ...
     * @return
     */
    protected static boolean case1(
            int g1Atom,
            int g3Atom,
            int g4Atom,
            int rowAtom1,
        ...
        if (((g1Atom == rowAtom1) || (g1Atom == rowAtom2))
                && (!(((columnAtom3 == g3Atom) || (columnAtom4 == g3Atom))
                        || ((columnAtom3 == g4Atom) || (columnAtom4 == g4Atom))))) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    protected static boolean case2(
            int g2Atom,
            int g3Atom,
            int g4Atom,
            int rowAtom1,
     ...
     * @return
     */
    protected static boolean case3(
            int g1Atom,
            int g3Atom,
            int g2Atom,
            int rowAtom1,
        ...
        if (((g3Atom == columnAtom3) || (g3Atom == columnAtom4))
                && (!(((rowAtom1 == g1Atom) || (rowAtom2 == g1Atom))
                        || ((rowAtom1 == g2Atom) || (rowAtom2 == g2Atom))))) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    protected static boolean case4(
            int g1Atom,
            int g2Atom,
            int g4Atom,
            int rowAtom1,
     ...
     * @return
     */
    protected static boolean cases(
            int g1Atom,
            int g2Atom,
            int g3Atom,
            int g4Atom,
     ...
     * @return
     */
    protected static List<Integer> setArcs(
            IAtomContainer source,
            IAtomContainer target,
            int neighborBondNumA,
            int neighborBondNumB,
     ...
     * @return
     */
    protected static int countArcsLeft(
            List<Integer> tempmarcs, int neighborBondNumA, int neighborBondNumB) {
        int arcsleft = 0;
     ...
     * @return
     */
    protected static int changeCharBonds(
            int correspondingAtom,
            String newSymbol,
            int neighborBondNum,
    ...
    static boolean isFurtherMappingPossible(
            IAtomContainer source,
            IAtomContainer target,
    ...
    static List<Integer> markUnMappedAtoms(
            boolean flag, IAtomContainer container, Map<Integer, Integer> presentMapping) {
    ...
    static List<Integer> markUnMappedAtoms(
            boolean flag, IAtomContainer container, List<Integer> mappedAtoms, int cliqueSize) {
        List<Integer> unmappedMolAtoms = new ArrayList<Integer>();
        int unmappedNum = 0;
        boolean atomIsUnmapped = true;
        for (int a = 0; a < container.getAtomCount(); a++) {
