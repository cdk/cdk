 ...
 *
 * <p>The SMSD algorithm is described in this paper. <font color="#FF0000">please refer Rahman
 * <i>et.al. 2009</i></font> {@cdk.cite SMSD2009}.
 *
 * @cdk.module smsd
 * @cdk.githash
...
public final class McGregor {
     ...
     */
    public McGregor(
            IQueryAtomContainer source, IAtomContainer target, List<List<Integer>> mappings) {
    ...
    public void startMcGregorIteration(int largestMappingSize, Map<Integer, Integer> presentMapping)
        ...
        List<Integer> unmappedAtomsMolA =
                McGregorChecks.markUnMappedAtoms(true, source, presentMapping);
        int counter = 0;
        int gSetBondNumA = 0;
        int gSetBondNumB = 0;
        int gNeighborBondnumA =
                0; // number of remaining molecule A bonds after the clique search, which are
                   // neighbors of the MCS_1
        int gNeighborBondNumB =
                0; // number of remaining molecule B bonds after the clique search, which are
                   // neighbors of the MCS_1
        ...
        if (!(source instanceof IQueryAtomContainer)) {
            ...
            queryProcess.process(
                    (IQueryAtomContainer) source, target, unmappedAtomsMolA, mappedAtoms, counter);
        }
        ...
        List<Integer> unmappedAtomsMolB =
                McGregorChecks.markUnMappedAtoms(false, target, presentMapping);
        ...
        // In case that unmapped atoms are connected with already mapped atoms, the mapped atoms are
        // labelled with
        // new special signs -> the result are two vectors: cBondNeighborsA and int_bonds_molB,
        // which contain those
        ...
        targetProcess.process(
                target, unmappedAtomsMolB, mappingSize, iBondSetB, cBondSetB, mappedAtoms, counter);
     ...
     * @throws IOException
     */
    public void startMcGregorIteration(
            int largestMappingSize, List<Integer> cliqueVector, List<Integer> compGraphNodes)
            throws IOException {
        this.globalMCSSize = (largestMappingSize / 2);
        List<String> cTab1Copy = McGregorChecks.generateCTabCopy(source);
        ...
        // find unmapped atoms of molecule A
        List<Integer> unmappedAtomsMolA =
                McGregorChecks.markUnMappedAtoms(true, source, mappedAtoms, cliqueSize);

        int counter = 0;
        int setNumA = 0;
        int setNumB = 0;
        int localNeighborBondnumA =
                0; // number of remaining molecule A bonds after the clique search, which are
                   // neighbors of the MCS_1
        int localNeighborBondNumB =
                0; // number of remaining molecule B bonds after the clique search, which are
                   // neighbors of the MCS_1
        ...
        // In case that unmapped atoms are connected with already mapped atoms, the mapped atoms are
        // labelled with
        // new special signs -> the result are two vectors: cBondNeighborsA and int_bonds_molA,
        // which contain those
        ...
        // find unmapped atoms of molecule B
        List<Integer> unmappedAtomsMolB =
                McGregorChecks.markUnMappedAtoms(false, target, mappedAtoms, cliqueSize);
        ...
        targetProcess.process(
                target, unmappedAtomsMolB, cliqueSize, iBondSetB, cBondSetB, mappedAtoms, counter);
    ...
    private int iterator(McgregorHelper mcGregorHelper) throws IOException {
        ...
        boolean furtherMappingFlag =
                McGregorChecks.isFurtherMappingPossible(
                        source, target, mcGregorHelper, isBondMatch());

        if (neighborBondNumA == 0
                || neighborBondNumB == 0
                || mappingCheckFlag
                || !furtherMappingFlag) {
    ...
    private void searchAndExtendMappings(
            Stack<List<Integer>> bestarcsCopy, McgregorHelper mcGregorHelper) throws IOException {
        int mappedAtomCount = mcGregorHelper.getMappedAtomCount();
        ...
        while (!bestarcsCopy.empty()) {
            ...
            List<Integer> newINeighborsA =
                    new ArrayList<Integer>(); // instead of iBondNeighborAtomsA
            List<Integer> newINeighborsB =
                    new ArrayList<Integer>(); // instead of iBondNeighborAtomsB
        ...
        for (int row = 0; row < neighborBondNumA; row++) {
            for (int column = 0; column < neighborBondNumB; column++) {
                ...
                if (matchGAtoms(g1A, g2A, g1B, g2B)) {
                    ...
                    if (McGregorChecks.isMatchFeasible(
                            source, reactantBond, target, productBond, isBondMatch())) {
    ...
    private void partsearch(
            int xstart, int ystart, List<Integer> tempMArcsOrg, McgregorHelper mcGregorHelper) {
        int neighborBondNumA = mcGregorHelper.getNeighborBondNumA();
        int neighborBondNumB = mcGregorHelper.getNeighborBondNumB();
        ...
        if (tempMArcs.get(xstart * neighborBondNumB + ystart) == 1) {
            ...
            int arcsleft =
                    McGregorChecks.countArcsLeft(tempMArcs, neighborBondNumA, neighborBondNumB);
            ...
            } while ((xIndex < neighborBondNumA)
                    && (tempMArcs.get(xIndex * neighborBondNumB + yIndex)
                            != 1)); // Correction by ASAD set value minus 1
    ...
    private boolean verifyNodes(
            List<Integer> matrix, BinaryTree currentStructure, int index, int fieldLength) {
        if (index < fieldLength) {
            if (matrix.get(index) == currentStructure.getValue()
                    && currentStructure.getEqual() != null) {
                setNewMatrix(false);
    ...
    private void startsearch(McgregorHelper mcGregorHelper) {
        ...
        while ((xIndex < neighborBondNumA)
                && (modifiedARCS.get(xIndex * neighborBondNumB + yIndex) != 1)) {
            yIndex++;
            if (yIndex == neighborBondNumB) {
                yIndex = 0;
                xIndex++;
        ...
        try {
            if (mappedAtomCount >= globalMCSSize) {
                ...
                if (mappedAtomCount > globalMCSSize) {
                    ...
                    //                        System.out.println("best_MAPPING_size: " +
                    // globalMCSSize);
                    mappings.clear();
                }
    ...
    private void setArcs(
        ...
        while ((xIndex < neighborBondNumA)
                && (tempMArcs.get(xIndex * neighborBondNumB + yIndex) != 1));
        if (xIndex < neighborBondNumA) {
    ...
    private void extendMapping(
            int xIndex,
            int yIndex,
            McgregorHelper mcGregorHelper,
            List<Integer> additionalMapping,
