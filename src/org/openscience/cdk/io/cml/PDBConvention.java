...
public class PDBConvention extends CMLCoreModule {
	...
	private PDBStrand oStrand;
	private PDBMonomer monomer;
	private boolean canEndElementMol;
    ...
    protected void storeAtomData() {
        ...
        /*monomer*/
        if(atomCounter > 0){
        	String cResidue = resNameV.get(0).toString()+"A"+resSeqV.get(0);
			monomer.setMonomerName(cResidue);
			monomer.setMonomerType(resNameV.get(0).toString());
			monomer.setChainID(chainIDV.get(0).toString());
			monomer.setICode(iCodeV.get(0).toString());
        }
		...
		for (int i = 0; i < atomCounter; i++) {
            ...
            Object[] objects = {monomer, oStrand};
			((ChemFileCDO) cdo).setObjects(objects);
