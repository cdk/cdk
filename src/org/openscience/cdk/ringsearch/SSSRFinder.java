...
public class SSSRFinder
{
    ...
    public RingSet findSSSR(Molecule mol)
    {
        ...
        //full set of atoms in the structure (which is only ever used for counting)
        int fullSetSize = molecule.getAtomCount();
        //and one to store the numbers
        //of the nodes that have been trimmed away.
        Vector trimSet = new Vector(fullSetSize);
        //Further there is a Vector nodesN2 to store the number of N2 nodes
        ...
        if (debug)
        {
            ...
            System.out.println("fullSet.size(): " + fullSetSize);
        }
        ...
        while(trimSet.size() < fullSetSize);
        if (debug)
        {
            System.out.println("fullSet.size(): " + fullSetSize);
     ...
     * explanation of how this algorithm worked.</p>
     *
     * @param   ring
     * @param   mol
     * @return
