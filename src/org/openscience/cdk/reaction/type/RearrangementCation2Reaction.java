 ...
 * 
 **/
public class RearrangementCation2Reaction implements IReactionProcess{
	 ...
	 *
	 */
	public RearrangementCation2Reaction(){
		logger = new LoggingTool(this);
	}
	/**
	 *  Gets the specification attribute of the RearrangementCation2Reaction object
	 *
	 *@return    The specification value
	 */
	public ReactionSpecification getSpecification() {
		return new ReactionSpecification(
				"http://almost.cubic.uni-koeln.de/jrg/Members/mrc/reactionDict/reactionDict#RearrangementCation2Reaction",
				this.getClass().getName(),
				"$Id: RearrangementCation2Reaction.java,v 1.6 2006/04/01 08:26:47 mrc Exp $",
				"The Chemistry Development Kit");
	}
	
	/**
	 *  Sets the parameters attribute of the RearrangementCation2Reaction object
	 *
	...
	public void setParameters(Object[] params) throws CDKException {
		if (params.length > 1) {
			throw new CDKException("RearrangementCation2Reaction only expects one parameter");
		}
	...
	/**
	 *  Gets the parameters attribute of the RearrangementCation2Reaction object
	 *
	...
	public IReactionSet initiate(IMoleculeSet reactants, IMoleculeSet agents) throws CDKException{
		...
		logger.debug("initiate reaction: RearrangementCation2Reaction");
		
		if (reactants.getMoleculeCount() != 1) {
			throw new CDKException("RearrangementCation2Reaction only expects one reactant");
		}
		if (agents != null) {
				...
				for(int j = 0 ; j < bonds.size() ; j++){
					...
					if(bondj.getFlag(CDKConstants.REACTIVE_CENTER)&& bondj.getOrder() == IBond.Order.SINGLE){
						...
						for(int k = 0 ; k < bondsI.size() ; k++){
							...
							if(bondk.getFlag(CDKConstants.REACTIVE_CENTER) && bondk.getOrder() == IBond.Order.DOUBLE){
								...
								if(atomConn.getFlag(CDKConstants.REACTIVE_CENTER) /*&& lp.length == 0*/ )
									...
									for(int l = 0 ; l<acCloned.getBondCount();l++){
										...
										if(bb.getFlag(BONDTOFLAG1)){
											...
											BondManipulator.decreaseBondOrder(bb);
											bondkClon = bb;											
										}
									}
							        ...
							        reaction.addMapping(mapping);
							        mapping = DefaultChemObjectBuilder.getInstance().newMapping(atomConn, acCloned.getAtom(atom2P));
							        reaction.addMapping(mapping);
...
//			return;
				...
				for(int j = 0 ; j < bonds.size() ; j++){
					...
					if(bondj.getOrder() == IBond.Order.SINGLE){
						...
						if(atom.getFormalCharge() == 0){
							...
							for(int k = 0 ; k < bondsI.size() ; k++){
								bondk = (IBond)bondsI.get(k);
								if(bondk.getOrder() == IBond.Order.DOUBLE){
									IAtom atomConn = bondk.getConnectedAtom(atom);
	...
	/**
	 *  Gets the parameterNames attribute of the RearrangementCation2Reaction object
	 *
	...
	/**
	 *  Gets the parameterType attribute of the RearrangementCation2Reaction object
	 *
