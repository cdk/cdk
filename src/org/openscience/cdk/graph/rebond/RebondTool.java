...
/**
 * Provides tools to rebond a molecule from 3D coordinates only. The algorithm uses
 * an efficient algorithm using a Binary Space Partitioning Tree (Bspt).
 *
 * @keyword    rebonding
...
public class RebondTool {
    ...
    public void rebond(AtomContainer container) {
        ...
        // generate a new binary space partition tree
        bspt = new Bspt(3);
    ...
    /** 
     * Returns the bond order for the bond. At this moment, it only returns 0 or 1,
     * but not 2 or 3, or aromatic bond order.
     */
    ...
    class TupleAtom implements Bspt.Tuple {
        ...
        public String toString() {
            return "<" + atom.getX3D() + "," + atom.getY3D() + "," + atom.getZ3D() + ">";
        }
    }
