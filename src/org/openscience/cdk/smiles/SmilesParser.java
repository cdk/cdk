 ...
 * try {
 *   StereoSmilesParser sp = new StereoSmilesParser();
 *   Molecule m = sp.parseSmiles("c1ccccc1");
 * } catch (InvalidSmilesException ise) {
 * }
 * </pre> <p>
 *
 *  This parser does not parse e/z-isomerie information, but the following
 *  features are supported: reaction smiles, partitioned structures, charged
 *  atoms, implicit hydrogen count, '*', chiral centres and isotope information. <p>
 *
 *  See {@cdk.cite WEI88}.
 *
...
public class SmilesParser 
{
    ...
    /*Stores information about the number of borders defined
     *from the last known chiral atom.
     */
    private Stack<String> chiralAtomInformation = new Stack<String>();                                      //for all chiral (C)atoms added: [ID;bondCount]
    private int chiralAtomCounter = 0;
    
    /**
     *  Constructor for the StereoSmilesParser object
     */
    public SmilesParser() 
    {
    ...
    public Molecule parseSmiles(String smiles) throws InvalidSmilesException 
    {
        ...
        /*brings every SMILES in a form like (A)[B@](C)(D)(E)
         *so that it doesn't
         *start with [C@]
         */
        smiles = preprocessSmilesForStereo(smiles);
            ...
            try 
            {
                        ...
                        (mychar == '*')) 
                {
                        ...
                        if (currentSymbol != null) 
                        {
                            if (currentSymbol.length() == 1) 
                            {
                                if (!(currentSymbol.toUpperCase()).equals(currentSymbol)) 
                                {
                                    ...
                                    atom.setAtomTypeName(currentSymbol);
                    ...
                    if ((lastNode != null) && bondExists) 
                    {
                        ...
                        int stereoOrientation = CDKConstants.STEREO_BOND_NONE;
                        
                        /*If lastNode has stereo information (@ or @@ in SMILES),
                         *the bonds to other atoms are defined as dashed wedge,
                         *wedge or normal using the corresponding CDKConstants.
                         *If not, the bond is defined as CDKConstants.STEREO_BOND_NONE.
                         */
                        if (lastNode.getStereoParity() == CDKConstants.STEREO_ATOM_PARITY_MINUS) 
                        {
                            if (getChiralAtomBondCount() == 0) 
                            {
                                stereoOrientation = CDKConstants.STEREO_BOND_DOWN;
                                incrementChiralAtomBondCount();
                            } else if (getChiralAtomBondCount() == 1) 
                            {
                                stereoOrientation = CDKConstants.STEREO_BOND_UP_INV;
                                incrementChiralAtomBondCount();
                            } else 
                            {
                                popChiralAtom();
                            }
                        } else if (lastNode.getStereoParity() == CDKConstants.STEREO_ATOM_PARITY_PLUS) 
                        {
                            if (getChiralAtomBondCount() == 0) 
                            {
                                stereoOrientation = CDKConstants.STEREO_BOND_UP_INV;
                        ...
                        bond = new Bond(atom, lastNode, bondStatus, stereoOrientation);
                        if (bondIsAromatic) 
                        {
                ...
                } else if (mychar == '[') 
                {
                    ...
                    if (isChiralAtom(currentSymbol)) 
                    {
                        /*stereo parity for atom in [] is set and an
                         *hydrogen is explicitly added if necessary
                         */
                        setStereoParity(atom, currentSymbol);
                        addImplicitHydrogenExplicit(atom, currentSymbol, molecule);
                    } else if (currentSymbol.equals("H")) 
                    {
                         ...
                         *If not, the bond is defined as CDKConstants.STEREO_BOND_NONE.
                         */
                        if (chiralAtomInformation.size() != 0) 
                        {
                            if (lastNode.getStereoParity() == CDKConstants.STEREO_ATOM_PARITY_MINUS) 
                            {
                                if (getChiralAtomBondCount() == 0) 
    ...
    /**
     *Tests whether a string contains @/@@ or not (in SMILES: whether
     *an atom is a chiral centre or not)
     *
     *@param s      string that is tested
     *@return       boolean whether string contains @/@@ or not
     */
    private boolean isChiralAtom(String s) 
    {
    ...
    /**
     *Returns the number of outgoing (!!) bonds defined from the last
     *known chiral atom.
     *
     *@return                             Number of outgoing bonds defined from the last known chiral atom
     */
    private int getChiralAtomBondCount() 
    {
        int returnInt = 0;
        String[] temp = ((String)chiralAtomInformation.peek()).split(";");
        returnInt = Integer.parseInt(temp[1]);
        return returnInt;
    }
    
    /**
     *Increments the number of outgoing bonds defined from the last
     *known chiral atom.
     */
    private void incrementChiralAtomBondCount() 
    {
        String[] temp = ((String)chiralAtomInformation.pop()).split(";");
        int newBondCount = (Integer.parseInt(temp[1])) + 1;
        chiralAtomInformation.push(temp[0] + ";" + Integer.toString(newBondCount));
    }
    /**
     *Pops the upper object from the stack that stores the information about the number of borders defined
     *from the last known chiral atom. This method is used, when 3 outgoing
     *bonds are defined.
     */
    private void popChiralAtom()
    {
        Object temp = chiralAtomInformation.pop();
    }
    
    
    /**
     *Tests whether a preprocessing is necessary or not
     *
     *@param  oldSmiles                   The original SMILES string
     *@return                             The preprocessed SMILES
     */
    private String preprocessSmilesForStereo(String oldSmiles) 
    {
        String newSmiles = "";
        if (oldSmiles.startsWith("[")) 
        {
            if (oldSmiles.charAt(1) == 'H') 
            {
                newSmiles = oldSmiles;
            } else 
            {
                newSmiles = stereoPrePro(oldSmiles);
            }
        } else 
        {
            newSmiles = oldSmiles;
    ...
    /**
     *Brings every SMILES in a form like (A)[B@](C)(D)(E) so that it
     *doesn't start with [A@]
     *
     *@param  oldSmiles                   The original SMILES string
     *@return                             The preprocessed SMILES
     */
    private String stereoPrePro(String oldSmiles) 
    {
        String newSmiles = "";
        int position = 0;
        char currentChar = oldSmiles.charAt(position);
        while (currentChar != ']') 
        {
            position++;
            currentChar = oldSmiles.charAt(position);
        }
        position++;
        currentChar = oldSmiles.charAt(position);
        if ((currentChar >= 'A' && currentChar <= 'Z') ||
                ...
                (currentChar == '*')) 
        {
            newSmiles = oldSmiles.substring(position, position+1) +
                    oldSmiles.substring(0, position) +
                    oldSmiles.substring(position+1, oldSmiles.length());
        } else if (currentChar == '(') 
        {
            int bracketOpen = position;
            ...
            newSmiles = oldSmiles.substring(bracketOpen, position+1) +
                    oldSmiles.substring(0, bracketOpen) +
                    oldSmiles.substring(position+1, oldSmiles.length());
        }
        return newSmiles;
    }
    ...
    /**
     *  Gets the AtomString attribute of the StereoSmilesParser object
     *
    ...
    /**
     *  Gets the Charge attribute of the StereoSmilesParser object
     *
    ...
    /**
     *  Gets the implicitHydrogenCount attribute of the StereoSmilesParser object
     *
    ...
    /**
     *  Gets the ElementSymbol attribute of the StereoSmilesParser object
     *
     *@param  s    Description of the Parameter
     *@param  pos  Description of the Parameter
    ...
    /**
     *  Gets the RingNumber attribute of the StereoSmilesParser object
     *
     *@param  s    Description of the Parameter
     *@param  pos  Description of the Parameter
    ...
    /**
     *Sets the stereo parity of an atom
     *
     *@param  atom                         Atom, wchichs stereo parity has to be set
     *@param s                             String that was between [] in SMILES and contains information about the stereo parity of the atom
     */
    private void setStereoParity(Atom atom, String s) 
    {
        int stereoParity = CDKConstants.STEREO_ATOM_PARITY_UNDEFINED;
        int counter = 0;
        ...
        if (counter == 1) 
        { // => only 1 @ => anti clockwise
            stereoParity = CDKConstants.STEREO_ATOM_PARITY_MINUS;
            atom.setStereoParity(stereoParity);
            chiralAtomInformation.push(Integer.toString(chiralAtomCounter) + ";" + 0);
        } else if (counter == 2) 
        { // => 2 @ => clockwise
            stereoParity = CDKConstants.STEREO_ATOM_PARITY_PLUS;
            atom.setStereoParity(stereoParity);
            chiralAtomInformation.push(Integer.toString(chiralAtomCounter) + ";" + 0);
        }
    }
    ...
    /**
     *  Adds an implicit hydrogen explicit if necessary
     *
     *@param  atom                         Atom, to which the potential hydrogen is added
     *@param s                             String that was between [] in SMILES and contains information whether an implicit hydrogen has to be added explicit or not
     *@param mol                           Molecule, to which the potential hydrogen is added
     */
    private void addImplicitHydrogenExplicit(Atom atom, String s, Molecule mol) 
    {
        boolean hydrogen = false;
        for (int i = 0; i < s.length(); i++) 
        {
        ...
        if (hydrogen) 
        {
            ...
            {
                b = new Bond(atom, h, CDKConstants.BONDORDER_SINGLE, CDKConstants.STEREO_BOND_UP);
            } else if (atom.getStereoParity() == CDKConstants.STEREO_ATOM_PARITY_MINUS) 
            {
                b = new Bond(atom, h, CDKConstants.BONDORDER_SINGLE, CDKConstants.STEREO_BOND_DOWN_INV);
            } else 
            {
                b = new Bond(atom, h, CDKConstants.BONDORDER_SINGLE, CDKConstants.STEREO_ATOM_PARITY_UNDEFINED);
            }
            incrementChiralAtomBondCount();
            mol.addBond(b);
        }
