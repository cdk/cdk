...
public class VanDerWaalsInteractions {
	...
	GVector depsilonSK = new GVector(3);
	...
	double[] epsilonAv = null; 	// vdW well depths (Average).
	...
	public void setMMFF94VanDerWaalsParameters(AtomContainer molecule, Hashtable parameterSet) throws Exception {
		...
		epsilonAv = new double[vdwInteractionNumber];
		...
		double gI;	// To epsilonSK calculation
		double gJ;	// To epsilonSK calculation
		double alphaI;	// To epsilonSK calculation and asteriskR[l]
		double alphaJ;	// To epsilonSK calculation and asteriskR[l]
		double nI;
		double nJ;
		...
		for (int i=0; i<molecule.getAtomCount(); i++) {
			for (int j=i+1; j<molecule.getAtomCount(); j++) {
				...
				if (((IntegerResult)shortestPathBetweenToAtoms.calculate(molecule).getValue()).intValue()>2){
					...
					epsilonSK[l] = ((181.16 * gI * gJ * alphaI * alphaJ) / (Math.sqrt(alphaI/nI) + Math.sqrt(alphaJ/nJ))) * 1 / Math.pow(asteriskR[l], 6);
					//System.out.println("epsilonSK = " + epsilonSK[l]);
	...
	public GVector gradientMMFF94SumEvdW_InPoint(AtomContainer molecule, double[] s) {
		...
		depsilonSK.setSize(molecule.getAtomCount() * 3);
		dterm1.setSize(molecule.getAtomCount() * 3);
		dterm2.setSize(molecule.getAtomCount() * 3);
		ds.setSize(molecule.getAtomCount() * 3);
		dt.setSize(molecule.getAtomCount() * 3);
		dIvdw.setSize(molecule.getAtomCount() * 3);
		...
		for (int i = 0; i < gradientMMFF94SumEvdW_InWishedCoordinates.getSize(); i++) {
			...
			depsilonSK.setElement(i,1);                 // depsilonSK : partial derivative of epsilonSK. To change in the future
			...
			for (int l = 0; l < vdwInteractionNumber; l++) {
				...
				sumGradientEvdW = sumGradientEvdW + (depsilonSK.getElement(i) * term1[l] * term2[l] + epsilonSK[l] * (dterm1.getElement(i) * term2[l] + term1[l] * dterm2.getElement(i))) * s[l] * t[l] * ivdw[l] + 
								(epsilonSK[l] * term1[l] * term2[l]) * (ds.getElement(i) * t[l] * ivdw[l] + s[l] * (dt.getElement(i) * ivdw[l] + t[l] * dIvdw.getElement(i)));
			}
