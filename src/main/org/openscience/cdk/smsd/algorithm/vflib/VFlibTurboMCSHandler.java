...
public class VFlibTurboMCSHandler extends AbstractMCSAlgorithm implements IMCSBase {
    ...
    private final static ILoggingTool logger =
            LoggingToolFactory.createLoggingTool(VFlibTurboMCSHandler.class);

    /**
     * Constructor for an extended VF Algorithm for the MCS search
     */
    @TestMethod("setMCSAlgorithm")
    public VFlibTurboMCSHandler() {
    ...
    private void searchVFMCSMappings() {
        ...
        if (ac1.getAtomCount() <= ac2.getAtomCount()) {
            ...
            vfLibSolutions = new HashMap<INode, IAtom>(mapper.getFirstMap(ac2));
            RONP = true;

        } else {
            query = TemplateCompiler.compile(ac2);
            mapper = new VFMCSMapper(query);
            vfLibSolutions = new HashMap<INode, IAtom>(mapper.getFirstMap(ac1));
            RONP = false;
        }
