			...
			do {			
				if (startBond == atomContainer.getBondCount() ) {					
					if (bestGuess[1] == 0)
						...
						double satAtoms = ((bestGuess[1]*1.0)/atomContainer
								.getAtomCount())*10000;
						satAtoms = Math.round(satAtoms)/100;
//						System.out.println("Staring on bond "+bestGuess[0]+
//								" gives "+satAtoms+"% saturated atoms.");
						logger.warn("Can't find any solution where all atoms "+
								"are saturated. A best guess gives "+satAtoms+
								"% Saturated atoms.");
						return;
					}
				}
	...
	public boolean bondOrderCanBeIncreased(IBond bond, 
		...
		if (atom0isUnsaturated == atom1isUnsaturated)
			...
			if (myIndex == 0)
				return false;
			/* If the previous bond is the reason it's no problem, 
			 * so just move on...*/
	...
	public double getMaxNoOfBonds(IAtom atom) throws CDKException {
		double noValenceElectrons = atom.getValency() == CDKConstants.UNSET ? -1 
				: atom.getValency();
