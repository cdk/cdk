...
public abstract class DescriptorTest<T extends IDescriptor> extends CDKTestCase {
    ...
    public void testGetParameterNames() throws Exception {
        ...
        for (String paramName : paramNames) {
            ...
            Assert.assertNotSame(
                    "A parameter name String must not be empty.", 0, paramName.length());
        }
    }
    ...
    public void testGetParameters() {
        ...
        if (params == null) {
            Assert.assertEquals(
                    ...
                    0,
                    descriptor.getParameterNames() == null
                            ? 0
                            : descriptor.getParameterNames().length);
            params = new Object[0];
        }
        ...
        for (int i = 0; i < paramNames.length; i++) {
            ...
            Assert.assertEquals(
                    "The getParameterType(String) return type is not consistent "
                            + "with the getParameters() types for parameter "
                            + i,
    ...
    /**
     * Tests that the specification no longer gives an empty CVS identifier, but one based on a
     * repository blob or commit.
     */
    @Test
