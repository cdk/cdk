...
public class LigancyFourChiralityTest extends CDKTestCase {
    ...
    public void testConstructor_ILigancyFourChirality() {
        ...
        ITetrahedralChirality cdkChiral =
        ...
        for (int i = 0; i < ligands.length; i++) {
            Assert.assertEquals(
                    ligands[i].getLigandAtom(), chirality.getLigands()[i].getLigandAtom());
            Assert.assertEquals(
                    ligands[i].getCentralAtom(), chirality.getLigands()[i].getCentralAtom());
            Assert.assertEquals(
                    ligands[i].getAtomContainer(), chirality.getLigands()[i].getAtomContainer());
        }
        Assert.assertEquals(Stereo.CLOCKWISE, chirality.getStereo());
    }
