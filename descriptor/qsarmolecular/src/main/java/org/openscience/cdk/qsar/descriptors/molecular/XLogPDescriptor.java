...
/**
 * Prediction of logP based on the atom-type method called XLogP. <b>Requires all hydrogens to be
 * explicit</b>.
 *
 ...
 * As mentioned in the xlogP tutorial don't use charges, always draw bonds. To some extend we can
 * support charges but not in every case.
 *
 ...
 * -Atomtype C 34/35/36 perception corrected [2005-11-22]; before Atomtype perception ring
 * perception is done -&gt; slows run time<br>
 *
 * <p>In question:<br>
 * -Correction factor for salicylic acid (in paper, but not used by the program)<br>
 * -Amid classification is not consequent (in 6 rings (R2)N-C(R)=0 is eg 46 and in !6 membered rings
 * it is amid)<br>
...
public class XLogPDescriptor extends AbstractMolecularDescriptor implements IMolecularDescriptor {
     ...
     *
     * <p>If checkAromaticity is true, the method check the aromaticity, if false, means that the
     * aromaticity has already been checked. It is necessary to use before the call of this mehtod
     * the addExplicitHydrogensToSatisfyValency method (HydrogenAdder classe).
     *
    ...
    public DescriptorValue calculate(IAtomContainer atomContainer) {
            ...
            if (atomRingSet.getAtomContainerCount() > 0) {
                if (atomRingSet.getAtomContainerCount() > 1) {
                    Iterator<IAtomContainer> containers =
                            RingSetManipulator.getAllAtomContainers(atomRingSet).iterator();
                ...
                for (int j = 0; j < atomRingSet.getAtomContainerCount(); j++) {
                    if (j == 0) {
                        atomi.setProperty(
                                CDKConstants.PART_OF_RING_OF_SIZE,
                    ...
                    if (((IRing) atomRingSet.getAtomContainer(j)).contains(atomi)) {
                        ...
                        if (((IRing) atomRingSet.getAtomContainer(j)).getRingSize()
                                < (Integer) atomi.getProperty(CDKConstants.PART_OF_RING_OF_SIZE)) {
                            atomi.setProperty(
                                    CDKConstants.PART_OF_RING_OF_SIZE,
                                    ((IRing) atomRingSet.getAtomContainer(j)).getRingSize());
                        }
        ...
        for (int i = 0; i < atomCount; i++) {
            ...
            if (symbol.equals("C")) {
                ...
                if (bondCount == 3) {
                    ...
                    if ((Boolean) atomi.getProperty("IS_IN_AROMATIC_RING")) {
                        if (getAromaticCarbonsCount(ac, atomi) >= 2
                                && getAromaticNitrogensCount(ac, atomi) == 0) {
                            if (hsCount == 0) {
            ...
            if (symbol.equals("N")) {
                // NO2
                if (ac.getBondOrderSum(atomi) >= 3.0
                        && getOxygenCount(ac, atomi) >= 2
            ...
            if (symbol.equals("O")) {
                if (bondCount == 1 && maxBondOrder == IBond.Order.DOUBLE) {
                    ...
                    // logger.debug("XLOGP: 75	A=O	-0.399");
                                ...
                                && hsCount == 0
                                && (getPresenceOfNitro(ac, atomi)
                                        || getPresenceOfCarbonil(ac, atomi) == 1)
        ...
        for (int i = 0; i < hBondAcceptors.size(); i++) {
            for (int j = 0; j < hBondDonors.size(); j++) {
                if (checkRingLink(rs, ac, ac.getAtom(hBondAcceptors.get(i)))
                    ...
                    int dist =
                            apsp.from(ac.getAtom(hBondAcceptors.get(i)))
                                    .distanceTo(ac.getAtom(hBondDonors.get(j)));
                    //					//logger.debug("
                    ...
                    if (checkRingLink(rs, ac, ac.getAtom(hBondAcceptors.get(i)))
                            && checkRingLink(rs, ac, ac.getAtom(hBondDonors.get(j).intValue()))) {
                        if (dist == 3
                                && pairCheck[hBondAcceptors.get(i)][hBondDonors.get(j)] == 0) {
                            xlogP += 0.429;
        ...
        if (checkAminoAcid > 1) {
            ...
            while (bonds.hasNext()) {
                ...
                if ((bondAtom0.getAtomicNumber() == IElement.C
                                && bondAtom1.getAtomicNumber() == IElement.N)
                        || (bondAtom0.getAtomicNumber() == IElement.N
                                        && bondAtom1.getAtomicNumber() == IElement.C)
                                && bond.getOrder() == IBond.Order.SINGLE) {
                    aminoAcid.removeBond(bondAtom0, bondAtom1);
                    QueryBond qbond =
                            new QueryBond(bondAtom0, bondAtom1, Expr.Type.SINGLE_OR_AROMATIC);
            ...
            try {
                if (universalIsomorphismTester.isSubgraph(ac, aminoAcid)) {
                    ...
                    for (int j = 0; j < list.size(); j++) {
                        ...
                        if (atom1.getAtomicNumber() == IElement.O
                                && ac.getMaximumBondOrder(atom1) == IBond.Order.SINGLE) {
                            if (ac.getConnectedBondsCount(atom1) == 2
                                    && getHydrogenCount(ac, atom1) == 0) {
                            } else {
                                xlogP -= 2.166;
    ...
    private int getHalogenCount(IAtomContainer ac, IAtom atom) {
        ...
        int acounter = 0;
        for (IAtom neighbour : neighbours) {
            if (neighbour.getAtomicNumber() == IElement.F
                    || neighbour.getAtomicNumber() == IElement.I
                    || neighbour.getAtomicNumber() == IElement.Cl
                    || neighbour.getAtomicNumber() == IElement.Br) {
                acounter += 1;
            }
        }
        return acounter;
    ...
    private int getAtomTypeXCount(IAtomContainer ac, IAtom atom) {
        ...
        for (IAtom neighbour : neighbours) {
            if ((neighbour.getAtomicNumber() == IElement.N
                            || neighbour.getAtomicNumber() == IElement.O)
    ...
    private int getAromaticCarbonsCount(IAtomContainer ac, IAtom atom) {
        ...
        for (IAtom neighbour : neighbours) {
            if (neighbour.getAtomicNumber() == IElement.C
                    && neighbour.getFlag(CDKConstants.ISAROMATIC)) {
                carocounter += 1;
            }
        }
        return carocounter;
        ...
        for (int i = 0; i < neighbours.size(); i++) {
            ...
            for (int j = 0; j < bonds.size(); j++) {
                ...
                if (bond.getOrder() != IBond.Order.SINGLE
                        && !bond.getOther(neighbour).equals(atom)
                        && neighbour.getAtomicNumber() != IElement.P
                        && neighbour.getAtomicNumber() != IElement.S) {
                    picounter += 1;
                } /*
            ...
            for (int i = 0; i < first.size(); i++) {
                ...
                if (conAtom.getAtomicNumber() == IElement.O) {
                    if (ac.getBond(neighbour0, conAtom).getOrder() == IBond.Order.SINGLE) {
                        if (ac.getConnectedBondsCount(conAtom) > 1
                                && getHydrogenCount(ac, conAtom) == 0) {
                            return false;
                        } else {
                            return true;
                        }
    ...
    /**
     * Gets the ifCarbonIsHydrophobic attribute of the XLogPDescriptor object. C must be sp2 or sp3
     * and, for all distances C-1-2-3 only C atoms are permitted
     *
     * @param ac Description of the Parameter
     * @param atom Description of the Parameter
    ...
    private boolean getIfCarbonIsHydrophobic(IAtomContainer ac, IAtom atom) {
        ...
        if (first.size() > 0) {
            for (int i = 0; i < first.size(); i++) {
                ...
                if (firstAtom.getAtomicNumber() == IElement.C
                        || firstAtom.getAtomicNumber() == IElement.H) {
                } else {
                    return false;
                }
                ...
                if (second.size() > 0) {
                    for (int b = 0; b < second.size(); b++) {
                        ...
                        if (secondAtom.getAtomicNumber() == IElement.C
                                || secondAtom.getAtomicNumber() == IElement.H) {
                        } else {
                            return false;
                        }
                        ...
                        if (third.size() > 0) {
                            for (int c = 0; c < third.size(); c++) {
                                ...
                                if (thirdAtom.getAtomicNumber() == IElement.C
                                        || thirdAtom.getAtomicNumber() == IElement.H) {
                                } else {
                                    return false;
                                }
                                ...
                                //		if (fourth[d].getAtomicNumber() == IElement.C ||
                                // fourth[d].getAtomicNumber() == IElement.H) {
                                //		} else {
                                //			return false;
                                //		}
                                //	}
