...
/**
 * A logP model donated by Lhasa Limited. It is based on an atom contribution model. See {@cdk.cite
 * Plante2018}.
 *
...
public class JPlogPDescriptor extends AbstractMolecularDescriptor implements IMolecularDescriptor {
    ...
    public DescriptorSpecification getSpecification() {
        return new DescriptorSpecification(
                "JPlogP developed at Lhasa Limited www.lhasalimited.org",
                this.getClass().getName(),
                "Jeffrey Plante - Lhasa Limited");
    }
    ...
    public Object getParameterType(String name) {
        if ("addImplicitH".equals(name)) return true;
        return null;
    }
    ...
    public DescriptorValue calculate(IAtomContainer container) {
        ...
        return new DescriptorValue(
                ...
                getParameterNames(),
                getParameters(),
                new DoubleResult(jplogp.calcLogP(struct)),
                getDescriptorNames());
    }

    /**
     * The class that calculated the logP according to the JPlogP method described in: Journal of
     * Cheminformatics 2018 10:61 https://doi.org/10.1186/s13321-018-0316-5
     *
     * <p>This is lower level access and should normally be obtained through the descriptor above.
     *
     * @author Jeffrey
     */
    protected class JPlogPCalculator {
        ...
        /**
         * Given a structure in the correct configuration (explicit H and aromatised) it will return
         * the logP as a Double or if it is out of domain (encounters an unknown atomtype) it will
         * return Double.NaN
         *
        ...
        /**
         * Returns the AtomCode for the logP atomtype as previously developed at Lhasa see citation
         * at top of class for more information
         *
         * @param atom the atom to type
         * @return Integer of the type CAANSS c = charge+1 AA = Atomic Number N = NonH Neighbour
         *     count SS = elemental subselection via bonding and neighbour identification
         */
        ...
        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for a Hydrogen
         * Atom
         *
         * @param atom
        ...
        protected int getHydrogenSpecial(IAtom atom) {
            ...
            if (bondCount > 0) {
                ...
                if (neighbour.getAtomicNumber() == IElement.C) {
                    if (carbonylConjugated(neighbour)) toadd = 51;
                    else {
                        double formalOxState = getNumMoreElectronegativethanCarbon(neighbour);
                        switch (numNeighbours) {
                            case 4:
                                if (formalOxState == 0.0) toadd = 46;
                                else if (formalOxState == 1.0) toadd = 47;
                                else if (formalOxState == 2.0) toadd = 48;
                                else if (formalOxState == 3.0) toadd = 49;
                                break;
                            case 3:
                                if (formalOxState == 0.0) toadd = 47;
                                else if (formalOxState == 1.0) toadd = 48;
                                else if (formalOxState >= 2.0) toadd = 49;
                                break;
                            case 2:
                                if (formalOxState == 0.0) toadd = 48;
                                else if (formalOxState >= 1.0) toadd = 49;
                                break;
                            case 1:
                                toadd = 121;
                                break;
        ...
        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for a
         * "Default" ie not C,N,O,H,F Atom
         *
         * @param atom
         * @return the final 2 digits for the given atom
         */
        protected int getDefaultSpecial(IAtom atom) {
            ...
            if (atom.isAromatic()) toadd = 10;
            else toadd = singleBondPolar + doubleBondPolar + tripleBondPolar + aromaticBondPolar;
            return toadd;
        }

        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for a Fluorine
         * Atom
         *
         * @param atom
         * @return the final 2 digits for the given atom
        ...
        protected int getFluorineSpecial(IAtom atom) {
            ...
            if (numconn == 1) {
                ...
                if (next.getSymbol().matches("S")) toadd = 8; // F-S
                else if (next.getSymbol().matches("B")) toadd = 9; // F-B
                else if (!next.getSymbol().matches("C")) toadd = 1; // F-hetero
                else if (neighbourconn == 2) toadd = 2;
                else if (neighbourconn == 3) toadd = 3;
                else if (neighbourconn == 4 && ox <= 2) toadd = 5;
                else if (neighbourconn == 4 && ox > 2) toadd = 7;
                else toadd = 99;
            } else toadd = 99;
            return toadd;
        }
        ...
        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for an Oxygen
         * Atom
         *
         * @param atom
         * @return the final 2 digits for the given atom
        ...
        protected int getOxygenSpecial(IAtom atom) {
            ...
            switch (numConnections) {
                case 2:
                    if (boundTo(atom, "N")) toadd = 1; // A-O-N
                    else if (boundTo(atom, "S")) toadd = 2; // A-O-S
                    else if (atom.isAromatic()) toadd = 8; // A-=O-=A
                    else toadd = 3; // A-O-A
                    break;
                case 1:
                    if (boundTo(atom, "N")) toadd = 4; // O=N
                    else if (boundTo(atom, "S")) toadd = 5; // O=S
                    else if (checkAlphaCarbonyl(atom, "O")) toadd = 6; // O=A-O
                    else if (checkAlphaCarbonyl(atom, "N")) toadd = 9; // O=A-N
                    else if (checkAlphaCarbonyl(atom, "S")) toadd = 10; // O=A-S
                    else toadd = 7; // O=A
                    break;
                default:
                    toadd = 0;
        ...
        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for a Nitrogen
         * Atom
         *
         * @param atom
         * @return the final 2 digits for the given atom
        ...
        protected int getNitrogenSpecial(IAtom atom) {
            ...
            switch (numConnections) {
                case 4:
                    ...
                    break;
                case 3:
                    if (nextToAromatic(atom)) toadd = 1; // A-N(-A)-Aromatic Atom
                    else if (carbonylConjugated(atom)) toadd = 2; // N-A=Polar
                    else if (doubleBondHetero(atom))
                        toadd = 10; // A-(A-)N={ONS} probably also charged
                    else if (singleBondPolar > 0) toadd = 3; // A-N(-A)-Polar
                    else toadd = 4; // A-N(-A)-A
                    break;
                case 2:
                    if (atom.isAromatic()) toadd = 5; // A-=N-=A
                    else if (doubleBondHetero(atom)) toadd = 6; // A-N=Polar
                    else toadd = 7; // A-N=A
                    break;
                case 1:
                    toadd = 8; // N%A
        ...
        /**
         * Determines and returns the SS (subsection) portion of the atomtype integer for a Carbon
         * Atom
         *
         * @param atom
         * @return the final 2 digits for the given atom
        ...
        protected int getCarbonSpecial(IAtom atom) {
            ...
            switch (numConnections) {
                case 4: // 4 connections
                    toadd = 2; // sp3 C
                    if (singleBondPolar > 0) toadd = 3; // sp3 carbon next to S,N,P,O
                    break;
                case 3: // 3 connections
                    if (atom.isAromatic()) {
                        ...
                        if (doubleBondPolar == 1 && singleBondPolar == 0)
                            ...
                            toadd = 14; // A-C(=P)-P
                        else toadd = 6; // A-C(=A)-A
                    }
                    break;
                case 2: // 2 connections
                    if (tripleBondPolar == 1 && singleBondPolar == 0) toadd = 12; // A-C%Polar
                    else if (tripleBondPolar == 0 && singleBondPolar == 1) toadd = 10; // Polar-C%A
                    else if (tripleBondPolar == 1 && singleBondPolar == 1) toadd = 15; // p-C%P
                    else toadd = 9; // A-C%A
                    break;
                default:
                    toadd = 0; // C???
                    if (singleBondPolar > 0
                            || doubleBondPolar > 0
                            || aromaticBondPolar > 0
                            || tripleBondPolar > 0) toadd = 1; // C??Polar
            }
            return toadd;
        }
        ...
        protected boolean boundTo(IAtom atom, String symbol) {
            for (IBond bond : atom.bonds()) {
                IAtom next = bond.getOther(atom);
                if (next.getSymbol().equals(symbol)) return true;
            }
            return false;
        }
        ...
        /**
         * @param atom
         * @return bond order for electron withdrawing atoms from the given atom ie =O = 2
         */
        protected double getNumMoreElectronegativethanCarbon(IAtom atom) {
            ...
            for (IBond bond : atom.bonds()) {
                ...
                if (electronWithdrawing(compare)) returnme += bondOrder;
            }
            return returnme;
        }
        ...
        /**
         * @param atom
         * @return true if the atom is considered electron withdrawing relative to carbon
         *     (N,O,S,F,Cl,Br,I)
         */
        ...
        protected int nonHNeighbours(IAtom atom) {
            ...
            for (IBond bond : atom.bonds()) {
                ...
                if (neighbor.getAtomicNumber() != 1) returnMe++;
            }
            return returnMe;
        }
        ...
        /**
         * @param atom
         * @return in array of bond information to polar atoms array form is [single, aromatic,
         *     double, triple]
         */
        protected int[] getPolarBondArray(IAtom atom) {
            ...
            for (IBond bond : atom.bonds()) {
                IAtom neighbor = bond.getOther(atom);
                if (isPolar(neighbor)) {
                    if (bond.isAromatic()) array[1]++;
                    else if (bond.getOrder().numeric() == 1) array[0]++;
                    else if (bond.getOrder().numeric() == 2) array[2]++;
                    else if (bond.getOrder().numeric() == 3) array[3]++;
                }
            }
            return array;
        }
        ...
        protected boolean isPolar(IAtom atom) {
            if (atom.getSymbol().matches("O")
                    || atom.getSymbol().matches("S")
                    || atom.getSymbol().matches("N")
        ...
        protected boolean carbonylConjugated(IAtom atom) {
            for (IBond bond : atom.bonds()) {
                ...
                if (!(bond.isAromatic())
                        ...
                        && doubleBondHetero(next)) return true;
            }
            return false;
        }
        ...
        protected boolean nextToAromatic(IAtom atom) {
            if (!atom.isAromatic()) {
                for (IBond bond : atom.bonds()) {
                    IAtom next = bond.getOther(atom);
                    if (next.isAromatic() && bond.getOrder().numeric() == 1) return true;
                }
            }
            return false;
        }
        ...
        /**
         * initialise the model with the required values. Could instead read from a serialised file,
         * but this is simpler and the number of coefficients isn't too large. Quite simple to
         * update as well when able to output the model to the screen with minor text manupilation
         * with regex strings.
         */
