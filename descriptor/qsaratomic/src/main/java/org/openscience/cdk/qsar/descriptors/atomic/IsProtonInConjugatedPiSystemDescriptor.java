 ...
 */
public class IsProtonInConjugatedPiSystemDescriptor extends AbstractAtomicDescriptor
     ...
     *
     * @param params Parameters are an integer (heavy atom position) and a boolean (true if is
     *     needed a checkAromaticity)
    ...
    public void setParameters(Object[] params) throws CDKException {
        if (params.length > 1) {
            throw new CDKException(
                    "IsProtonInConjugatedPiSystemDescriptor only expects one parameters");
        }
    ...
    /**
     * The method is a proton descriptor that evaluates if a proton is joined to a conjugated
     * system.
     *
    ...
    public DescriptorValue calculate(IAtom atom, IAtomContainer atomContainer) {
        ...
        IAtomContainer mol =
                clonedAtom.getBuilder().newInstance(IAtomContainer.class, clonedAtomContainer);
        if (checkAromaticity) {
            try {
