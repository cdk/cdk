...
/**
 * Generates an extended fingerprint for a given {@link IAtomContainer}, that extends the {@link
 * Fingerprinter} with additional (25) bits describing ring features and isotopic masses.
 *
 * <p><i>JWM Comment: It's better to actually just hash the rings over the entire length simply
 * using a different seed. The original version of the class used non-unique SSSR which of course
 * doesn't work for substructure screening so this fingerprint can only be used for similarity.</i>
 *
 * @author shk3
...
public class ExtendedFingerprinter implements IFingerprinter {
    ...
    /**
     * Generates a fingerprint of the default size for the given AtomContainer, using path and ring
     * metrics. It contains the informations from getBitFingerprint() and bits which tell if the
     * structure has 0 rings, 1 or less rings, 2 or less rings ... 10 or less rings (referring to
     * smallest set of smallest rings) and bits which tell if there is a fused ring system with
     * 1,2...8 or more rings in it
     *
     ...
     * Generates a fingerprint of the default size for the given AtomContainer, using path and ring
     * metrics. It contains the informations from getBitFingerprint() and bits which tell if the
     * structure has 0 rings, 1 or less rings, 2 or less rings ... 10 or less rings and bits which
     * tell if there is a fused ring system with 1,2...8 or more rings in it. The RingSet used is
     * passed via rs parameter. This must be a smallesSetOfSmallestRings. The List must be a list of
     * all ring systems in the molecule.
     *
     ...
     *     getExtendedFingerprint(AtomContainer ac), which does the calculation)
    ...
    public IBitFingerprint getBitFingerprint(
            IAtomContainer atomContainer, IRingSet ringSet, List<IRingSet> rslist)
            throws CDKException {
    ...
    /**
     * Set the pathLimit for the base daylight/path fingerprint. If too many paths are generated
     * from a single atom an exception is thrown.
     *
    ...
    /**
     * Set the hashPseudoAtoms for the base daylight/path fingerprint. This indicates whether
     * pseudo-atoms should be hashed, for substructure screening this is not desirable - but this
     * fingerprint uses SSSR so can't be used for substructure screening regardless.
     *
