 ...
 *
 * <p>This class implements a fast descriptor based 3D similarity measure described by Ballester et
 * al ({@cdk.cite BALL2007}). The approach calculates the distances of each atom to four specific
 * points: the centroid of the molecule, the atom that is closest to the centroid, the atom that is
 * farthest from the centroid and the atom that is farthest from the previous atom. Thus we get 4
 * sets of distance distributions. The final descriptor set is generated by evaluating the first
 * three moments of each distance distribution.
 *
 * <p>The similarity between two molecules is then evaluated using the inverse of a normalized
 * Manhattan type metric.
 *
 * <p>This class allows you to evaluate the 3D similarity between two specified molecules as well as
 ...
 *
 * <p><b>Note</b>: The methods of this class do not perform hydrogen removal. If you want to do the
 * calculations excluding hydrogens, you'll need to do it yourself. Also, if the molecule has
...
public class DistanceMoment {
     ...
     *
     * <p>The method does not remove hydrogens. If this is required, remove them from the molecules
