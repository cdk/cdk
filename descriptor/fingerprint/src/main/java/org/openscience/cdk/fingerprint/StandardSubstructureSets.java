...
public class StandardSubstructureSets {
    ...
    /**
     * Subset of the MACCS fingerprint definitions. The subset encompasses the pattern that are
     * countable:
     *
     * <ul>
     *   <li>Patterns have obvious counting nature, <i>e.g., 6-Ring, C=O, etc.</i>
     *   <li>Patterns like <i>"Is there at least 1 of this and that?", "Are there at least 2
     *       ..."</i> etc. are merged
     *   <li>Patterns clearly corresponding to binary properties, <i>e.g., actinide group
     *       ([Ac,Th,Pa,...]), isotope, etc.,</i> have been removed.
     * </ul>
     *
    ...
    public static String[] getCountableMACCSSMARTS() throws Exception {
        return readSMARTSPattern(
                "org/openscience/cdk/fingerprint/data/SMARTS_countable_MACCS_keys.txt");
    }

    /**
     * Load a list of SMARTS patterns from the specified file.
     *
     * <p>Each line in the file corresponds to a pattern with the following structure:
     * PATTERN_DESCRIPTION: SMARTS_PATTERN, <i>e.g., Thioketone: [#6][CX3](=[SX1])[#6]</i>
     *
     * <p>Empty lines and lines starting with a "#" are skipped.
     *
    ...
    private static String[] readSMARTSPattern(String filename) throws Exception {
        InputStream ins =
                StandardSubstructureSets.class.getClassLoader().getResourceAsStream(filename);
