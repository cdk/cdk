...
{	
	...
	private int initialIdentityFCFP(int aidx)
	{
		return (maskDon[aidx] ? 0x01 : 0)|
			   ...
			   (maskPos[aidx] ? 0x04 : 0)|
			   (maskNeg[aidx] ? 0x08 : 0)|
			   (atomArom[aidx] ? 0x10 : 0)|  // strictly bond aromaticity more accurate rendition
			   (maskHal[aidx] ? 0x20 : 0);
	}
	...
	private int circularIterate(int iter,int atom)
	{
		...
		while (p<adj.length-1)
		{
			...
			{
				int sw=seq[i]; seq[i]=seq[i+2]; seq[i+2]=sw;
				sw=seq[i+1]; seq[i+1]=seq[i+3]; seq[i+3]=sw;
				if (p>0) p--;
			}
			else p++;
		}
...
//wr("CONSIDER:"+newFP.iteration+",hash="+newFP.hashCode); //foo
	...
	private void excavateMolecule()
	{
		...
		for (int n=0;n<na;n++) amask[n]=mol.getAtom(n).getAtomicNumber()>1; // true for heavy elements
		atomAdj=new int[na][];
		bondAdj=new int[na][];
		...
		for (int n=0;n<na;n++) if (amask[n] && atomAdj[n]==null) {atomAdj[n]=new int[0]; bondAdj[n]=atomAdj[n];}
		...
		for (int rsz=3;rsz<=7;rsz++)
		{
			...
			for (int n=0;n<na;n++) if (ringBlock[n]>0)
			{
				path[0]=n;
	...
	private void markRingBlocks()
	{
        		...
        		for (int n=0;n<atomAdj[current].length;n++) 
        		{
        		    ...
        		    if (join!=back && visited[join])
        		    {
            			...
            			for (int i=plen;i==plen || path[i+1]!=join;i--)
            			{
            			    ...
            			    {
	            				for (int j=0;j<na;j++) if (ringBlock[j]==id) ringBlock[j]=last;
            			    }
    	    			}
        		    }
        		}
    	...
    	{
    	    nextID--;
    	    for (int j=na-1;j>=i;j--) if (ringBlock[j]==ringBlock[i]) ringBlock[j]=nextID;
    	}
	...
	private void detectStrictAromaticity()
	{
		...
		for (int[] r : smallRings) if (r.length==6)
		{
			boolean consider=true;
			for (int n=0;n<6;n++)
			{
				final int a=r[n];
				if (!piAtom[a]) {consider=false; break;}
				int b=findBond(a,r[n==5 ? 0 : n+1]);
				if (bondOrder[b]!=1 && bondOrder[b]!=2) {consider=false; break;}
			}
    ...
    private int[] rubricTetrahedral(int aidx)
    {
		...
		for (int n=0;n<adjc;n++)
		{
			...
			if (stereo==IBond.Stereo.UP || stereo==IBond.Stereo.DOWN) {wedgeOr3D=true; break;}
	...
	private void calculateBioTypes()
	{
		...
		for (int[] r : smallRings) if (r.length>=5 && r.length<=7)
		{
	...
	private void considerBioTypeAromaticity(final int[] ring)
	{
		...
		{
			final int a=ring[n];
			if (hasDouble[a]) {countDouble++; continue;}
	...
	private void considerBioTypeTetrazole(final int[] ring)
	{
		...
		for (int n=0;n<5;n++)
		{
			...
			String el=atom.getSymbol();
			if (el.equals("C")) countC++; else if (el.equals("N")) countN++;
			if (bondOrder[findBond(ring[n],ring[n==4 ? 0 : n+1])]==2) ndbl++;
		}
		if (countC!=1 || countN!=4 || ndbl!=2) return;
		for (int n=0;n<5;n++) if (mol.getAtom(ring[n]).getSymbol().equals("N")) tetrazole[ring[n]]=true;
	}
	...
	private boolean determineDonor(int aidx)
	{
		...
		else if (el.equals("S"))
		{
			...
			for (int n=0;n<atomAdj[aidx].length;n++) if (hasDouble[atomAdj[aidx][n]]) return false;
			return true;
		}
		else if (el.equals("C"))
		{
    ...
    private boolean determinePositive(int aidx)
    {
    		...
    		if (hasDouble[aidx] && hcount[aidx]==0)
    		{
    			...
    			if (other>=0)
    			{
        			...
        			for (int n=0;n<atomAdj[other].length;n++) 
        			{
        				...
        				if (a==aidx) continue;
        				if (hasDouble[a]) {amines=0; break;}
        				final String ael=mol.getAtom(a).getSymbol();
        				if (ael.equals("N"))
        				{
        					if (hcount[a]>0) {amines=0; break;}
        					amines++;
        				}
        				else if (!ael.equals("C")) {amines=0; break;}
        			}
    	...
    	else if (el.equals("C"))
    	{
    		...
    		for (int n=0;n<atomAdj[aidx].length;n++)
    		{
    			...
    			if (tetrazole[a]) {imine=false; amine=false; break;}
    ...
    private boolean determineNegative(int aidx)
    {
    	...
    	{
    		for (int n=0;n<atomAdj[aidx].length;n++) if (bondOrderBioType(bondAdj[aidx][n])==1)
    		{
    			final int a=atomAdj[aidx][n];
	...
	private String arrayStr(int[] val)
	{
		...
		String str=String.valueOf(val[0]);
		for (int n=1;n<val.length;n++) str+=","+val[n];
		return "{"+str+"}";
	}
	private String arrayStr(float[] val)
	{
		if (val==null) return "null";
		if (val.length==0) return "{}";
		String str=String.valueOf(val[0]);
	...
	private String arrayStr(boolean[] val)
	{
		if (val==null) return "null";
		if (val.length==0) return "{}";
		String str="";
