...
public class ReactionRenderer extends AbstractRenderer<IReaction> implements IRenderer<IReaction> {
    ...
    /**
     * A renderer that draws {@link IReaction}s with the passed {@link IGenerator} for both {@link
     * IAtomContainer} and {@link IReaction} content.
     *
     * @param generators a list of classes that implement the {@link IGenerator} interface
     * @param reactionGenerators a list of {@link IGenerator} related to {@link IReaction}s
     * @param fontManager a class that manages mappings between zoom and font sizes
     */
    public ReactionRenderer(
            List<IGenerator<IAtomContainer>> generators,
    ...
    /**
     * Set the scale for an IReaction. It calculates the average bond length of the model and
    ...
    @Override
    public void paint(
            IReaction reaction, IDrawVisitor drawVisitor, Rectangle2D bounds, boolean resetCenter) {
        ...
        this.setupTransformToFit(
                bounds,
                modelBounds,
                AverageBondLengthCalculator.calculateAverageBondLength(reaction),
                resetCenter);
