...
public class MurckoFragmenterTest extends CDKTestCase {
    ...
    public void testMF2() throws Exception {
        ...
        assertThat(
                frameworks,
                hasItems(
                        "c1ccc(cc1)CCC2CCC(CC3C=CC=C3)C2",
                        "c1ccc(cc1)CCC2CCCC2",
                        "C=1C=CC(C1)CC2CCCC2",
    ...
    public void testMF4() throws Exception {
        ...
        assertThat(
                frameworks,
                hasItems("n1coc(c1)-c2ccccc2", "n1coc(-c2ccccc2)c1-c3ccccc3", "n1cocc1-c2ccccc2"));
    }
    ...
    public void testMF7() throws Exception {
        IAtomContainer mol =
                smilesParser.parseSmiles("Cc1nnc(s1)N[C@H](C(=O)c2ccccc2)NC(=O)c3ccco3");
        fragmenter.generateFragments(mol);
    ...
    public void testBug1848591() throws Exception {
        IAtomContainer mol =
                smilesParser.parseSmiles("c1(ccc(cc1C)CCC(C(CCC)C2C(C2)CC)C3C=C(C=C3)CC)C");
        MurckoFragmenter fragmenter = new MurckoFragmenter(true, 6);
        fragmenter.generateFragments(mol);
    ...
    public void testCarbinoxamine_Bug3088164() throws Exception {
        ...
        for (int i = 0; i < f.length; i++) {
            ...
            Assert.assertTrue(
                    f[i] + " did not match the container, " + newsmiles, f[i].equals(newsmiles));
        }
    }
    ...
    public void testPirenperone_Bug3088164() throws Exception {
        ...
        IAtomContainer mol =
                smilesParser.parseSmiles(
                        "Fc1ccc(cc1)C(=O)C4CCN(CCC\\3=C(\\N=C2\\C=C/C=C\\N2C/3=O)C)CC4");
