...
/**
 * A permutation group with a Schreier-Sims representation. For a number n, a list of permutation
 * sets is stored (U0,...,Un-1). All n! permutations of [0...n-1] can be reconstructed from this
 * list by backtracking - see, for example, the <a href="#generateAll">generateAll</a> method.
 *
 * <p>So if G is a group on X = {0, 1, 2, 3, ..., n-1}, then:
 *
 * <blockquote>
 *
 ...
 * </blockquote>
 *
 * and G<sub>0</sub>, G<sub>1</sub>, G<sub>2</sub>, ..., G<sub>n-1</sub> are subgroups of G.
 *
 * <p>Now let orb(0) = {g(0) : g &isin; G} be the orbit of 0 under G. Then |orb(0)| (the size of the
 * orbit) is n<sub>0</sub> for some integer 0 &lt; n<sub>0</sub> &le; n and write orb(0) =
 * {x<sub>0,1</sub>, x<sub>0,2</sub>, ..., x<sub>0,n₀</sub>} and for each i, 1 &le; i &le;
 * n<sub>0</sub> choose some h<sub>0,1</sub> in G such that h<sub>0,i</sub>(0) = x<sub>0,1</sub>.
 * Set U<sub>0</sub> = {h<sub>0,1</sub>, ..., h<sub>0,n₀</sub>}.
 *
 * <p>Given the above, the list of permutation sets in this class is
 * [U<sub>0</sub>,..,U<sub>n</sub>]. Also, for all i = 1, ..., n-1 the set U<sub>i</sub> is a left
 * transversal of G<sub>i</sub> in G<sub>i-1</sub>.
 *
 * <p>This is port of the code from the C.A.G.E.S. book {@cdk.cite Kreher98}. The mathematics in the
 * description above is also from that book (pp. 203).
 *
 * @author maclean
 * @cdk.module group
 */
public class PermutationGroup {
    ...
    /**
     * An interface for use with the apply method, which runs through all the permutations in this
     * group.
     */
    ...
    /**
     * Creates a group from a set of generators. See the makeSymN method for where this is used to
     * make the symmetric group on N using the two generators (0, 1) and (1, 2, ..., n - 1, 0)
     *
    ...
    /**
     * Make the symmetric group Sym(N) for N. That is, a group of permutations that represents _all_
    ...
    public List<Permutation> transversal(final PermutationGroup subgroup) {
        ...
        Backtracker transversalBacktracker =
                new Backtracker() {
    ...
    /**
     * Test a permutation to see if it is in the group. Note that this also alters the permutation
     * passed in.
     *
