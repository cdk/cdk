...
public class StructureDiagramGeneratorTest extends CDKTestCase {
    ...
    public void testBugLecture2007() throws Exception {
        ...
        // IAtomContainer mol =
        // sp.parseSmiles("Oc1nc(nc2c(nn(c12)C)CCC)c3cc(ccc3(OCC))S(=O)(=O)N4CCN(C)CC4");
    ...
    public void testBug1670871() throws Exception {
        ...
        IAtomContainer mol =
                sp.parseSmiles(
                        "CC(=O)OC1C=CC(SC23CC4CC(CC(C4)C2)C3)N(C1SC56CC7CC(CC(C7)C5)C6)C(C)=O");
        IAtomContainer ac = layout(mol);
        // MoleculeViewer2D.display(new AtomContainer(ac), false);
        assertTrue(GeometryUtil.has2DCoordinates(ac));
    }
    ...
    public void testBug1598409() throws Exception {
        String smiles =
                "c1(:c(:c2-C(-c3:c(-C(=O)-c:2:c(:c:1-[H])-[H]):c(:c(:c(:c:3-[H])-[H])-N(-[H])-[H])-[H])=O)-[H])-[H]";
    ...
    public void testBug884993() throws Exception {
        ...
        IAtomContainer mol =
                sp.parseSmiles(
                        "[N+](=O)([O-])C1=C(O)C(=CC(=C1)[N+](=O)[O-])[N+](=O)[O-].C23N(CCCC2)CCCC3");
        IAtomContainer ac = layout(mol);
        assertTrue(GeometryUtil.has2DCoordinates(ac));
    }

    /**
     * Test for bug #1677912 "SDG JUnit test hangs" The SMILES parsing takes hence a longer timeout.
     *
     * @cdk.bug 1677912
     */
    @Test(timeout = 10000)
    public void testBug1677912SDGHangs() throws Exception {
        // Parse the SMILES
        String smiles =
                "[NH](-[CH]1-[CH]2-[CH2]-[CH]3-[CH2]-[CH]-1-[CH2]-[CH](-[CH2]-2)-[CH2]-3)-C(=O)-C(=O)-[CH2]-c1:n:c(:c(:[cH]:c:1-C(=O)-O-[CH3])-C(=O)-O-[CH3])-[CH2]-C(=O)-C(=O)-[NH]-[CH]1-[CH]2-[CH2]-[CH]3-[CH2]-[CH]-1-[CH2]-[CH](-[CH2]-2)-[CH2]-3";
    ...
    public void testBug1714794() throws Exception {
        String problematicMol2AsSmiles =
                "N1c2c(c3c(c4c(c(c3O)C)OC(OC=CC(C(C(C(C(C(C(C(C=CC=C(C1=O)C)C)O)C)O)C)OC(=O)C)C)OC)(C4=O)C)c(c2C=NN(C12CC3CC(C1)CC(C2)C3)C)O)O";
        SmilesParser parser = new SmilesParser(SilentChemObjectBuilder.getInstance());
        ...
        IChemModel model =
                (IChemModel)
                        r.read(SilentChemObjectBuilder.getInstance().newInstance(IChemModel.class));
     ...
     * ring were selected for placing. Thus, those atoms later lacked 2D coordinates (they were
     * null) and the RingPlacer crashed with a NullPointerException such as:
     *
     * <p>java.lang.NullPointerException at javax.vecmath.Tuple2d.<init>(Tuple2d.java:66) at
     * javax.vecmath.Vector2d.<init>(Vector2d.java:74) at
     * org.openscience.cdk.layout.RingPlacer.placeFusedRing(RingPlacer.java:379) at
     * org.openscience.cdk.layout.RingPlacer.placeRing(RingPlacer.java:99) at
     * org.openscience.cdk.layout.RingPlacer.placeConnectedRings(RingPlacer.java:663) at
     * org.openscience.cdk.layout.StructureDiagramGenerator.layoutRingSet(StructureDiagramGenerator.java:516)
     * at
     * org.openscience.cdk.layout.StructureDiagramGenerator.generateCoordinates(StructureDiagramGenerator.java:379)
     * at
     * org.openscience.cdk.layout.StructureDiagramGenerator.generateCoordinates(StructureDiagramGenerator.java:445)
     *
    ...
    /**
     * Test for bug #1784850 "SDG hangs in infinite loop". Fixed by correcting the safteyCounter
     * check.
     *
     * <p>Author: Andreas Schueller <a.schueller@chemie.uni-frankfurt.de>
     *
    ...
    /**
     * For the SMILES compound below (the largest molecule in Chembl) a handful of atoms had invalid
     * (NaN) Double coordinates.
     *
    ...
    public void testBug1279() throws Exception {
        ...
        layout(mol);
        for (IAtom atom : mol.atoms()) assertNotNull(atom.getPoint2d());
    }

    @Test
    public void alleneWithImplHDoesNotCauseNPE() throws Exception {
    ...
    public void ionicBondsInAlCl3() throws Exception {
        ...
        for (IAtom atom : mol.atoms()) assertNotNull(atom.getPoint2d());
        assertThat(
                mol.getAtom(0).getPoint2d().distance(mol.getAtom(1).getPoint2d()),
                closeTo(1.5 * SDG.getBondLength(), 0.001));
        assertThat(
                mol.getAtom(0).getPoint2d().distance(mol.getAtom(2).getPoint2d()),
                closeTo(1.5 * SDG.getBondLength(), 0.001));
        assertThat(
                mol.getAtom(0).getPoint2d().distance(mol.getAtom(3).getPoint2d()),
                closeTo(1.5 * SDG.getBondLength(), 0.001));
    }
    ...
    public void ionicBondsInK2CO3() throws Exception {
        ...
        assertThat(
                ...
                closeTo(1.5 * SDG.getBondLength(), 0.001));
        assertThat(
                mol.getAtom(4).getPoint2d().distance(mol.getAtom(5).getPoint2d()),
                closeTo(1.5 * SDG.getBondLength(), 0.001));
    }
    ...
    public void chembl12276() throws Exception {
        SmilesParser sp = new SmilesParser(SilentChemObjectBuilder.getInstance());
        IAtomContainer mol =
                sp.parseSmiles("[Cl-].C(C1=CC=CC2=C(C=CC=C12)[N+](=O)[O-])[N+](C)(CCCl)CCCl");
        layout(mol);
        for (IAtom atom : mol.atoms()) assertNotNull(atom.getPoint2d());
        assertThat(mol.getAtom(0).getAtomicNumber(), is(17));
        assertThat(mol.getAtom(15).getAtomicNumber(), is(7));
        assertThat(
                mol.getAtom(0).getPoint2d().distance(mol.getAtom(15).getPoint2d()),
                closeTo(1.5 * SDG.getBondLength(), 0.001));
    }
    ...
    public void multipleSalts() throws Exception {
        ...
        IAtomContainer mol =
                sp.parseSmiles(
                        "[K+].[Al+3].[Cl-].[Cl-].[K+].[Cl-].[Cl-].[Al+3].[Cl-].[Pt+2]([NH3])[NH3].[Cl-].[Cl-].[Cl-].[O-][C+]([O-])[O-]");
        layout(mol);
        for (IAtom atom : mol.atoms()) assertNotNull(atom.getPoint2d());
        ...
        for (IAtom atom : mol.atoms()) {
            if (atom.getSymbol().equals("Cl")) chlorines.add(atom);
            else if (atom.getSymbol().equals("O")) oxygens.add(atom);
            else if (atom.getSymbol().equals("Al")) aluminiums.add(atom);
            else if (atom.getSymbol().equals("K")) potassiums.add(atom);
            else if (atom.getSymbol().equals("Pt")) platinum = atom;
        }
        ...
        for (IAtom chlorine : chlorines) {
            double delta =
                    chlorine.getPoint2d().distance(platinum.getPoint2d())
                            - 1.5 * SDG.getBondLength();
            if (Math.abs(delta) < 0.01) ptFound++;
        }
        ...
        for (IAtom potassium : potassiums) {
            ...
            for (IAtom oxygen : oxygens) {
                double delta =
                        oxygen.getPoint2d().distance(potassium.getPoint2d())
                                - 1.5 * SDG.getBondLength();
                if (Math.abs(delta) < 0.01) kFound++;
            }
        ...
        for (IAtom aluminium : aluminiums) {
            ...
            for (IAtom chlorine : chlorines) {
                double delta =
                        chlorine.getPoint2d().distance(aluminium.getPoint2d())
                                - 1.5 * SDG.getBondLength();
                if (Math.abs(delta) < 0.01) clFound++;
            }
    ...
    @Test
    public void placeCrossingSgroupBrackets() throws Exception {
    ...
    @Test
    public void placeNonCrossingSgroupBrackets() throws Exception {
        IAtomContainer mol = new org.openscience.cdk.silent.AtomContainer();
        mol.addAtom(new org.openscience.cdk.silent.Atom("C"));
        mol.addAtom(new org.openscience.cdk.silent.Atom("C"));
        mol.addAtom(new org.openscience.cdk.silent.Atom("C"));
    ...
    public void positionalVariation() throws Exception {
        ...
        int numCrossing = 0;
        for (int i = 0; i < 6; i++) {
            if (isCrossing(mol.getBond(i), mol.getBond(10))) numCrossing++;
            if (isCrossing(mol.getBond(i), mol.getBond(11))) numCrossing++;
        }
    }
    ...
    public void disconnectedMultigroupPlacement() throws Exception {
        ...
        for (IAtom atom : mol.atoms()) {
            patoms.add(atom);
            if (patoms.size() == 6) break;
        }
        sgroup.putValue(SgroupKey.CtabParentAtomList, patoms);
        mol.setProperty(CDKConstants.CTAB_SGROUPS, Collections.singletonList(sgroup));
        layout(mol);
        for (int i = 0; i < 6; i++) {
            assertEquals(mol.getAtom(i).getPoint2d(), mol.getAtom(i + 6).getPoint2d(), 0.01);
            assertEquals(mol.getAtom(i).getPoint2d(), mol.getAtom(i + 12).getPoint2d(), 0.01);
        }
    }

    /**
     * These molecules are laid out 'H2N=NH2.H2N=NH2', ensure we give them more space than usual
     * (bond length)
     */
    @Test
    public void dihydroazine() throws Exception {
        ...
        layout(mol);
        assertThat(
                mol.getAtom(2).getPoint2d().x - mol.getAtom(1).getPoint2d().x,
    ...
    public void NH4OH() throws Exception {
        ...
        layout(mol);
        assertThat(
                mol.getAtom(1).getPoint2d().x - mol.getAtom(0).getPoint2d().x,
                is(greaterThan(SDG.getBondLength())));
    }
    ...
    public void fragmentDoubleBondConfiguration() throws Exception {
        ...
        for (IStereoElement se : elements) {
            if (se instanceof IDoubleBondStereochemistry) {
                IDoubleBondStereochemistry.Conformation config =
                        ((IDoubleBondStereochemistry) se).getStereo();
                if (config == IDoubleBondStereochemistry.Conformation.TOGETHER) numCis++;
                else if (config == IDoubleBondStereochemistry.Conformation.OPPOSITE) numTrans++;
            }
        }
    ...
    /**
     * Reaction from US20050272744A1 [0195], a bond is broken and made. The reaction layout should
     * not crash.
     */
    @Test
    public void alignReactionBondBrokenAndMade() throws CDKException {
        String smiles =
                "[CH3:18][NH:19][CH3:20].[cH:14]1[cH:13][cH:12][c:11]([cH:16][cH:15]1)[CH2:10][O:9][C:1](=[O:17])[NH:2][C@H:3]2[CH2:8][C:6](=[O:7])[O:5][CH2:4]2>C1CCOC1>[CH3:18][N:19]([CH3:20])[C:6](=[O:7])[CH2:8][C@H:3]([CH2:4][OH:5])[NH:2][C:1](=[O:17])[O:9][CH2:10][c:11]1[cH:12][cH:13][cH:14][cH:15][cH:16]1";
        SmilesParser smipar = new SmilesParser(SilentChemObjectBuilder.getInstance());
    ...
    public void alignReactionBondBrokenAndMade2() throws CDKException {
        String smiles =
                "[CH2:2]1[CH2:3][CH:4]2[CH2:5][CH2:6][CH:1]1[O:7]2.[IH:8]>>[OH:7][C@H:1]1[CH2:6][CH2:5][C@H:4]([I:8])[CH2:3][CH2:2]1";
        SmilesParser smipar = new SmilesParser(SilentChemObjectBuilder.getInstance());
        IReaction reaction = smipar.parseReactionSmiles(smiles);
        StructureDiagramGenerator sdg = new StructureDiagramGenerator();
        sdg.setAlignMappedReaction(true);
