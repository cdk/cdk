...
/**
 * Class providing methods for generating coordinates for ring atoms. Various situations are
 * supported, like condensation, spiro-attachment, etc. They can be used for Automated Structure
 * Diagram Generation or in the interactive buildup of ringsystems by the user.
 *
 * @cdk.module sdg
 * @cdk.githash
 */
public class RingPlacer {
    ...
    private void placeBridgedRing(
        ...
        for (IAtom atom : sharedAtoms.atoms())
            if (!atom.equals(bondAtom1) && !atom.equals(bondAtom2)) otherAtoms.add(atom);

        final boolean snap =
                ring.getProperty(SNAP_HINT) != null && ring.getProperty(SNAP_HINT, Boolean.class);

        boolean swap =
                snap
                        ? det(
                                        bondAtom1.getPoint2d(),
                                        GeometryUtil.get2DCenter(otherAtoms),
                                        bondAtom2.getPoint2d())
                                < 0
                        : det(
                                        bondAtom1.getPoint2d(),
                                        GeometryUtil.get2DCenter(otherAtoms),
        ...
        if (snap) {
            ...
            ringCenterVector =
                    getPerpendicular(
                            ...
                            bondAtom2Vector,
                            new Vector2d(
                                    midPoint.x - sharedAtomsCenter.x,
                                    midPoint.y - sharedAtomsCenter.y));

            offset = 0;
            for (IAtom atom : otherAtoms) {
                double dist = atom.getPoint2d().distance(midPoint);
                if (dist > offset) offset = dist;
            }
        } else {
        ...
        startAngle =
                GeometryUtil.getAngle(
                        bondAtom1.getPoint2d().x - ringCenter.x,
                        bondAtom1.getPoint2d().y - ringCenter.y);
    ...
    public void placeSpiroRing(
        ...
        if (degree != 4) {
            ...
            for (IBond bond : mBonds) {
                ...
                if (!nbr.getFlag(CDKConstants.ISPLACED)) continue;
                numPlaced++;
            }
        ...
        double startAngle =
                GeometryUtil.getAngle(
                        startAtom.getPoint2d().x - ringCenter.x,
        ...
        for (int i = 0; i < ring.getBondCount(); i++) {
            currentBond = ring.getNextBond(currentBond, currentAtom);
            currentAtom = currentBond.getOther(currentAtom);
            if (!currentAtom.equals(startAtom)) atomsToDraw.addElement(currentAtom);
        }
    ...
    /**
     * Generated coordinates for a given ring, which is fused to another ring. The rings share
     * exactly one bond.
     *
     * @param ring The ring to be placed
     * @param sharedAtoms The atoms of this ring, also members of another ring, which are already
    ...
    public void placeFusedRing(
            IRing ring, IAtomContainer sharedAtoms, Vector2d ringCenterVector, double bondLength) {
    ...
    boolean completePartiallyPlacedRing(IRingSet rset, IRing ring, double bondLength) {
        if (ring.getFlag(CDKConstants.ISPLACED)) return true;
        IRing partiallyPlacedRing = molecule.getBuilder().newInstance(IRing.class);
        for (IAtom atom : ring.atoms())
            if (atom.getPoint2d() != null) atom.setFlag(CDKConstants.ISPLACED, true);
    ...
    /**
     * Gat a vector perpendicular to the line, a-b, that is pointing the same direction as 'ref'.
     *
    ...
    /**
     * Returns the bridge atoms, that is the outermost atoms in the chain of more than two atoms
     * which are shared by two rings
     *
    ...
    Vector2d getRingCenterOfFirstRing(IRing ring, Vector2d bondVector, double bondLength) {
        ...
        rotangle += Math.PI / 2;
        return new Vector2d(
                Math.cos(rotangle) * newRingPerpendicular,
    ...
    void placeConnectedRings(IRingSet rs, IRing ring, int handleType, double bondLength) {
        ...
        for (IAtomContainer container : connectedRings.atomContainers()) {
            ...
            if (!connectedRing.getFlag(CDKConstants.ISPLACED)) {
                ...
                final IAtomContainer sharedAtoms =
                        AtomContainerManipulator.getIntersection(ring, connectedRing);
                ...
                if ((numSharedAtoms == 2 && handleType == FUSED)
                    ...
                    if (Math.abs(newRingCenterVector.x) < 0.001
                            && Math.abs(newRingCenterVector.y) < 0.001) {

                        // first see if we can use terminal bonds
                        IAtomContainer terminalOnly =
                                molecule.getBuilder().newInstance(IAtomContainer.class);

                        for (IAtom atom : ring.atoms()) {
                            if (ring.getConnectedBondsCount(atom) == 1) terminalOnly.addAtom(atom);
                        }
                        ...
                        if (terminalOnly.getAtomCount() == 2) {
                            ...
                            // project coordinates on 12 axis (30 degree snaps) and choose one with
                            // most spread
    ...
    /**
     * Sorts ring systems prioritising the most complex. To sort correctly, {@link
     * #countHetero(List)} must first be invoked.
     */
    static final Comparator<IRingSet> RING_COMPARATOR =
            new Comparator<IRingSet>() {
                @Override
                public int compare(IRingSet a, IRingSet b) {
                    ...
                    int cmp =
                            Boolean.compare(
                                    a.getAtomContainerCount() == 1, b.getAtomContainerCount() == 1);
                    if (cmp != 0) return cmp;
    ...
    /**
     * Counds the number of hetero atoms and hetero rings in a ringset. The properties {@code
     * sdg:numHeteroRings} and {@code sdg:numHeteroAtoms} are set.
     *
        ...
        for (IRingSet rset : rsets) {
            ...
            for (IAtomContainer ring : rset.atomContainers()) {
                ...
                for (IAtom atom : ring.atoms()) {
                    ...
                    if (elem != null && elem != 6 && elem != 1) numHeteroAtoms++;
                }
                if (numHeteroAtoms > prevNumHeteroAtoms) numHeteroRings++;
            }
