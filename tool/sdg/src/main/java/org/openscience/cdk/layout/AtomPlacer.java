...
/**
 * Methods for generating coordinates for atoms in various situations. They can be used for
 * Automated Structure Diagram Generation or in the interactive buildup of molecules by the user.
 *
 * @author steinbeck
...
public class AtomPlacer {
    ...
    /**
     * Distribute the bonded atoms (neighbours) of an atom such that they fill the remaining space
     * around an atom in a geometrically nice way. IMPORTANT: This method is not supposed to handle
     * the case of one or no place neighbor. In the case of one placed neigbor, the chain placement
     ...
     */
    public void distributePartners(
            IAtom atom,
        ...
        if (Math.abs(occupiedDirection.length()) < 0.001) occupiedDirection = new Vector2d(0, 1);
        logger.debug(
                "distributePartners->occupiedDirection.lenght(): " + occupiedDirection.length());
        ...
        if (placedNeighbours.getAtomCount() == 1) {
            ...
            addAngle =
                    Math.PI
                            * 2
                            / (unplacedNeighbours.getAtomCount() + placedNeighbours.getAtomCount());
            /*
            ...
            populatePolygonCorners(
                    atomsToDraw, new Point2d(atom.getPoint2d()), startAngle, addAngle, bondLength);
            return;
        ...
        if (doAngleSnap(atom, placedNeighbours)) {
            ...
            for (IAtom unplaced : unplacedNeighbours.atoms())
                if (molecule.getConnectedBondsCount(unplaced) == 1) numTerminal++;

            if (numTerminal == unplacedNeighbours.getAtomCount()) {
                final Vector2d a =
                        newVector(placedNeighbours.getAtom(0).getPoint2d(), atom.getPoint2d());
                final Vector2d b =
                        newVector(placedNeighbours.getAtom(1).getPoint2d(), atom.getPoint2d());
                ...
                populatePolygonCorners(
                        FluentIterable.from(unplacedNeighbours.atoms()).toList(),
                        atom.getPoint2d(),
                        startAngle,
                        sweep,
                        bondLength);
        ...
        double angle1 =
                GeometryUtil.getAngle(
                        sortedAtoms[0].getPoint2d().x - atom.getPoint2d().x,
                        sortedAtoms[0].getPoint2d().y - atom.getPoint2d().y);
        double angle2 =
                GeometryUtil.getAngle(
                        sortedAtoms[1].getPoint2d().x - atom.getPoint2d().x,
                        sortedAtoms[1].getPoint2d().y - atom.getPoint2d().y);
        double angle3 =
                GeometryUtil.getAngle(
                        distanceMeasure.x - atom.getPoint2d().x,
                        distanceMeasure.y - atom.getPoint2d().y);
        if (debug) {
            try {
                logger.debug(
                        "distributePartners->sortedAtoms[0]: ",
                        (molecule.indexOf(sortedAtoms[0]) + 1));
                logger.debug(
                        "distributePartners->sortedAtoms[1]: ",
                        (molecule.indexOf(sortedAtoms[1]) + 1));
        ...
        if (debug) {
            try {
                ...
                logger.debug(
                        "distributePartners->remainingAngle: " + Math.toDegrees(remainingAngle));
                logger.debug("distributePartners->addAngle: " + Math.toDegrees(addAngle));
                logger.debug(
                        "distributePartners-> partners.getAtomCount(): "
                                + unplacedNeighbours.getAtomCount());
            } catch (Exception exc) {
                logger.debug(exc);
            }
        }
        ...
        startAngle =
                GeometryUtil.getAngle(
                        startAtom.getPoint2d().x - atom.getPoint2d().x,
                        startAtom.getPoint2d().y - atom.getPoint2d().y);
     ...
     *
     * <p>Expects the first atom to be placed and places the next atom according to
     * initialBondVector. The rest of the chain is placed such that it is as linear as possible (in
     * the overall result, the angles in the chain are set to 120 Deg.)
     *
     ...
     */
    public void placeLinearChain(
            IAtomContainer atomContainer, Vector2d initialBondVector, double bondLength) {
        IAtomContainer withh =
        ...
        //            }catch(Exception ex){
        //                logger.warn("Exception in hydrogen adding. This could mean that cleanup
        // does not respect E/Z: ", ex.getMessage());
        //                logger.debug(ex);
        //            }
        ...
        for (int f = 0; f < atomContainer.getAtomCount() - 1; f++) {
            ...
            if (GeometryUtil.has2DCoordinates(atomContainer)) {
                try {
                    if (f > 2
                            && BondTools.isValidDoubleBondConfiguration(
                                    withh,
                                    withh.getBond(withh.getAtom(f - 2), withh.getAtom(f - 1)))) {
                        trans =
                                BondTools.isCisTrans(
                                        withh.getAtom(f - 3),
                                        withh.getAtom(f - 2),
                                        withh.getAtom(f - 1),
                                        withh.getAtom(f - 0),
                    ...
                    logger.debug(
                            "Excpetion in detecting E/Z. This could mean that cleanup does not respect E/Z");
                }
                bondVector =
                        getNextBondVector(
                                nextAtom, atom, GeometryUtil.get2DCenter(molecule), trans);
            } else {
                bondVector =
                        getNextBondVector(nextAtom, atom, GeometryUtil.get2DCenter(molecule), true);
            }
    ...
    /**
     * Returns the next bond vector needed for drawing an extended linear chain of atoms. It assumes
     * an angle of 120 deg for a nice chain layout and calculates the two possible placments for the
     * next atom. It returns the vector pointing farmost away from a given start atom.
     *
     ...
     */
    public Vector2d getNextBondVector(
            IAtom atom, IAtom previousAtom, Point2d distanceMeasure, boolean trans) {
        ...
        double angle =
                GeometryUtil.getAngle(
                        previousAtom.getPoint2d().x - atom.getPoint2d().x,
                        previousAtom.getPoint2d().y - atom.getPoint2d().y);
        double addAngle;

        if (isTerminalD4(atom)) addAngle = Math.toRadians(45);
        else if (isColinear(atom, bonds)) addAngle = Math.toRadians(180);
        else if (Elements.isMetal(atom)) addAngle = (2 * Math.PI) / bonds.size();
        else {
    ...
    /**
     * Populates the corners of a polygon with atoms. Used to place atoms in a geometrically regular
     * way around a ring center or another atom. If this is used to place the bonding partner of an
     * atom (and not to draw a ring) we want to place the atoms such that those with highest
     * "weight" are placed furthermost away from the rest of the molecules. The "weight" mentioned
     * here is calculated by a modified morgan number algorithm.
     *
     * @param atoms All the atoms to draw
     * @param thetaBeg A start angle (in radians), giving the angle of the most clockwise atom which
     *     has already been placed
     * @param thetaStep An angle (in radians) to be added for each atom from atomsToDraw
     * @param center The center of a ring, or an atom for which the partners are to be placed
     * @param radius The radius of the polygon to be populated: bond length or ring radius
     */
    public void populatePolygonCorners(
            final List<IAtom> atoms,
        ...
        logger.debug(
                "populatePolygonCorners(numAtoms=",
                numAtoms,
                ", center=",
                center,
                ", thetaBeg=",
                Math.toDegrees(thetaBeg),
                ", r=",
                radius);
    ...
    /**
     * Partition the bonding partners of a given atom into placed (coordinates assinged) and not
     * placed.
     *
    ...
    /**
     * Search an aliphatic molecule for the longest chain. This is the method to be used if there
     * are no rings in the molecule and you want to layout the longest chain in the molecule as a
     * starting point of the structure diagram generation.
     *
     * @param molecule The molecule to be search for the longest unplaced chain
     * @return An AtomContainer holding the longest chain.
     * @exception org.openscience.cdk.exception.NoSuchAtomException Description of the Exception
     */
    public static IAtomContainer getInitialLongestChain(IAtomContainer molecule)
        ...
        logger.debug(
                "Longest chaing in molecule is of length "
                        + maxPathLength
                        + " between atoms "
                        + (bestStartAtom + 1)
                        + " and "
    ...
    /**
     * Search a molecule for the longest unplaced, aliphatic chain in it. If an aliphatic chain
     * encounters an unplaced ring atom, the ring atom is also appended to allow for it to be laid
     * out. This gives us a vector for attaching the unplaced ring later.
     *
     * @param molecule The molecule to be search for the longest unplaced chain
     ...
     * @exception org.openscience.cdk.exception.CDKException Description of the Exception
     */
     ...
     * usually consists of one start atom, and searches for the longest aliphatic chain which is yet
     * unplaced. If the search encounters an unplaced ring atom, it is also appended to the chain so
     * that this last bond of the chain can also be laid out. This gives us the orientation for the
            ...
            if (!atom.getFlag(CDKConstants.ISINRING)) {
                ...
                for (int g = 0; g < bonds.size(); g++) {
                    ...
                    if (!nextAtom.getFlag(CDKConstants.VISITED)
                        ...
                        pathes[nextAtomNr] =
                                ac.getBuilder().newInstance(IAtomContainer.class, pathes[atomNr]);
                        logger.debug(
                                "Making copy of path "
                                        + (atomNr + 1)
                                        + " to form new path "
                        ...
                        logger.debug(
                                "Adding atom " + (nextAtomNr + 1) + " to path " + (nextAtomNr + 1));
    ...
    /**
     * Returns a string with the numbers of all atoms in an AtomContainer relative to a given
     * molecule. I.e. the number the is listesd is the position of each atom in the molecule.
     *
     * @param ac The AtomContainer for which the placed atoms are to be listed
    ...
    /**
     * Returns a string with the numbers of all atoms in a Vector relative to a given molecule. I.e.
     * the number the is listesd is the position of each atom in the molecule.
     *
    ...
    /**
     * Prioritise atoms of a molecule base on how 'buried' they are. The priority is cacheted with a
     * morgan-like relaxation O(n^2 lg n). Priorities are assign from 1..|V| (usually less than |V|
     * due to symmetry) where the lowest numbers have priority.
     *
     * @param mol molecule
     * @param adjList fast adjacency lookup
     * @return the priority
    ...
    static int[] getPriority(IAtomContainer mol, int[][] adjList) {
        ...
        for (int rep = 0; rep < n; rep++) {
            for (int i = 0; i < n; i++) {
                rank[i] = 3 * prev[i];
                for (int w : adjList[i]) rank[i] += prev[w];
            }
            ...
            prev[order[0]] = clsNum;
            for (int i = 1; i < n; i++) {
                if (rank[order[i]] != rank[order[i - 1]]) clsNum++;
                prev[order[i]] = clsNum;
            }
        ...
        for (int i = 0; i < n; i++) prev[i] = 1 + nDistinct - prev[i];
    ...
    static boolean isColinear(IAtom atom, Iterable<IBond> bonds) {
        ...
        switch (atom.getAtomicNumber()) {
            ...
            case 14:
            case 32:
                if (numTpl == 1 && numSgl == 1) return true;
                if (numDbl == 2 && numSgl == 0) return true;
                break;
        }
