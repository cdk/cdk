...
/**
 * Assigns non-planar labels (wedge/hatch) to the tetrahedral and extended tetrahedral stereocentres
 * in a 2D depiction. Labels are assigned to atoms using the following priority.
 *
 * <ol>
 *   <li>bond to non-stereo atoms
 *   <li>acyclic bonds
 *   <li>bonds to atoms with lower degree (i.e. terminal)
 *   <li>lower atomic number
 * </ol>
 *
...
final class NonplanarBonds {
    ...
    NonplanarBonds(
            ...
            final int[][] g,
            final GraphUtil.EdgeToBondMap edgeToBond) {
        ...
        for (IStereoElement element : container.stereoElements()) {
            if (element instanceof ITetrahedralChirality) {
                ...
                doubleBondElements[atomToIndex.get(doubleBond.getBegin())] =
                        doubleBondElements[atomToIndex.get(doubleBond.getEnd())] =
                                (IDoubleBondStereochemistry) element;
            }
        }
    ...
    private boolean snapBondsToPosition(IAtom beg, List<IBond> bonds, double... angles) {
        ...
        if (angles.length != bonds.size()) throw new IllegalArgumentException();
        boolean res = true;
        for (int i = 0; i < bonds.size(); i++) {
            if (!snapBondToPosition(
                    beg, bonds.get(i), getRotated(ref, p, Math.toRadians(angles[i])))) res = false;
        }
        return res;
    }
    ...
    private void floodFill(Map<IAtom, Integer> visit, IAtom beg, int num) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (IBond bnd : atm.bonds()) {
                ...
                if (visit.get(nbr) == null) queue.add(nbr);
            }
        }
    }
    ...
    private void modifyAndLabel(SquarePlanar se) {
        ...
        for (IAtom atom : atoms) {
            ...
            if (bond.isInRing()) {
                if (!rmap.containsKey(atom)) floodFill(rmap, atom, ++rcount);
                rnums.add(rmap.get(atom));
            } else rnums.add(0);
            bonds.add(bond);
        }

        if (rcount > 0 && checkAndHandleRingSystems(bonds, rnums) == SPIRO_REJECT) return;

        for (IAtom atom : container.atoms()) atom.setFlag(CDKConstants.VISITED, false);
        for (IBond bond : container.bonds()) bond.setFlag(CDKConstants.VISITED, false);
    ...
    private boolean doMirror(List<IAtom> atoms) {
        ...
        for (int i = 0; i < atoms.size(); i++) {
            ...
            for (int j = i + 1; j < atoms.size(); j++) {
                IAtom b = atoms.get(j);
                if (a.getAtomicNumber() > b.getAtomicNumber()) p *= -1;
            }
        }
        return p < 0;
    }
    ...
    private void modifyAndLabel(TrigonalBipyramidal se) {
        ...
        if (rcount > 0 && (res = checkAndHandleRingSystems(bonds, rnums)) == SPIRO_REJECT) return;
    ...
    /**
     * This is complicated, we have a set of bonds and the rings they belong to. We move the bonds
     * around such that we can depict them in a nice way given the constraints of the geometry.
     *
    ...
    private int checkAndHandleRingSystems(List<IBond> bonds, List<Integer> rnums) {
        ...
        if (!isSpiro(rnums)) return SPIRO_REJECT;
        ...
        if (bonds.size() == 4) {
            ...
            // rotate such that there is a spiro (or no rings) in position 1/2 in the plane, these
            // are laid out
            // adjacent so is the only place we can nicely place the spiro
            int rotate;
            if (rnums.get(1).equals(rnums.get(2))) rotate = 0; // don't rotate
            else if (rnums.get(2).equals(rnums.get(3))) rotate = 1;
            else if (rnums.get(3).equals(rnums.get(0))) rotate = 2;
            else rotate = 0;
        ...
        if (bonds.size() == 5) {
            ...
            if (rnums.get(0) != 0 && rnums.get(0).equals(rnums.get(4))) return SPIRO_REJECT;
            ...
            if (rnums.get(1) != 0 && rnums.get(1).equals(rnums.get(3))
                ...
                rotate = 0; // don't rotate
                    ...
                    || rnums.get(3) == 0 && !rnums.get(2).equals(rnums.get(1))) rotate = 1;
            else if (rnums.get(3) != 0 && rnums.get(3).equals(rnums.get(2))
                    || rnums.get(1) == 0 && !rnums.get(3).equals(rnums.get(2))) rotate = 2;
            else rotate = 0;
            for (int i = 0; i < rotate; i++) {
                rotate(bonds, 1, 3);
                rotate(rnums, 1, 3);
        ...
        if (bonds.size() == 6) {
            ...
            if (rnums.get(0) != 0 && rnums.get(0).equals(rnums.get(5))
                    ...
                    || rnums.get(2) != 0 && rnums.get(2).equals(rnums.get(4))) return SPIRO_REJECT;

            // rotate such that there is a spiro (or no rings) in position 2/3 in the plane, these
            // are laid out
            // adjacent so is the only place we can nicely place the spiro
            int rotate;
            if (rnums.get(2).equals(rnums.get(3))) rotate = 0; // don't rotate
            else if (rnums.get(3).equals(rnums.get(4))) rotate = 1;
            else if (rnums.get(4).equals(rnums.get(1))) rotate = 2;
            else if (rnums.get(1).equals(rnums.get(2))) rotate = 3;
            else return SPIRO_REJECT;
    ...
    /**
     * Ensures out rings are only spiro by inspecting the ring set numbers. This is the case if we
     * have &ge; 3 of a given number in the list.
     *
     * <pre>
     *     [0,0,1,2,1,2] => yes
    ...
    private boolean isSpiro(List<Integer> rnums) {
        ...
        for (Integer rnum : rnums) {
            if (rnum != 0 && ++counts[rnum] > 2) return false;
        }
        return true;
    }
    ...
    private void setBondDisplay(IBond bond, IAtom focus, IBond.Stereo display) {
        if (bond.getBegin().equals(focus)) bond.setStereo(display);
        else bond.setStereo(flip(display));
    }
    ...
    /**
     * Sets a wedge bond, because wedges are relative we may need to flip the storage order on the
     * bond.
     *
     * @param bond the bond
    ...
    private void setWedge(IBond bond, IAtom end, IBond.Stereo style) {
        if (!bond.getEnd().equals(end)) bond.setAtoms(new IAtom[] {bond.getEnd(), bond.getBegin()});
        bond.setStereo(style);
    }

    /**
     * Assign non-planar labels (wedge/hatch) to the bonds of extended tetrahedral elements to
     * correctly represent its stereochemistry.
     *
    ...
    private void label(final ExtendedTetrahedral element) {
        ...
        for (int i = 0; i < 4; i++) bonds[i] = findBond(left, right, atoms[i]);

        // find the clockwise ordering (in the plane of the page) by sorting by
        // polar coordinates
        int[] rank = new int[4];
        for (int i = 0; i < 4; i++) rank[i] = i;
        ...
        if (priority[0] + priority[1] < priority[2] + priority[3]) {
            if (priority[0] < 5) setWedge(bonds[0], atoms[0], labels[0]);
            if (priority[1] < 5) setWedge(bonds[1], atoms[1], labels[1]);
        } else {
            if (priority[2] < 5) setWedge(bonds[2], atoms[2], labels[2]);
            if (priority[3] < 5) setWedge(bonds[3], atoms[3], labels[3]);
        }
    }

    /**
     * Assign non-planar labels (wedge/hatch) to the bonds to atropisomers
     *
     * @param element a extended tetrahedral element
     */
    private void label(final Atropisomeric element) {
        ...
        if (bonds[0] == null || bonds[1] == null || bonds[2] == null || bonds[3] == null)
            throw new IllegalStateException(
                    "Unexpected configuration ordering, beg/end bonds should be in that order.");
        ...
        phantom.setPoint2d(
                new Point2d(
                        (beg.getPoint2d().x + end.getPoint2d().x) / 2,
    ...
    /**
     * Assign labels to the bonds of tetrahedral element to correctly represent its stereo
     * configuration.
     *
    ...
    private void label(final ITetrahedralChirality element) {
        ...
        for (int v : priority(atomToIndex.get(focus), atoms, n)) {
            IBond bond = bonds[v];
            if (bond.getStereo() != NONE || bond.getOrder() != SINGLE) continue;
            // first label
            if (firstlabel == null) {
                ...
                if (!assignTwoLabels) break;
            }
            // second label
            else if (labels[v] != firstlabel) {
                // don't add if it's possibly a stereo-centre
                if (isSp3Carbon(atoms[v], graph[container.indexOf(atoms[v])].length)) break;
                bond.setAtoms(new IAtom[] {focus, atoms[v]}); // avoids UP_INVERTED/DOWN_INVERTED
                bond.setStereo(labels[v]);
                break;
            }
    ...
    private int countRingBonds(IBond[] bonds) {
        ...
        for (IBond bond : bonds) {
            if (bond != null && bond.isInRing()) rbonds++;
        }
        return rbonds;
    }

    /**
     * Obtain the parity of a value x. The parity is -1 if the value is odd or +1 if the value is
     * even.
     *
     * @param x a value
     * @return the parity
    ...
    /**
     * Obtain the parity (winding) of a tetrahedral element. The parity is -1 for clockwise (odd),
     * +1 for anticlockwise (even) and 0 for unspecified.
     *
    ...
    /**
     * Obtain a prioritised array where the indices 0 to n which correspond to the provided {@code
     * atoms}.
     *
    ...
    private int[] priority(int focus, IAtom[] atoms, int n) {
        ...
        for (int j = 1; j < n; j++) {
            ...
            while ((i >= 0)
                    && hasPriority(
                            focus, atomToIndex.get(atoms[v]), atomToIndex.get(atoms[rank[i]]))) {
                rank[i + 1] = rank[i--];
            }
            rank[i + 1] = v;
        }
            ...
            for (IBond bond : container.getConnectedBondsList(atom)) {
                ...
                if (container.getConnectedBondsCount(nbr) == 1) {
                    for (IAtom terminal : terminals) {
                        if (Objects.equals(terminal.getAtomicNumber(), nbr.getAtomicNumber())
                                && Objects.equals(terminal.getMassNumber(), nbr.getMassNumber())
                                && Objects.equals(terminal.getFormalCharge(), nbr.getFormalCharge())
                                && Objects.equals(
                                        terminal.getImplicitHydrogenCount(),
    ...
    /**
     * Does the atom at index {@code i} have priority over the atom at index {@code j} for the
     * tetrahedral atom {@code focus}.
     *
    ...
    boolean hasPriority(int focus, int i, int j) {
        ...
        if (iIsSp3 != jIsSp3) return !iIsSp3;
        ...
        if (iAtom.getAtomicNumber() > 0 && jAtom.getAtomicNumber() == 0) return true;
        if (iAtom.getAtomicNumber() == 0 && jAtom.getAtomicNumber() > 0) return false;
        ...
        if (iDegree == 1 && jDegree > 1) return true;
        if (jDegree == 1 && iDegree > 1) return false;

        // prioritise by atomic number, H < N < O < ... < C
        if (iElem < jElem) return true;
        if (iElem > jElem) return false;
    ...
    /**
     * Labels a double bond as unspecified either by marking an adjacent bond as wavy (up/down) or
     * if that's not possible (e.g. it's conjugated with other double bonds that have a
     * conformation), setting the bond to a crossed double bond.
     *
    ...
    private void labelUnspecified(IBond doubleBond) {
        ...
        for (int i = 0; i < nAdj; i++) {
            if (doubleBondElements[atomToIndex.get(adj[rank[i]])] == null
                ...
                edgeToBond
                        .get(atomToIndex.get(focus[rank[i]]), atomToIndex.get(adj[rank[i]]))
                        .setStereo(UP_OR_DOWN);
                return;
            }
        }
     ...
     * @see <a href="http://www.inchi-trust.org/download/104/InChI_TechMan.pdf">Double bond
     *     stereochemistry, InChI Technical Manual</a>
     */
    private boolean isCisTransEndPoint(int idx) {
        ...
        if (atom.getAtomicNumber() == null
                ...
                || atom.getImplicitHydrogenCount() == null) return false;
    ...
    /**
     * Generate a bond type code for a given atom. The bond code can be quickly tested to count the
     * number of single, double, or 'other' bonds.
     *
    ...
    private int getBondTypes(int idx) {
        ...
        for (int end : graph[idx]) {
            ...
            if (bond.getOrder() == SINGLE) btypes += 0x00_0001;
            else if (bond.getOrder() == DOUBLE) btypes += 0x00_0100;
    ...
    private List<IBond> findUnspecifiedDoubleBonds(int[][] adjList) {
        ...
        for (IBond bond : container.bonds()) {
            ...
            if (bond.getOrder() != DOUBLE) continue;
            ...
            if (ringSearch.cyclic(beg, end)) continue;

            // stereo bond, ignore it depiction is correct
            if ((doubleBondElements[beg] != null
                            && doubleBondElements[beg].getStereoBond().equals(bond))
                    || (doubleBondElements[end] != null
                            && doubleBondElements[end].getStereoBond().equals(bond))) continue;

            // is actually a tetrahedral centre
            if (tetrahedralElements[beg] != null || tetrahedralElements[end] != null) continue;

            if (!isCisTransEndPoint(beg) || !isCisTransEndPoint(end)) continue;

            if (!hasOnlyPlainBonds(beg, bond) || !hasOnlyPlainBonds(end, bond)) continue;
    ...
    private boolean hasLinearEqualPaths(int[][] adjList, int start, int prev) {
        ...
        if (aAtom.isInRing() || bAtom.isInRing()) return false;
        IAtom aNext = aAtom;
        IAtom bNext = bAtom;
        while (aNext != null && bNext != null) {
            ...
            // different atoms
            if (notEqual(aAtom.getAtomicNumber(), bAtom.getAtomicNumber())) return false;
            if (notEqual(aAtom.getFormalCharge(), bAtom.getFormalCharge())) return false;
            if (notEqual(aAtom.getMassNumber(), bAtom.getMassNumber())) return false;
            ...
            if (hCntA != hCntB) return false;

            // differing in co
            if (cntA != cntB || (cntA > 1 && cntB > 1)) return false;
        }

        if (aNext != null || bNext != null) return false;
    ...
    /**
     * Check that an atom (v:index) is only adjacent to plain single bonds (may be a bold or hashed
     * wedged - e.g. at fat end) with the single exception being the allowed double bond passed as
     * an argument.
     *
     * @param v atom index
    ...
    private boolean hasOnlyPlainBonds(int v, IBond allowedDoubleBond) {
        ...
        for (int neighbor : graph[v]) {
            ...
            // single bonds
                ...
                if (adjBond.getStereo() == UP_OR_DOWN
                        || adjBond.getStereo() == UP_OR_DOWN_INVERTED) {
                    return false;
                }
                count++;
            }
