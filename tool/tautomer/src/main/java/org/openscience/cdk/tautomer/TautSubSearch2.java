...
public final class TautSubSearch2 extends Pattern {
    ...
    private TautStore base;
    private List<TautStore> cache = new ArrayList<>();
    private Tautomers.Role[] roles;

    public TautSubSearch2(IChemObjectBuilder builder, String smarts) {
    ...
    private boolean matches(IAtomContainer mol, int idx) {
        ...
        switch (op.type) {
            ...
            case TautCheck:
                ...
                if (cache == null) {
                    cache = new ArrayList<>();
                    for (IAtomContainer taut : Tautomers.generateNoIdentity(mol, roles, Tautomers.Order.SEQUENTIAL)) {
                        Aromaticity.apply(Aromaticity.Model.Daylight, taut);
                        if (recheck())
                            return true;
                        cache.add(new TautStore(mol));
                    }
                } else {
                    for (TautStore store : cache) {
                        store.apply(mol);
                        if (recheck())
                            return true;
                    }
    ...
    private boolean checkRoles() {
        for (int i = 0; i < amap.length; i++)
            if (!((IQueryAtom) query.getAtom(i)).matches(amap[i]) &&
                roles[amap[i].getIndex()] == Tautomers.Role.X &&
                (!amap[i].isInRing() || amap[i].getAtomicNumber() == IAtom.C)) {
                return false;
            }
        return true;
    }
