...
/**
 * A stereo encoder factory encoding double bond configurations by 2D and 3D coordinates. This
 * factory will attempt to encode all double bonds that meet the following conditions. Are not
 * {@literal -N=N-} bonds, non-cumulated, non-query and have each double bonded atom has at least
 * one substituent. In future the encoding rules may be more strict or even configurable but
 * currently they may be over zealous when encoding configurations with 3D coordinates. <br>
...
public final class GeometricDoubleBondEncoderFactory implements StereoEncoderFactory {
    ...
    /**
     * Create a new encoder for the specified left and right atoms. The parent is the atom which is
     * connected by a double bond to the left and right atom. For simple double bonds the parent of
     * each is the other atom, in cumulenes the parents are not the same.
     *
     ...
     */
    static StereoEncoder newEncoder(
            ...
            if (geometric != null) {
                return new GeometryEncoder(
                        ...
                        new CombinedPermutationParity(
                                permutation(leftNeighbors), permutation(rightNeighbors)),
                        geometric);
            }
        }
    ...
    /**
     * Generate a new geometric parity (2D or 3D) for the given molecule and atom indices. This
     * method ensure that 2D and 3D coordinates are available on the specified atoms and returns
     * null if the 2D or 3D coordinates are not fully available.
     *
     * @param mol a molecule
     * @param l left double bonded atom
     * @param r right double bonded atom
     * @param l1 first substituent atom of <i>l</i>
     * @param l2 second substituent atom of <i>l</i> or <i>l</i> if there is none
     * @param r1 first substituent atom of <i>r</i>
     * @param r2 second substituent atom of <i>r</i> or <i>r</i> if there is none
     * @return geometric parity or null
     */
    static GeometricParity geometric(
            IAtomContainer mol, int l, int r, int l1, int l2, int r1, int r2) {
    ...
    /**
     * Create a permutation parity for the given neighbors. The neighbor list should include the
    ...
    /**
     * Utility method for shifting a specified value in an index to the back (see {@link
     * #permutation(int[])}).
     *
    ...
    /**
     * Test whether we accept atom and it's connected bonds for inclusion in a double bond
     * configuration. This method checks for query bonds (up/down) as well as double bond counts. If
     * there is more then one double bond in the connect bonds then it cannot have Z/E
     * configuration.
     *
    ...
    static boolean accept(IAtom atom, List<IBond> bonds) {
        ...
        for (IBond bond : bonds) {
            ...
            if (IBond.Stereo.UP_OR_DOWN.equals(stereo)
                    || IBond.Stereo.UP_OR_DOWN_INVERTED.equals(stereo)) return false;
        }
