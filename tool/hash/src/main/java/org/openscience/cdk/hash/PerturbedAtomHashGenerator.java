...
/**
 * A perturbed hash generator {@cdk.cite Ihlenfeldt93} which differentiates molecules with uniform
 * atom environments and symmetry. The generator first calculates the basic hash codes ({@link
 * BasicAtomHashGenerator}) and then checks for duplicate values (uniform environments). These
 * duplicate values are then filtered down ({@link EquivalentSetFinder}) to a set (<i>S</i>) which
 * can introduce systematic differences with. We then combine the |<i>S</i>| different invariant
 * values with the original value to produce a unique value of each atom. There may still be
 * duplicate values but providing the depth is appropriate then the atoms are truly equivalent. <br>
 * The class requires a lot of configuration however it can be easily built with the {@link
 * HashGeneratorMaker}.
 *
 * <blockquote>
 *
...
final class PerturbedAtomHashGenerator extends AbstractHashGenerator implements AtomHashGenerator {
    ...
    /**
     * Create a perturbed hash generator using the provided seed generator to initialise atom
     * invariants and using the provided stereo factory.
     *
     ...
     * @param finder equivalent set finder for driving the systematic perturbation
     ...
     * @see org.openscience.cdk.hash.SeedGenerator
     */
    public PerturbedAtomHashGenerator(
            SeedGenerator seeds,
    ...
    public long[] generate(IAtomContainer container) {
        ...
        return generate(
                container, seeds.generate(container), factory.create(container, graph), graph);
    }

    private long[] generate(
            IAtomContainer container, long[] seeds, StereoEncoder encoder, int[][] graph) {
    ...
    /**
     * Combines the values in an n x m matrix into a single array of size n. This process scans the
     * rows and xors all unique values in the row together. If a duplicate value is found it is
     * rotated using a pseudorandom number generator.
     *
