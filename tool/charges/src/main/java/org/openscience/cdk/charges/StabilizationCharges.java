...
/**
 * The stabilization of the positive and the negative charge obtained (e.g in the polar breaking of
 * a bond) is calculated from the sigma- and lone pair-electronegativity values of the atoms that
 * are in conjugation to the atoms obtaining the charges. Based on H. Saller
 * Dissertation @cdk.cite{SallerH1895}.
 *
...
public class StabilizationCharges {
     ...
     * @param atom IAtom for which effective atom StabilizationCharges factor should be calculated
     * @return stabilizationValue
     */
    public double calculatePositive(IAtomContainer atomContainer, IAtom atom) {
        /* restrictions */
        //    	if(atomContainer.getConnectedSingleElectronsCount(atom) > 0 || atom.getFormalCharge()
        // != 1){
        ...
        if (resonanceS.getAtomContainerCount()
                < 2) // meaning it was not find any resonance structure
        return 0.0;
        ...
        for (Iterator<IAtomContainer> itA = resonanceS.atomContainers().iterator();
                itA.hasNext(); ) {
            ...
            final ShortestPaths shortestPaths =
                    new ShortestPaths(resonance, resonance.getAtom(positionStart));
            ...
            for (Iterator<IAtom> itAtoms = resonance.atoms().iterator(); itAtoms.hasNext(); ) {
                ...
                if (containerS.getAtomContainer(0).contains(atomR)) {
                    ...
                    double result =
                            electronegativity.calculatePiElectronegativity(resonance, atomP);
                    result1.add(result);
