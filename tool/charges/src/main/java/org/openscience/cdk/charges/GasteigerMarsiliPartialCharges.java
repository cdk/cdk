 ...
 * This class only implements the original method which only applies to &sigma;-bond systems.
 *
 * @author chhoppe
 * @author rojas
 * @cdk.module charges
...
public class GasteigerMarsiliPartialCharges implements IChargeCalculator {
    ...
    /**
     * Sets chi_cat value for hydrogen, because H poses a special problem due to lack of possible
     * second ionisation.
     *
    ...
    /**
     * Gets chi_cat value for hydrogen, because H poses a special problem due to lack of possible
     * second ionisation.
     *
     ...
     */
    public IAtomContainer assignGasteigerMarsiliSigmaPartialCharges(
            IAtomContainer ac, boolean setCharge) throws Exception {
        ...
        for (int i = 0; i < MX_ITERATIONS; i++) {
            ...
            for (int j = 0; j < ac.getAtomCount(); j++) {
                ...
                gasteigerFactors[STEP_SIZE * j + j + 4] =
                        gasteigerFactors[STEP_SIZE * j + j + 2] * q * q
                                + gasteigerFactors[STEP_SIZE * j + j + 1] * q
                                + gasteigerFactors[STEP_SIZE * j + j];
                //				logger.debug("g4: "+gasteigerFactors[STEP_SIZE * j + j + 4]);
            }
            if (!isDifferent) /* automatically break the maximum iterations */ break out;
            ...
            while (bonds.hasNext()) {
                ...
                if (gasteigerFactors[STEP_SIZE * atom1 + atom1 + 4]
                        >= gasteigerFactors[STEP_SIZE * atom2 + atom2 + 4]) {
                ...
                q =
                        (gasteigerFactors[STEP_SIZE * atom1 + atom1 + 4]
                                        - gasteigerFactors[STEP_SIZE * atom2 + atom2 + 4])
                                / deoc;
                //				logger.debug("qq: "+q);
    ...
    public double[] assignGasteigerSigmaMarsiliFactors(IAtomContainer ac) throws CDKException {
        ...
        for (int i = 0; i < ac.getAtomCount(); i++) {
            ...
            switch (symbol) {
                ...
                default:
                    throw new CDKException(
                            "Partial charge not-supported for element: '" + symbol + "'.");
            }
