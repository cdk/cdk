...
/**
 * Place aliphatic <b>chains</b> with Z matrix method. Please use {@link ModelBuilder3D} to place
 * general molecules.
 *
 * @author chhoppe
...
public class AtomPlacer3D {
    ...
    private static final double DIHEDRAL_EXTENDED_CHAIN = (180.0 / 180) * Math.PI;
    ...
    private static final double DEFAULT_SP2_ANGLE = 120.000;
    private static final double DEFAULT_SP_ANGLE = 180.000;
        ...
        for (int i = 0; i < chain.getAtomCount(); i++) {
            if (isHeavyAtom(chain.getAtom(i))) {
                if (!chain.getAtom(i).getFlag(CDKConstants.VISITED)) {
                    ...
                    if (molecule.getBond(
                                    molecule.getAtom(firstAtoms[counter - 1]),
                                    molecule.getAtom(nextAtomNr))
                            == null)
                        throw new CDKException(
                                "atoms do not form a chain, please use ModelBuilder3D");
                    ...
                    if (counter > 1) {
                        ...
                        hybridisation =
                                getHybridisationState(molecule.getAtom(firstAtoms[counter - 1]));
                    ...
                    if (counter > 2) {
                        ...
                        try {
                            if (getDoubleBondConfiguration2D(
                                            ...
                                            (molecule.getAtom(firstAtoms[counter - 2]))
                                                    .getPoint2d(),
                                            (molecule.getAtom(firstAtoms[counter - 3]))
                                                    .getPoint2d())
                                    == 5) {
     ...
     * The rest is added by applying the Zmatrix distances, angles and dihedrals. Assign coordinates
     * directly to the atoms.
     *
     * @param molecule the molecule to be placed in 3D
     * @param flagBranched marks branched chain author: egonw,cho
     */
    public void zmatrixChainToCartesian(IAtomContainer molecule, boolean flagBranched) {
        ...
        for (int index = 0; index < distances.length; index++) {
            if (index == 0) {
                ...
                cd.sub(
                        molecule.getAtom(thirdAtoms[index]).getPoint3d(),
                        molecule.getAtom(secondAtoms[index]).getPoint3d());

                Vector3d bc = new Vector3d();
                bc.sub(
                        molecule.getAtom(secondAtoms[index]).getPoint3d(),
                        molecule.getAtom(firstAtoms[index - 3]).getPoint3d());
                ...
                if (index == 3 && flagBranched) {
                    ba =
                            AtomTetrahedralLigandPlacer3D.rotate(
                                    cd, n2, (-angles[index] / 180) * Math.PI);
                    ba =
                            AtomTetrahedralLigandPlacer3D.rotate(
                                    ba, cd, (-angles[index] / 180) * Math.PI);
                } else {
                    ba =
                            AtomTetrahedralLigandPlacer3D.rotate(
                                    cd, n2, (-angles[index] / 180) * Math.PI);
    ...
    /**
     * Gets the doubleBondConfiguration2D attribute of the AtomPlacer3D object using existing 2D
     * coordinates.
     *
    ...
    public double getAngleValue(String id1, String id2, String id3) {
        ...
        if (pSet.containsKey(("angle" + id1 + ";" + id2 + ";" + id3))) {
            ...
            // logger.debug("KEYErrorAngle:Unknown angle key in pSet: " +id2 + " ; " + id3 + " ; " +
            // id1+" take default angle:"+DEFAULT_ANGLE);
            return -1;
        }
    ...
    /**
     * Gets the nextUnplacedHeavyAtomWithAliphaticPlacedNeighbour from an atom container or
     * molecule.
     *
     * @param molecule
     * @return The nextUnplacedHeavyAtomWithAliphaticPlacedNeighbour value author: steinbeck,cho
     */
    ...
    IAtom getUnplacedHeavyAtom(IAtomContainer molecule) {
        for (IAtom atom : molecule.atoms()) {
            if (isUnplacedHeavyAtom(atom)) return atom;
        }
        return null;
    }
     ...
     *
     * @param molecule
     * @return The nextUnplacedHeavyAtomWithUnplacedAliphaticNeigbor author: steinbeck,cho
     */
    ...
    public IAtom getUnplacedRingHeavyAtom(IAtomContainer molecule, IAtom atom) {
        ...
        for (IBond bond : bonds) {
            ...
            if (isUnplacedHeavyAtom(connectedAtom)
                    && connectedAtom.getFlag(CDKConstants.ISINRING)) {
     ...
     * @return a placed heavy atom connected to a given atom author: steinbeck
     */
    ...
    /**
     * Determine if the atom is non-hydrogen and is in a ring. Ring membership is determined from a
     * property flag only, rather than a ring membership test
     *
     * @param atom The atom to be checked
