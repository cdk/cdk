...
public class ModelBuilder3D {
     ...
     */
    private ModelBuilder3D(
            TemplateHandler3D templateHandler, String ffname, IChemObjectBuilder builder)
            throws CDKException {
    ...
    public static ModelBuilder3D getInstance(
            TemplateHandler3D templateHandler, String ffname, IChemObjectBuilder chemObjectBuilder)
            throws CDKException {
    ...
    private void setForceField(String ffname, IChemObjectBuilder builder) throws CDKException {
        ...
        try {
            ...
            throw new CDKException(
                    "Problem with ForceField configuration due to>" + ex1.getMessage(), ex1);
        }
    }
    ...
    public IAtomContainer generate3DCoordinates(IAtomContainer molecule, boolean clone)
            throws CDKException, NoSuchAtomTypeException, CloneNotSupportedException, IOException {
        ...
        if (ap3d.numberOfUnplacedHeavyAtoms(molecule) == 1) {
            ...
            try {
                ...
                logger.error(
                        "PlaceSubstitutensERROR: Cannot place substitutents due to:"
                                + ex3.getMessage());
                logger.debug(ex3);
                throw new CDKException(
                        "PlaceSubstitutensERROR: Cannot place substitutents due to:"
        ...
        if (ringSetMolecule.getAtomContainerCount() > 0) {
            ...
            if (!checkAllRingAtomsHasCoordinates(largestRingSetContainer)) {
                throw new CDKException(
                        "RingAtomLayoutError: Not every ring atom is placed! Molecule cannot be layout.");
            }
        ...
        try {
            ...
            logger.debug(ex3);
            throw new CDKException(
                    "PlaceSubstitutensERROR: Cannot place substitutents due to:" + ex3.getMessage(),
                    ex3);
        }
     ...
     */
    private void layoutMolecule(
            List ringSetMolecule,
        ...
        do {
            ...
            if (atom != null) {
                ...
                IAtomContainer ringSetAContainer =
                        RingSetManipulator.getAllInOneContainer(ringSetA);
                ...
                setBranchAtom(
                        ...
                        unplacedAtom,
                        atom,
                        ap3d.getPlacedHeavyAtoms(molecule, atom),
                        ap3d,
                        atlp3d);
                layoutRingSystem(
                        firstAtomOriginalCoord,
                        unplacedAtom,
                        ringSetA,
     ...
     */
    private void layoutRingSystem(
            Point3d originalCoord,
            IAtom placedRingAtom,
            IRingSet ringSet,
            Point3d centerPlacedMolecule,
            IAtom atomB,
            AtomPlacer3D ap3d) {
        // logger.debug("****** Layout ring System ******");System.out.println(">around
        // atom:"+molecule.indexOf(placedRingAtom));
        IAtomContainer ac = RingSetManipulator.getAllInOneContainer(ringSet);
        Point3d newCoord = placedRingAtom.getPoint3d();
        Vector3d axis =
                new Vector3d(
                        atomB.getPoint3d().x - newCoord.x,
                        atomB.getPoint3d().y - newCoord.y,
        ...
        Vector3d startAtomVector =
                new Vector3d(
                        newCoord.x - atomB.getPoint3d().x,
                        newCoord.y - atomB.getPoint3d().y,
                        newCoord.z - atomB.getPoint3d().z);
        IAtom farthestAtom = ap3d.getFarthestAtom(placedRingAtom.getPoint3d(), ac);
        Vector3d farthestAtomVector =
                new Vector3d(
                        farthestAtom.getPoint3d().x - newCoord.x,
                        farthestAtom.getPoint3d().y - newCoord.y,
                        farthestAtom.getPoint3d().z - newCoord.z);
        ...
        double angle =
                Math.acos(dotProduct / (farthestAtomVector.length() * farthestVector.length()));
        ...
        for (int i = 1; i < 360; i++) {
            ...
            if (centerPlacedMolecule.distance(new Point3d(ringCenter.x, ringCenter.y, ringCenter.z))
                    > distance) {
                rotAngleMax = i;
                distance =
                        centerPlacedMolecule.distance(
                                new Point3d(ringCenter.x, ringCenter.y, ringCenter.z));
            }
        }
     ...
     */
    private void setBranchAtom(
            IAtomContainer molecule,
            IAtom unplacedAtom,
            IAtom atomA,
            IAtomContainer atomNeighbours,
            AtomPlacer3D ap3d,
            AtomTetrahedralLigandPlacer3D atlp3d)
            throws CDKException {
        ...
        double angle =
                (ap3d.getAngleValue(atypeNameB, atypeNameA, atypeNameUnplaced)) * Math.PI / 180;
        /*
        ...
        Point3d[] branchPoints =
                atlp3d.get3DCoordinatesForLigands(
                        atomA,
                        noCoords,
                        atomNeighbours,
                        atomC,
        ...
        if (atomA.getStereoParity() != CDKConstants.UNSET && atomA.getStereoParity() != 0
            ...
            if (atomNeighbours.getAtomCount() > 1) {
                stereo =
                        atlp3d.makeStereocenter(
                                atomA.getPoint3d(),
                                molecule.getBond(atomA, unplacedAtom),
                                (atomNeighbours.getAtom(0)).getPoint3d(),
                                (atomNeighbours.getAtom(1)).getPoint3d(),
                                branchPoints);
            }
        }
        if (stereo != -1) {
     ...
     */
    private void searchAndPlaceBranches(
            IAtomContainer molecule,
        ...
        for (int i = 0; i < chain.getAtomCount(); i++) {
            ...
            for (int j = 0; j < atoms.size(); j++) {
                ...
                if (!(atom.getSymbol()).equals("H")
                    ...
                    // logger.debug("SEARCH PLACE AND FOUND Branch Atom
                    // "+molecule.indexOf(chain.getAtomAt(i))+
                    //		" New Atom:"+molecule.indexOf(atoms[j])+" -> STORE");
                    connectedAtoms.add(ap3d.getPlacedHeavyAtoms(molecule, chain.getAtom(i)));
                    // logger.debug("Connected
                    // atom1:"+molecule.indexOf(connectedAtoms.getAtomAt(0))+" atom2:"+
                    // molecule.indexOf(connectedAtoms.getAtomAt(1))+ "
                    // Length:"+connectedAtoms.getAtomCount());
                    try {
                        setBranchAtom(
                                molecule, atom, chain.getAtom(i), connectedAtoms, ap3d, atlp3d);
                    } catch (CDKException ex2) {
                        logger.error(
                                "SearchAndPlaceBranchERROR: Cannot find enough neighbour atoms due to"
                                        + ex2.toString());
                        throw new CDKException(
                                "SearchAndPlaceBranchERROR: Cannot find enough neighbour atoms: "
                                        + ex2.getMessage(),
                                ex2);
                    }
     ...
     */
    private void placeLinearChains3D(
            IAtomContainer molecule,
        ...
        IAtomContainer longestUnplacedChain =
                molecule.getBuilder().newInstance(IAtomContainer.class);
        if (startAtoms.getAtomCount() == 0) {
            ...
            for (int i = 0; i < startAtoms.getAtomCount(); i++) {
                ...
                dihPlacedAtom =
                        ap3d.getPlacedHeavyAtom(molecule, thirdPlacedAtom, startAtoms.getAtom(i));
                ...
                longestUnplacedChain.add(
                        atomPlacer.getLongestUnplacedChain(molecule, startAtoms.getAtom(i)));
                setAtomsToUnVisited(molecule);
                ...
                if (longestUnplacedChain.getAtomCount() < 4) {
                    ...
                    searchAndPlaceBranches(
                            molecule, longestUnplacedChain, ap3d, atlp3d, atomPlacer);
                }
    ...
    private boolean checkAllRingAtomsHasCoordinates(IAtomContainer ac) {
        for (int i = 0; i < ac.getAtomCount(); i++) {
            if (ac.getAtom(i).getPoint3d() != null
                    && ac.getAtom(i).getFlag(CDKConstants.ISINRING)) {
    ...
    /**
     * Returns the number of loaded templates. Note that it may return 0 because templates are lazy
     * loaded, that is upon the first ring being laid out.
     *
