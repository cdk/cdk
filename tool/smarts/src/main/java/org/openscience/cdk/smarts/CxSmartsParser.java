...
/**
 * Parse CXSMART (ChemAxon Extended SMARTS) layers. The layers are suffixed after the SMILES but before the title
 * and encode a large number of the features. CXSMILES/CXSMARTS was not intended for outside consumption so has some
 * quirks but does provide some useful features. This parser handles a subset of the grammar:
 * <br>
 * <pre>
 * - Atom Labels
 * - Atom Values
 ...
 * </pre>
 *
 * Currently, the SMARTS parsing is only used for coordinates.
 * Note this class is more or less a copy of CxSmilesParser but we do not want
 * to expose that as a public API.
 */
final class CxSmartsParser {
     ...
     * @return parse was a success (or not)
     */
    private static boolean processFragmentGrouping(final CharIter iter, final CxSmartsState state) {
     ...
     * ({@link org.openscience.cdk.smarts.CxSmartsState}) representation.
     *
     * @param str input character string (SMILES title field)
     * @param state output CXSMILES state
     * @return position where CXSMILES ends (below 0 means no CXSMILES)
     */
    static int processCx(final String str, final CxSmartsState state) {
