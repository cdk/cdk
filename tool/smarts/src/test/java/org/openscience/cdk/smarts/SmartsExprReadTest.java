...
public class SmartsExprReadTest {
    ...
    public void opPrecedence1() {
        ...
        Expr expected =
                or(
                        and(expr(IS_AROMATIC), expr(ELEMENT, 6)),
    ...
    public void opPrecedence2() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = and(expr(IS_AROMATIC), or(expr(ELEMENT, 6), expr(ELEMENT, 7)));
        assertThat(actual, is(expected));
    }
    ...
    public void opPrecedence4() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = or(expr(ELEMENT, 6), and(expr(ELEMENT, 7), expr(IS_AROMATIC)));
        assertThat(actual, is(expected));
    }
    ...
    public void opPrecedence5() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = or(expr(ELEMENT, 7), and(expr(ELEMENT, 6), expr(IS_AROMATIC)));
        assertThat(actual, is(expected));
    }
    ...
    public void orList() {
        ...
        Expr expected =
                or(
                        expr(ELEMENT, 9),
                        or(expr(ELEMENT, 17), or(expr(ELEMENT, 35), expr(ELEMENT, 53))));
        assertThat(actual, is(expected));
    }
    ...
    public void explicitHydrogen() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = and(expr(ISOTOPE, 2), and(expr(ELEMENT, 1), expr(FORMAL_CHARGE, 1)));
        assertThat(actual, is(expected));
    }
    ...
    public void explicitHydrogenNeg() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = and(expr(ELEMENT, 1), expr(FORMAL_CHARGE, -1));
        assertThat(actual, is(expected));
    }
    ...
    public void explicitHydrogenWithAtomMap() {
        ...
        Expr expected = and(expr(ISOTOPE, 2), and(expr(ELEMENT, 1), expr(FORMAL_CHARGE, 1)));
        assertThat(
                mol.getAtom(0).getProperty(CDKConstants.ATOM_ATOM_MAPPING, Integer.class), is(2));
        assertThat(actual, is(expected));
    }
    ...
    public void nonExplicitHydrogen() {
        ...
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = and(expr(ISOTOPE, 2), and(expr(TOTAL_H_COUNT, 1), expr(FORMAL_CHARGE, +1)));
        assertThat(actual, is(expected));
    }
    ...
    public void nonExplicitHydrogen2() {
        ...
        assertTrue(Smarts.parse(mol, "[2,H+]"));
        Expr actual = getAtomExpr(mol.getAtom(0));
        Expr expected = or(expr(ISOTOPE, 2), and(expr(TOTAL_H_COUNT, 1), expr(FORMAL_CHARGE, +1)));
        assertThat(actual, is(expected));
    }
    ...
    public void atomMaps() {
        ...
        assertThat(
                mol.getAtom(0).getProperty(CDKConstants.ATOM_ATOM_MAPPING, Integer.class), is(10));
    }
    ...
    public void insaturationCactvsOrMoe() {
        assertThat(getAtomExpr("[i1]", Smarts.FLAVOR_CACTVS), is(expr(INSATURATION, 1)));
        assertThat(getAtomExpr("[i1]", Smarts.FLAVOR_MOE), is(expr(INSATURATION, 1)));
    }

    @Test
    public void heteroSubCountCactvs() {
        assertThat(getAtomExpr("[z]", Smarts.FLAVOR_CACTVS), is(expr(HAS_HETERO_SUBSTITUENT)));
        assertThat(
                getAtomExpr("[z1]", Smarts.FLAVOR_CACTVS), is(expr(HETERO_SUBSTITUENT_COUNT, 1)));
    }
    ...
    public void hybridisationNumberDaylight() {
        assertFalse(Smarts.parse(new QueryAtomContainer(null), "[^2]", Smarts.FLAVOR_DAYLIGHT));
    }
    ...
    public void atomStereoLeftOrUnspec() {
        Expr actual = getAtomExpr("[@?]");
        Expr expected = or(expr(STEREOCHEMISTRY, IStereoElement.LEFT), expr(STEREOCHEMISTRY, 0));
        assertThat(actual, is(expected));
    }
    ...
    public void singleOrDouble() {
        Expr actual = getBondExpr("*-,=*");
        Expr expected = or(expr(ALIPHATIC_ORDER, 1), expr(ALIPHATIC_ORDER, 2));
        assertThat(expected, is(actual));
    }
    ...
    public void operatorPrecedence() {
        Expr actual = getBondExpr("*@;-,=*");
        Expr expected =
                and(expr(IS_IN_RING), or(expr(ALIPHATIC_ORDER, 1), expr(ALIPHATIC_ORDER, 2)));
        assertThat(expected, is(actual));
    }
    ...
    public void testTrivialRecursive() {
        ...
        assertThat(expr, is(or(expr(ELEMENT, 9), or(expr(ELEMENT, 17), expr(ELEMENT, 35)))));
    }
    ...
    public void testTrivialRecursive2() {
        Expr expr = getAtomExpr("[!$([F,Cl,Br])]");
        assertThat(
                expr, is(or(expr(ELEMENT, 9), or(expr(ELEMENT, 17), expr(ELEMENT, 35))).negate()));
    }
    ...
    public void degreeRange() {
        ...
        assertThat(expr, is(or(expr(DEGREE, 1), or(expr(DEGREE, 2), expr(DEGREE, 3)))));
    }
    ...
    public void implHRange() {
        ...
        assertThat(
                expr,
                is(or(expr(IMPL_H_COUNT, 1), or(expr(IMPL_H_COUNT, 2), expr(IMPL_H_COUNT, 3)))));
    }
    ...
    public void totalHCountRange() {
        ...
        assertThat(
                expr,
                is(or(expr(TOTAL_H_COUNT, 1), or(expr(TOTAL_H_COUNT, 2), expr(TOTAL_H_COUNT, 3)))));
    }

    @Test
    public void valenceRange() {
        Expr expr = getAtomExpr("[v{1-3}]");
        assertThat(expr, is(or(expr(VALENCE, 1), or(expr(VALENCE, 2), expr(VALENCE, 3)))));
    }
    ...
    public void ringBondCountRange() {
        ...
        assertThat(
                ...
                is(
                        or(
                                expr(RING_BOND_COUNT, 2),
                                or(expr(RING_BOND_COUNT, 3), expr(RING_BOND_COUNT, 4)))));
    }
    ...
    public void ringSmallestSizeCountRange() {
        ...
        assertThat(
                expr,
                is(or(expr(RING_SMALLEST, 5), or(expr(RING_SMALLEST, 6), expr(RING_SMALLEST, 7)))));
    }
    ...
    public void supportHGt() {
        ...
        assertThat(expr, is(and(expr(TOTAL_H_COUNT, 0).negate(), expr(TOTAL_H_COUNT, 1).negate())));
    }

    @Test
    public void supportHLt() {
        Expr expr = getAtomExpr("[H<2]");
        assertThat(expr, is(or(expr(TOTAL_H_COUNT, 0), expr(TOTAL_H_COUNT, 1))));
    }

    @Test
    public void supportDGt() {
        Expr expr = getAtomExpr("[D>1]");
        assertThat(expr, is(and(expr(DEGREE, 0).negate(), expr(DEGREE, 1).negate())));
    }

    @Test
    public void supportDLt() {
        Expr expr = getAtomExpr("[D<2]");
        assertThat(expr, is(or(expr(DEGREE, 0), expr(DEGREE, 1))));
    }
}
