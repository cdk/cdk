...
public class SmartsFragmentExtractorTest {
    ...
    private static int[] makeSeq(int beg, int to) {
        ...
        for (int i = 0; i < a.length; i++) a[i] = beg++;
        return a;
    }

    @Test
    public void methylExact() throws Exception {
        String smarts = generate("CC(C)CCC", SmartsFragmentExtractor.MODE_EXACT, makeSeq(0, 1));
    ...
    public void methylForJCompoundMap() throws Exception {
        String smarts =
                generate("CC(C)CCC", SmartsFragmentExtractor.MODE_JCOMPOUNDMAPPER, makeSeq(0, 1));
    ...
    public void indole() throws Exception {
        String smarts =
                generate("[nH]1ccc2c1cccc2", SmartsFragmentExtractor.MODE_EXACT, makeSeq(0, 4));
    ...
    public void biphenylIncludesSingleBond() throws Exception {
        String smarts =
                generate("c1ccccc1-c1ccccc1", SmartsFragmentExtractor.MODE_EXACT, makeSeq(0, 12));
    ...
    public void fullereneC60() throws Exception {
        String smarts =
                generate(
                        ...
                        SmartsFragmentExtractor.MODE_EXACT, makeSeq(0, 60));
        assertThat(
                smarts,
                is(
