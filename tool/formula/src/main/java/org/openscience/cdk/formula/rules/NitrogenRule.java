 ...
 *
 * <p>If a compound has an odd number of nitrogen atoms, then the molecular ion (the [M]+) will have
 * an odd mass and the value for m/e will be odd.
 *
 * <p>If a compound has no nitrogen atom or an even number of nitrogen atoms, then the m/e value of
 * [M]+ will be even.
 *
 * <table border="1">
...
public class NitrogenRule implements IRule {
    ...
    public double validate(IMolecularFormula formula) throws CDKException {
        ...
        int numberN =
                MolecularFormulaManipulator.getElementCount(
                        formula, formula.getBuilder().newInstance(IElement.class, "N"));
        numberN += getOthers(formula);

        if (formula.getCharge() == null
                || formula.getCharge() == 0
                || !isOdd(Math.abs(formula.getCharge()))) {
    ...
    /**
     * Get the number of other elements which affect to the calculation of the nominal mass. For
     * example Fe, Co, Hg, Pt, As.
     *
    ...
    private int getOthers(IMolecularFormula formula) {
        ...
        for (int i = 0; i < elements.length; i++)
            number +=
                    MolecularFormulaManipulator.getElementCount(
                            formula, formula.getBuilder().newInstance(IElement.class, elements[i]));
