 ...
 * </pre>
 *
 * This class offers two implementations: The Round Robin algorithm {@cdk.cite Boecker2008} on mass
 * ranges {@cdk.cite Duehrkop2013} is used on most inputs. For special cases (e.g. single elements,
 * extremely large mass ranges) a full enumeration algorithm {@cdk.cite Pluskal2012} is used.
 *
 * <p>The Round Robin algorithm was originally developed for the SIRIUS 3 software. The full
 * enumeration algorithm was originally developed for a MZmine 2 framework module, published in
 * Pluskal et al. {@cdk.cite Pluskal2012}.
 *
 * @cdk.module formula
...
public class MolecularFormulaGenerator implements IFormulaGenerator {
     ...
     */
    public MolecularFormulaGenerator(
        ...
        this.formulaGenerator =
                isIllPosed(minMass, maxMass, mfRange)
                        ? new FullEnumerationFormulaGenerator(builder, minMass, maxMass, mfRange)
                        : new RoundRobinFormulaGenerator(builder, minMass, maxMass, mfRange);
    }

    /**
     * Decides wheter to use the round robin algorithm or full enumeration algorithm. The round
     * robin implementation here is optimized for chemical elements in organic compounds. It gets
     * slow if - the mass of the smallest element is very large (i.e. hydrogen is not allowed) - the
     * maximal mass to decompose is too large (round robin always decomposes integers. Therefore,
     * the mass have to be small enough to be represented as 32 bit integer) - the number of
     * elements in the set is extremely small (in this case, however, the problem becomes trivial
     * anyways)
     *
     * <p>In theory we could handle these problems by optimizing the way DECOMP discretizes the
     * masses. However, it's easier to just fall back to the full enumeration method if a problem
     * occurs (especially, because most of the problems lead to trivial cases that are fast to
     * compute).
     *
     * @return true if the problem is ill-posed (i.e. should be calculated by full enumeration
     *     method)
     */
    private static boolean isIllPosed(
            double minMass, double maxMass, MolecularFormulaRange mfRange) {
     ...
     * MolecularFormulaGenerator#getNextFormula()} until all possible formulas are generated. There
     * is no guaranteed order to the formulas in the resulting {@link IMolecularFormulaSet}.
     *
     ...
     * formulas will not be included in the returned {@link IMolecularFormulaSet}.
     *
     * @see #getNextFormula()
     */
    @Override
     ...
     * MolecularFormulaGenerator#getNextFormula()}, this method returns 0. After all possible
     ...
     * executing the {@link MolecularFormulaGenerator#getNextFormula()} method, that method call
     * will return immediately with null return value. If another thread is executing the {@link
     * MolecularFormulaGenerator#getAllFormulas()} method, that method call will return immediately,
     * returning all formulas generated until this moment. The search cannot be restarted once
     * canceled - any subsequent calls to {@link MolecularFormulaGenerator#getNextFormula()} will
     * return null.
     */
    @Override
    public void cancel() {
