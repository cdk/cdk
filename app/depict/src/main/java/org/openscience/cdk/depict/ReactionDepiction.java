...
/**
 * Internal - depiction of a single reaction. We divide the reaction into two draw steps. The first
 * step draws the main components (reactants and products) whilst the second draws the side
 * components (agents: catalysts, solvents, spectators, etc). Reaction direction is drawn a single
 * headed arrow (forward and backward) or an equilibrium (bidirectional).
 */
final class ReactionDepiction extends Depiction {
    ...
    public ReactionDepiction(
        ...
        if (reactants.isEmpty()) this.mainComp.add(new Bounds());
        else this.mainComp.set(this.mainComp.size() - 1, new Bounds());
        ...
        if (!products.isEmpty()) this.mainComp.remove(this.mainComp.size() - 1);
        ...
        if (!reactantTitles.isEmpty() || !productTitles.isEmpty()) {
            if (!reactantTitles.isEmpty() && reactantTitles.size() != reactants.size())
                throw new IllegalArgumentException(
                        "Number of reactant titles differed from number of reactants");
            if (!productTitles.isEmpty() && productTitles.size() != products.size())
                throw new IllegalArgumentException(
                        "Number of product titles differed from number of products");
            ...
            if (reactants.isEmpty()) mainTitles.add(new Bounds()); // gap for arrow
            for (Bounds productTitle : productTitles) {
                mainTitles.add(productTitle);
                mainTitles.add(new Bounds());
            }
            // remove trailing space for plus
            if (!products.isEmpty()) mainTitles.remove(mainTitles.size() - 1);
        ...
        mainDim =
                Dimensions.ofGrid(
                        mainComp, yOffsets = new double[nRow + 1], xOffsets = new double[nCol + 1]);

        double middleRequired = Math.max(prelimSideDim.w, conditions.width());

        // avoid v. small arrows, we take in to account the padding provided by the arrow head
        // height/length
        if (middleRequired < minArrowWidth - arrowHeight - arrowHeight) {
            ...
            for (int i = 0; i < xOffsetSide.length; i++) xOffsetSide[i] += xAdjust;
            ...
            // arrow padding
            for (int i = 0; i < xOffsetSide.length; i++) xOffsetSide[i] += arrowHeight;
            ...
            this.sideDim = new Dimensions(2 * arrowHeight + middleRequired, prelimSideDim.h);
            this.condDim = new Dimensions(2 * arrowHeight + middleRequired, conditions.height());
        }
    }
    ...
    public BufferedImage toImg() {
        ...
        Dimensions titleRequired =
                new Dimensions(title.width(), title.height()).scale(scale * zoom);

        final double firstRowHeight = scale * zoom * yOffsets[1];
        final Dimensions total =
                calcTotalDimensions(
                        margin,
                        padding,
                        mainRequired,
        ...
        final BufferedImage img =
                new BufferedImage(
                        (int) Math.ceil(total.w),
                        (int) Math.ceil(total.h),
        ...
        mainCompOffset =
                fitting * sideRequired.h + nSideRow * padding - fitting * firstRowHeight / 2;
        ...
        final double totalRequiredWidth =
                2 * margin
                        + Math.max(0, nCol - 1) * padding
                        + Math.max(0, nSideCol - 1) * padding
                        + (rescale * xOffsets[nCol]);
        final double totalRequiredHeight =
                2 * margin
                        + Math.max(0, nRow - 1) * padding
                        + (!title.isEmpty() ? padding : 0)
                        + Math.max(mainCompOffset, 0)
                        + fitting * mainRequired.h
                        + fitting * Math.max(0, titleRequired.h);
        ...
        for (int i = 0; i < mainComp.size(); i++) {
            ...
            if (i > arrowIdx) x += Math.max(0, nSideCol - 1) * padding;
        ...
        if (!conditions.isEmpty()) {
            ...
            draw(
                    ...
                    zoom,
                    conditions,
                    rect(xBase, yBase, fitting * condRequired.w, fitting * condRequired.h));
        }
    ...
    String toVecStr(String fmt, String units) {
        ...
        double margin =
                getMarginValue(
                        units.equals(Depiction.UNITS_MM)
        ...
        if (!fmt.equals(SVG_FMT)) wrapper = new FreeHepWrapper(fmt, total.w, total.h);
        final IDrawVisitor visitor =
                fmt.equals(SVG_FMT)
                        ? new SvgDrawVisitor(total.w, total.h, units)
    ...
    private double calcFitting(
            ...
            String fmt) {
        if (dimensions == Dimensions.AUTOMATIC) return 1; // no fitting
        ...
        double mainCompOffset =
                sideRequired.h > 0
                        ? sideRequired.h + (nSideRow * padding) - (firstRowHeight / 2)
                        : 0;
        if (mainCompOffset < 0) mainCompOffset = 0;

        Dimensions required =
                mainRequired
                        .add(sideRequired.w, mainCompOffset)
        ...
        if (PDF_FMT.equals(fmt) || PS_FMT.equals(fmt)) targetDim = targetDim.scale(MM_TO_POINT);

        double resize = Math.min(targetDim.w / required.w, targetDim.h / required.h);

        if (resize > 1 && !model.get(BasicSceneGenerator.FitToScreen.class)) resize = 1;
        return resize;
    }

    private Dimensions calcTotalDimensions(
            double margin,
            double padding,
            Dimensions mainRequired,
            Dimensions sideRequired,
        ...
        if (dimensions == Dimensions.AUTOMATIC) {
            ...
            if (titleExtra > 0) titleExtra += padding;
            ...
            if (PDF_FMT.equals(fmt) || PS_FMT.equals(fmt)) return dimensions.scale(MM_TO_POINT);
    ...
    private Bounds createArrow(double minWidth, double minHeight) {
        ...
        switch (direction) {
            case FORWARD:
                arrow.add(
                        new LineElement(
                                0, 0, minWidth - 0.5 * headLength, 0, minHeight / 14, fgcol));
