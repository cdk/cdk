...
/**
 * Internal - depicts a set of molecules aligned in a grid. This class also handles the degenerate
 * case of a single molecule as a 1x1 grid.
 */
final class MolGridDepiction extends Depiction {
    ...
    public MolGridDepiction(
        ...
        if (titles.isEmpty()) {
            ...
            for (int r = 0; r < nRow; r++) {
                ...
                // need to pad list
                while (molsublist.size() < nCol) molsublist.add(new Bounds());
    ...
    public BufferedImage toImg() {
        ...
        final double xBase =
                margin
                        + (total.w - 2 * margin - (nCol - 1) * padding - (rescale * xOffset[nCol]))
                                / 2;
        final double yBase =
                margin
                        + (total.h - 2 * margin - (nRow - 1) * padding - (rescale * yOffset[nRow]))
                                / 2;
    ...
    private Dimensions calcTotalDimensions(
            double margin, double padding, Dimensions required, String fmt) {
