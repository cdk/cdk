...
/**
 * Internal - An SvgDrawVisitor, currently only certain elements are supported but covers depictions
 * generated by the {@link StandardGenerator} (only {@link LineElement} and {@link GeneralPath}).
 *
 * <p>Usage:
 *
 * <pre>{@link
...
final class SvgDrawVisitor implements IDrawVisitor {
    ...
    private void appendRelativePoints(
            StringBuilder sb, double[] points, double xBase, double yBase, int numPoints) {
        switch (numPoints) {
            case 1:
    ...
    String toStr(Color col) {
        if (col.getAlpha() == 255) {
            ...
            return String.format(
                    "rgba(%d,%d,%d,%.2f)",
                    col.getRed(), col.getGreen(), col.getBlue(), col.getAlpha() / 255d);
        }
    }
    ...
    /**
     * Pre-visit allows us to prepare the visitor for more optimal output. Currently we - find the
     * most common stoke/fill/stroke-width values and set these as defaults
     *
    ...
    public void previsit(Collection<? extends IRenderingElement> elements) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (element instanceof Bounds) {
                ...
                if (((GeneralPath) element).fill) fillFreq.add(((GeneralPath) element).color);
            } else {
                // ignored
            }
        }
        ...
        if (!defaultsWritten) {
            ...
            if (strokeWidth != null) defaultStrokeWidth = toStr(strokeWidth);
        }
    }
    ...
    private void visit(String id, String cls, LineElement elem) {
        double[] points =
                new double[] {
                    elem.firstPointX, elem.firstPointY, elem.secondPointX, elem.secondPointY
                };
        transform(points, 2);
        appendIdent();
        sb.append("<line");
        ...
        if (cls != null) sb.append(" class='").append(cls).append("'");
        sb.append(" x1='")
                .append(toStr(points[0]))
                .append("'")
                .append(" y1='")
                .append(toStr(points[1]))
                .append("'")
                .append(" x2='")
                .append(toStr(points[2]))
                .append("'")
                .append(" y2='")
                .append(toStr(points[3]))
                .append("'");
    ...
    private void visit(MarkedElement elem) {
        ...
        while (marked instanceof ElementGroup) {
            ...
            if (iter.hasNext()) marked = iter.next();
            else marked = null;
            if (iter.hasNext()) marked = null; // non-singleton
        }

        if (marked == null) marked = elem.element();
        ...
        if (marked instanceof LineElement) {
            ...
            appendIdent();
            sb.append("<g");
            if (id != null) sb.append(" id='").append(elem.getId()).append("'");
            if (cls != null) sb.append(" class='").append(cls).append("'");
            sb.append(">\n");
            indentLvl += 2;
            visit(marked);
    ...
    public void visit(final IRenderingElement root) {
        ...
        if (!defaultsWritten) {
            appendIdent();
            sb.append("<g").append(" stroke-linecap='round'").append(" stroke-linejoin='round'");
            ...
            if (defaultFill != null) sb.append(" fill='").append(toStr(defaultFill)).append("'");
            sb.append(">\n");
            indentLvl += 2;
        ...
        while (!queue.isEmpty()) {
            ...
            if (elem instanceof ElementGroup) {
                for (IRenderingElement child : (ElementGroup) elem) queue.add(child);
                ...
                System.err.println(
                        elem.getClass()
                                + " rendering element is not supported by"
    ...
    public String toString() {
        if (defaultsWritten) return sb.toString() + "  </g>\n</svg>\n";
