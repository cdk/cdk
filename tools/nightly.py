...
&nbsp; <a href='dnames.html#bond'>Bond</a> &nbsp; <a href='dnames.html#atom'>Atom</a>
    ...
    page = page + """
	<br><br><br>
	<a href='nightly.tar.gz'>Download</a> this site (excluding jars and tarballs). <br>
        <br><br>Generated by <a href=\"nightly.py\">nightly.py</a>
        <p>
    ...
    # make the nightly site tarball
    cmd  = """
tar -zcvf %s %s --exclude %s --exclude %s --exclude=*.gz --exclude=*.zip --exclude=*.jar > /dev/null
    """ % (os.path.join(nightly_web, 'nightly.tar.gz'), \
	   nightly_web, \
           os.path.join(nightly_web, 'api'), \
           os.path.join(nightly_web, 'javadoc'))
    os.system(cmd)
