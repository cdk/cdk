...
/**
 * An algorithm for topological symmetry. This algorithm derived from the algorithm {@cdk.cite
 * Hu94}.
 *
 * @cdk.githash
 * @author Junfeng Hao
...
public class EquivalentClassPartitioner {
    ...
    /**
     * Prepare the node identifier. The purpose of this is to increase the differentiation of the
     * nodes. Detailed information please see the corresponding literature.
     *
     * @param atomContainer atoms and bonds of the molecule
    ...
    public double[] prepareNode(IAtomContainer atomContainer) {
        ...
        for (IAtom atom : atomContainer.atoms()) {
            ...
            if (bonds.size() == 1) {
                ...
                if (symbol.equals("C")) {
                    if (order == IBond.Order.SINGLE) nodeSequence[i] = 1; // CH3-
                    else if (order == IBond.Order.DOUBLE) nodeSequence[i] = 3; // CH2=
                    else if (order == IBond.Order.TRIPLE) nodeSequence[i] = 6; // CH#
                } else if (symbol.equals("O")) {
                    if (order == IBond.Order.SINGLE) nodeSequence[i] = 14; // HO-
                    else if (order == IBond.Order.DOUBLE) nodeSequence[i] = 16; // O=
                    // missing the case of an aromatic double bond
                } else if (symbol.equals("N")) {
                    if (order == IBond.Order.SINGLE) nodeSequence[i] = 18; // NH2-
                    ...
                    if (order == IBond.Order.SINGLE) nodeSequence[i] = 31; // HS-
                    else if (order == IBond.Order.DOUBLE) nodeSequence[i] = 33; // S=
                } else if (symbol.equals("P")) nodeSequence[i] = 38; // PH2-
                else if (symbol.equals("F")) nodeSequence[i] = 42; // F-
                else if (symbol.equals("Cl")) nodeSequence[i] = 43; // Cl-
                else if (symbol.equals("Br")) nodeSequence[i] = 44; // Br-
                else if (symbol.equals("I")) nodeSequence[i] = 45; // I-
                else {
                    logger.debug(
                            "in case of a new node, please "
                ...
                if (symbol.equals("C")) {
                    ...
                    if (bond0.getFlag(CDKConstants.ISAROMATIC)
                            && bond1.getFlag(CDKConstants.ISAROMATIC)) nodeSequence[i] = 11; // ArCH
                } else if (symbol.equals("N")) {
                    if (order0 == IBond.Order.SINGLE && order1 == IBond.Order.SINGLE)
                    ...
                    // is no aromatic bond order
                    if (bond0.getFlag(CDKConstants.ISAROMATIC)
                            && bond1.getFlag(CDKConstants.ISAROMATIC)) nodeSequence[i] = 30; // ArN
                    ...
                    if (order0 == IBond.Order.SINGLE && order1 == IBond.Order.SINGLE)
                        nodeSequence[i] = 15; // -O-
                            ...
                            && bond1.getFlag(CDKConstants.ISAROMATIC)) nodeSequence[i] = 17; // ArO
                } else if (symbol.equals("S")) {
                    if (order0 == IBond.Order.SINGLE && order1 == IBond.Order.SINGLE)
                        nodeSequence[i] = 32; // -S-
                    else if (order0 == IBond.Order.DOUBLE && order1 == IBond.Order.DOUBLE)
                        nodeSequence[i] = 35; // =S=
                            ...
                            && bond1.getFlag(CDKConstants.ISAROMATIC)) nodeSequence[i] = 37; // ArS
                ...
                if (symbol.equals("C")) {
                    if (order0 == IBond.Order.SINGLE
                            ...
                            && order2 == IBond.Order.SINGLE) nodeSequence[i] = 4; // >C-
                    else if (order0 == IBond.Order.DOUBLE
                    ...
                    if (order0 == IBond.Order.SINGLE
                            && order1 == IBond.Order.SINGLE
                            && order2 == IBond.Order.SINGLE) nodeSequence[i] = 21; // >N-
                    else if (order0 == IBond.Order.SINGLE
                            || order1 == IBond.Order.SINGLE
                    ...
                    if (order0 == IBond.Order.DOUBLE
                            || order1 == IBond.Order.DOUBLE
                            || order2 == IBond.Order.DOUBLE) nodeSequence[i] = 34; // >S=
                } else if (symbol.equals("P")) {
                    if (order0 == IBond.Order.SINGLE
                            && order1 == IBond.Order.SINGLE
                            && order2 == IBond.Order.SINGLE) nodeSequence[i] = 40; // >P-
                } else {
                    logger.debug(
                            "in case of a new node, "
                                    + "please report this bug to cdk-devel@lists.sf.net.");
    ...
    public double[] buildWeightMatrix(double[][] nodeMatrix, double[][] bondMatrix) {
        for (int i = 0; i < nodeNumber; i++) {
            ...
            for (int j = 0; j < layerNumber; j++) {
                weight[i + 1] +=
                        nodeMatrix[i][j + 1] * bondMatrix[i][j] * Math.pow(10.0, (double) -(j + 1));
            }
        }
        weight[0] = 0.0;
        return weight;
