...
/**
 * This class is intended to provide the user an efficient way of implementing matrix of double
 * number and using normal operations (linear operations, addition, subtraction, multiplication,
 * inversion, concatenation) on them. The internal representation of a matrix is an array of array
 * of double objects. For the moment, double class is the best way I have developed to perform exact
 * operation on numbers; however, for irdoubles, normal operations on float and doubles have to be
 * performed, with the well-known risks of error this implies. This class also provides a way of
 * representing matrix as arrays of String for output use.
 *
 * <p>Please note that although in most books matrix elements' indexes take values between [1..n] I
 * chose not to disturb Java language way of calling indexes; so the indexes used here take values
 * between [0..n-1] instead.
 *
...
public class GIMatrix {
    ...
    /**
     * Class constructor. Uses an array of integers to create a new Matrix object. Note that
     * integers will be converted to double objects so mathematical operations may be properly
     * performed and provide exact solutions. The given array should be properly instantiated as a
     * matrix i.e. it must contain a fixed number of lines and columns, otherwise an exception will
     * be thrown. Array must be at leat 1x1.
     *
    ...
    /**
     * Class constructor. Uses an array of doubles to create a new Matrix object. The given array
     * should be properly instantiated as a matrix i.e. it must contain a fixed number of lines and
     * columns, otherwise an exception will be thrown. Array must be at leat 1x1.
     *
    ...
    /**
     * Class constructor. Creates a new Matrix object using a table of matrices (an array of Matrix
     * objects). The given array should be properly instantiated i.e. it must contain a fixed number
     * of lines and columns, otherwise an exception will be thrown.
     *
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                try {
                    ...
                    temp[i][j] =
                            tempMatrix.getValueAt(
                                    i - k * tempMatrix.height(), j - h * tempMatrix.width());
                } catch (IndexOutOfBoundsException e) {
                } // never happens
            }
        }
     ...
     * @exception IndexOutOfBoundsException if the given index is out of the matrix's range
     * @exception BadMatrixFormatException in case the given Matrix is unproper to replace a line of
     *     this Matrix
     */
    public void setLine(int i, GIMatrix line)
            throws IndexOutOfBoundsException, BadMatrixFormatException {
        if ((i < 0) || (i >= m)) throw new IndexOutOfBoundsException();
     ...
     * @exception IndexOutOfBoundsException if the given index is out of the matrix's range
     * @exception BadMatrixFormatException in case the given Matrix is unproper to replace a column
     *     of this Matrix
     */
    ...
    /**
     * Verifies if two given matrix are equal or not. The matrix must be of the same size and
     * dimensions, otherwise an exception will be thrown.
     *
     * @param matrix the Matrix object to be compared to
     * @return true if both matrix are equal element to element
     * @exception BadMatrixFormatException if the given matrix doesn't have the same dimensions as
     *     this one
     */
    public boolean equals(GIMatrix matrix) throws BadMatrixFormatException {
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++) if (!(array[i][j] == temp[i][j])) return false;
        return true;
    ...
    public boolean isSymmetric() throws BadMatrixFormatException {
        ...
        for (int i = 1; i < m; i++)
            ...
            for (int j = 0; j < i; j++) if (!(array[i][j] == array[j][i])) return false;
    ...
    /**
     * Verifies if the matrix is triangular superior or not. A triangular superior matrix has zero
     * (0) values everywhere under it's diagonal.
     *
    ...
    public boolean isTriangularSuperior() throws BadMatrixFormatException {
        ...
        for (int i = 1; i < m; i++)
            ...
            for (int j = 0; j < i; j++) if (!(array[i][j] == 0.0)) return false;
        return true; // the matrix has passed the test
    } // method isTriangularSuperior

    /**
     * Verifies if the matrix is triangular inferior or not. A triangular inferior matrix has zero
     * (0) values everywhere upper it's diagonal.
     *
    ...
    public boolean isTriangularInferior() throws BadMatrixFormatException {
        ...
        for (int i = 1; i < m; i++)
            // starts at index 1 because index (0,0) is on the diagonal
            for (int j = i; j < n; j++) if (!(array[i][j] == 0.0)) return false;
        return true; // the matrix has passed the test
    } // method isTriangularInferior()

    /**
     * Verifies whether or not the matrix is diagonal. A diagonal matrix only has elements on its
     * diagonal and zeros (0) at every other index. The matrix must be square.
     *
    ...
    /**
     * Returns the transpose of this matrix. The transpose of a matrix A = {a(i,j)} is the matrix B
     * = {b(i,j)} such that b(i,j) = a(j,i) for every i,j i.e. it is the symmetrical reflection of
     * the matrix along its diagonal. The matrix must be square to use this method, otherwise an
    ...
    /**
     * Gauss-Jordan algorithm. Returns the reduced-echeloned matrix of this matrix. The algorithm
     * has not yet been optimised but since it is quite simple, it should not be a serious problem.
     *
    ...
    public GIMatrix GaussJordan() {
        ...
        /*
         * From: LEROUX, P. Algebre lineaire: une approche matricielle. Modulo
         * Editeur, 1983. p. 75. (In French)
         */
    } // method GaussJordan()
    ...
    /**
     * Returns a matrix containing all of the diagonal elements of this matrix and zero (0)
     * everywhere else. This matrix is called the diagonal of the matrix.
     *
    ...
    /**
     * Returns the resulting matrix of an elementary linear operation that consists of inverting two
     * lines.
     *
    ...
    /**
     * Returns the resulting matrix of an elementary linear operation that consists of adding one
     * line, multiplied by some constant factor, to another line.
     *
     * @param i the first line number
    ...
    public GIMatrix add(GIMatrix b) {
        ...
        for (i = 0; i < m; i++)
            for (j = 0; j < n; j++) result.array[i][j] = array[i][j] + b.array[i][j];
        return result;
    }

    /**
     * Returns the result of the scalar multiplication of the matrix, that is the multiplication of
     * every of its elements by a given number.
     *
    ...
    /**
     * Returns the result of the matrix multiplication of this matrix by another one. The matrix
     * passed as parameter <i>follows</i> this matrix in the multiplication, so for an example if
     * the dimension of the actual matrix is mxn, the dimension of the second one should be nxp in
     * order for the multiplication to be performed (otherwise an exception will be thrown) and the
     * resulting matrix will have dimension mxp.
     *
    ...
    /**
     * Returns the determinant of this matrix. The matrix must be square in order to use this
     * method, otherwise an exception will be thrown. <i>Warning: this algorithm is very inefficient
     * and takes too much time to compute with large matrices.</i>
     *
    ...
    private double det(double[][] mat) {
        ...
        double temp =
                mat[0][0]
                        * det(M(mat, 0, 0)); // (-1)^(0+0)*m[0][0]*det(M(i,j)) ... first assignation
    ...
    /**
     * Returns the trace of this matrix, that is the sum of the elements of its diagonal. The matrix
     * must be square in order to use this method, otherwise an exception will be thrown.
     *
    ...
    //     * The double numbers are printed using the form "p/q" or "a b/c" depending on the value
    // of the boolean,
    //     * and use a minimal number of spaces as specified. It is the user's responsibility to
    // grant a number
    ...
    // In the case of the implementation of a table i.e. an array of matrices, verifies if the table
    // is proper.
