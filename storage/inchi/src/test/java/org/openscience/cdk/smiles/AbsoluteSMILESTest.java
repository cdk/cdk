...
public class AbsoluteSMILESTest {
    ...
    public void tetramethyltetraazatricyclooctane() throws Exception {
        test(
                "C[C@H]1N2N([C@@H](C)N3[C@H](C)N13)[C@H]2C",
    ...
    public void dbStereoCanonGeneration() throws Exception {
        ...
        Assert.assertEquals(
                cansmi.create(mol), cansmi.create(smipar.parseSmiles(cansmi.create(mol))));
    }
    ...
    static void test(String... inputs) throws Exception {
        ...
        for (String input : inputs) output.add(sg.create(sp.parseSmiles(input)));

        org.hamcrest.MatcherAssert.assertThat(
                Joiner.on(".").join(inputs) + " were not canonicalised, outputs were " + output,
                output.size(),
                is(1));
    }
}
