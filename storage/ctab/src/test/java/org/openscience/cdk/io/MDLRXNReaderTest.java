...
public class MDLRXNReaderTest extends SimpleChemObjectReaderTest {
    ...
    public void testReadReactions1() throws Exception {
        ...
        for (int i = 0; i < educts.getAtomContainer(0).getAtomCount(); i++) {
            Assert.assertEquals(
                    atomSymbolsOfEduct1[i], educts.getAtomContainer(0).getAtom(i).getSymbol());
        }
        ...
        for (int i = 0; i < products.getAtomContainer(0).getAtomCount(); i++) {
            Assert.assertEquals(
                    atomSymbolsOfProduct1[i], products.getAtomContainer(0).getAtom(i).getSymbol());
        }
    }
    ...
    public void testRDFChemFile() throws Exception {
        ...
        Assert.assertEquals(
                2, chemFile.getChemSequence(0).getChemModel(0).getReactionSet().getReactionCount());
        Assert.assertEquals(
                2,
    ...
    public void testRDFReactioniSet() throws Exception {
        ...
        Assert.assertEquals(
                3, reactionSet.getReaction(0).getReactants().getAtomContainer(0).getAtomCount());
        Assert.assertEquals(
                2, reactionSet.getReaction(0).getReactants().getAtomContainer(1).getAtomCount());
        Assert.assertEquals(2, reactionSet.getReaction(0).getProductCount());
        Assert.assertEquals(
                2, reactionSet.getReaction(0).getProducts().getAtomContainer(0).getAtomCount());
        Assert.assertEquals(
                2, reactionSet.getReaction(0).getProducts().getAtomContainer(1).getAtomCount());

        Assert.assertEquals(1, reactionSet.getReaction(1).getReactantCount());
        Assert.assertEquals(
                3, reactionSet.getReaction(1).getReactants().getAtomContainer(0).getAtomCount());
        Assert.assertEquals(1, reactionSet.getReaction(1).getProductCount());
        Assert.assertEquals(
                2, reactionSet.getReaction(1).getProducts().getAtomContainer(0).getAtomCount());
    }
    ...
    public void testAsadExamples() throws Exception {
        ...
        reader.close();
        Assert.assertEquals(
                reactionSet.getReaction(0).getMappingCount(),
                reactionSet2.getReaction(0).getMappingCount());
        for (int i = 0; i < reactionSet.getReaction(0).getMappingCount(); i++) {
            Assert.assertEquals(
                    getAtomNumber(
                            reactionSet, reactionSet.getReaction(0).getMapping(i).getChemObject(0)),
                    getAtomNumber(
                            reactionSet2,
                            reactionSet2.getReaction(0).getMapping(i).getChemObject(0)));
            Assert.assertEquals(
                    getAtomNumber(
                            reactionSet, reactionSet.getReaction(0).getMapping(i).getChemObject(1)),
                    getAtomNumber(
                            reactionSet2,
                            reactionSet2.getReaction(0).getMapping(i).getChemObject(1)));
        }
    }

    /**
     * Tells the position of an atom in a reaction. Format is
     * "reaction/product:numberofreaction/product_atomnumber".
     *
     ...
     */
    private String getAtomNumber(IReactionSet reactionSet, IChemObject chemObject)
            throws CDKException {
        for (int i = 0; i < reactionSet.getReaction(0).getReactantCount(); i++) {
            for (int k = 0;
                ...
                if (reactionSet
                        ...
                        .getAtomContainer(i)
                        .getAtom(k)
                        .equals(chemObject)) return "reactant:" + i + "_" + k;
            }
        }
        for (int i = 0; i < reactionSet.getReaction(0).getProductCount(); i++) {
            for (int k = 0;
                    k < reactionSet.getReaction(0).getProducts().getAtomContainer(i).getAtomCount();
                    k++) {
                if (reactionSet
                        .getReaction(0)
                        .getProducts()
                        .getAtomContainer(i)
                        .getAtom(k)
                        .equals(chemObject)) return "product:" + i + "_" + k;
            }
        }
