...
/**
 * Convert a CDK {@link IAtomContainer} to a Beam graph object for generating SMILES. Once converted
 * the Beam ChemicalGraph can be manipulated further to generate a standard-from SMILES and/or
 * arrange the vertices in a canonical output order.
 *
 * <p><b>Important:</b> The conversion respects the implicit hydrogen count and if the number of
 * implicit hydrogen ({@link IAtom#getImplicitHydrogenCount()}) is null an exception will be thrown.
 * To ensure correct conversion please ensure all atoms have their implicit hydrogen count set.
 *
 * <blockquote>
 *
 * <pre>
...
final class CDKToBeam {
    ...
    /** Whether to convert the molecule with isotope and stereo information - Isomeric SMILES. */
    ...
    /**
     * Convert a CDK {@link IAtomContainer} to a Beam ChemicalGraph. The graph can when be written
     * directly as to a SMILES or manipulated further (e.g canonical ordering/standard-form and
            ...
            for (IStereoElement se : ac.stereoElements()) {
                if (SmiFlavor.isSet(flavour, SmiFlavor.StereoTetrahedral)
                    ...
                    addGeometricConfiguration(
                            (IDoubleBondStereochemistry) se, flavour, gb, indices);
        ...
        try {
            switch (e) {
                ...
                default:
                    ...
                    if (isotope != null) return isotope.getMassNumber();
                    return null;
            }
    ...
    /**
     * Convert an CDK {@link IAtom} to a Beam Atom. The symbol and implicit hydrogen count are not
     * optional. If the symbol is not supported by the SMILES notation (e.g. 'R1') the element will
     * automatically default to UNKNOWN ('*').
     *
     * @param a cdk Atom instance
     * @return a Beam atom
     * @throws NullPointerException the atom had an undefined symbol or implicit hydrogen count
     */
    static Atom toBeamAtom(final IAtom a, final int flavour) {

        final boolean aromatic =
                SmiFlavor.isSet(flavour, SmiFlavor.UseAromaticSymbols)
        ...
        if (element == Element.Unknown) {
            ...
            ab.hydrogens(
                    checkNotNull(
                            hCount,
                            "One or more atoms had an undefined number of implicit hydrogens"));
        }
     ...
     * @return a Beam edge
     * @throws IllegalArgumentException the bond did not have 2 atoms or an unsupported order
     ...
     * @throws NullPointerException the bond order was null and the bond was not-aromatic
            ...
            default:
                if (!SmiFlavor.isSet(flavour, SmiFlavor.UseAromaticSymbols) && b.isAromatic())
                    throw new CDKException(
                            "Cannot write Kekulé SMILES output due to aromatic bond with unset bond order - molecule should be Kekulized");
     ...
     */
    private static void addGeometricConfiguration(
            IDoubleBondStereochemistry dbs,
            int flavour,
            GraphBuilder gb,
        ...
        if (SmiFlavor.isSet(flavour, SmiFlavor.UseAromaticSymbols)
                && db.getFlag(CDKConstants.ISAROMATIC)) return;
     ...
     * @param indices atom indices
     */
    private static void addTetrahedralConfiguration(
            ITetrahedralChirality tc, GraphBuilder gb, Map<IAtom, Integer> indices) {
     ...
     * @param indices atom indices
     */
    private static void addExtendedTetrahedralConfiguration(
            ExtendedTetrahedral et, GraphBuilder gb, Map<IAtom, Integer> indices) {
        ...
        gb.extendedTetrahedral(u)
                ...
                .neighbors(vs[1], vs[2], vs[3])
                .winding(
                        et.winding() == CLOCKWISE
                                ? Configuration.CLOCKWISE
                                : Configuration.ANTI_CLOCKWISE)
                .build();
    }

    private static void addExtendedCisTransConfig(
            ExtendedCisTrans ect,
            GraphBuilder gb,
            Map<IAtom, Integer> indices,
        ...
        if (ends != null) {
            ...
            else config = Configuration.DoubleBond.UNSPECIFIED;
            gb.extendedGeometric(indices.get(ends[0]), indices.get(ends[1]))
                    .configure(
                            indices.get(carriers[0].getOther(ends[0])),
