...
/**
 * Convert the Beam toolkit object model to the CDK. Currently the aromatic bonds from SMILES are
 * loaded as singly bonded {@link IBond}s with the {@link
...
final class BeamToCDK {
    ...
    /**
     * Base atom objects for cloning - SMILES is very efficient and noticeable lag is seen using the
     * IChemObjectBuilders.
     */
    ...
    /**
     * Base atom container for cloning - SMILES is very efficient and noticeable lag is seen using
     * the IChemObjectBuilders.
     */
    private final IAtomContainer emptyContainer;

    /**
     * Create a new converter for the Beam SMILES toolkit. The converter needs an {@link
     * IChemObjectBuilder}. Currently the 'cdk-silent' builder will give the best performance.
     *
     ...
     * @throws IllegalArgumentException the Beam graph was not 'expanded' - and contained organic
     *     subset atoms. If this happens use the Beam Functions.expand() to
     */
    IAtomContainer toAtomContainer(Graph g, boolean kekule) {
        ...
        for (int i = 0; i < g.order(); i++) {
            checkAtomStereo =
                    checkAtomStereo || g.configurationOf(i).type() != Configuration.Type.None;
            atoms[i] = toCDKAtom(g.atom(i), g.implHCount(i));
        }
        ac.setAtoms(atoms);
        // get the atom-refs
        for (int i = 0; i < g.order(); i++) atoms[i] = ac.getAtom(i);
        ...
        for (final Edge e : g.edges()) {
            ...
            // extension F[C@]=[C@@]F
                ...
                if (uConf.type() == Configuration.Type.DoubleBond
                    ...
                    if (uNbrs.length < 2 || uNbrs.length > 3) continue;
                    if (vNbrs.length < 2 || vNbrs.length > 3) continue;
                    ...
                    ac.addStereoElement(
                            new DoubleBondStereochemistry(
                                    ac.getBond(ac.getAtom(u), ac.getAtom(v)), bonds, conf));
                }
            }
        }
    }
    ...
    /**
     * Utility for find the first directional edge incident to a vertex. If there are no directional
     * labels then null is returned.
     *
    ...
    private Edge findDirectionalEdge(Graph g, int u) {
        ...
        for (Edge e : edges) {
            ...
            if (b == Bond.UP || b == Bond.DOWN) {
                ...
                else if (((first.either() == e.either()) == (first.bond() == b))) return null;
            }
        }
        return first;
    }
    ...
    /**
     * Creates a tetrahedral element for the given configuration. Currently only tetrahedral centres
     * with 4 explicit atoms are handled.
     *
     * @param u central atom
     * @param vs neighboring atom indices (in order)
     * @param atoms array of the CDK atoms (pre-converted)
     * @param c the configuration of the neighbors (vs) for the order they are given
    ...
    private IStereoElement newTrigonalBipyramidal(int u, int[] vs, IAtom[] atoms, Configuration c) {
        ...
        return new TrigonalBipyramidal(
                atoms[u],
    ...
    private IStereoElement newOctahedral(int u, int[] vs, IAtom[] atoms, Configuration c) {
        ...
        if (order < 1 || order > 30) return null;
        return new Octahedral(
                atoms[u],
                new IAtom[] {
                    atoms[vs[0]],
                    atoms[vs[1]],
    ...
    private IStereoElement newExtendedTetrahedral(int u, Graph g, IAtom[] atoms) {
        ...
        Stereo stereo =
                g.configurationOf(u).shorthand() == Configuration.CLOCKWISE
                        ? Stereo.CLOCKWISE
                        : Stereo.ANTI_CLOCKWISE;

        return new org.openscience.cdk.stereo.ExtendedTetrahedral(
                atoms[u],
                new IAtom[] {atoms[xs[0]], atoms[xs[1]], atoms[xs[2]], atoms[xs[3]]},
                stereo);
    }
    ...
    /**
     * Create a new CDK {@link IAtom} from the Beam Atom. If the element is unknown (i.e. '*') then
     * an pseudo atom is created.
     *
    ...
    /**
     * Create a new atom for the provided symbol. The atom is created by cloning an existing
     * 'template'. Unfortunately IChemObjectBuilders really show a slow down when SMILES processing.
     *
