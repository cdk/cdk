...
/**
 * Iterating SMILES file reader. It allows to iterate over all molecules in the SMILES file, without
 * being read into memory all. Suitable for very large SMILES files. These SMILES files are expected
 * to have one molecule on each line. If a line could not be parsed and empty molecule is returned
 * and the property {@link #BAD_SMILES_INPUT} is set to the attempted input. The error is also
 * logged.
 *
...
public class IteratingSMILESReader extends DefaultIteratingChemObjectReader<IAtomContainer> {
    ...
    /**
     * Constructs a new IteratingSMILESReader that can read Molecule from a given InputStream and
     * IChemObjectBuilder.
     *
