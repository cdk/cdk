...
/**
 * A factory for creating ChemObjectReaders. The type of reader created is determined from the
 * content of the input. Formats of GZiped files can be detected too.
 *
...
public class ReaderFactory {
    ...
    /**
     * Detects the format of the Reader input, and if known, it will return a CDK Reader to read the
     * format, or null when the reader is not implemented.
     *
        ...
        if (input instanceof GZIPInputStream) {
            ...
            if (reader != null) {
                try {
                    ...
                    IOException wrapper =
                            new IOException(
                                    "Exception while setting the InputStream: " + e1.getMessage());
    ...
    /**
     * Detects the format of the Reader input, and if known, it will return a CDK Reader to read the
     * format. This method is not able to detect the format of gziped files. Use
     * createReader(InputStream) instead for such files.
     *
     * @see #createReader(InputStream)
     */
